...
**简称：** SFL-TDCRMC
**版本：** v15.0 Execution–Entitlement Final Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v15 Execution–Entitlement Final Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为 Execution–Entitlement Final Closure（P10 theme-day key、entitlement state/NAV、TotalNetProceeds 状态过滤、ExecutionAllocatedBudget、provisional/final AllIn、Gate8 Intent 强制保留、Gate9 unresolved pair、odd-lot 执行分支、lock owner）；不新增题材指标、角色、状态、对照、Gate 或退出腿。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v15.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
- **总回报（v15 唯一，Gate 8A/8B/9 共用）：**
  \(BuyNetCost\) = 生命周期实际净买入成本（冲击单次计入）；
  \(TotalNetProceeds\) = 卖出净所得
  + 已结算公司行动现金流（`accounting_status=SETTLED` 后实际 Cash / 股份最终处置所得）
  + `UnsettledEntitlementTerminalValue`（**仅** `accounting_status=RECOGNIZED` 的 receivable 按 §15.5.2）
  − 后续税款追缴（`DIV_TAX_CLAWBACK`）；
  **状态过滤（强制）：** `LEGAL_ONLY` **不得**单独计入终值（其价值仍在含权 PositionValue / 含权卖出价中）；`SETTLED` **不得**再加 receivable terminal value（只走实际处置）；
  \(R^{net} = TotalNetProceeds / BuyNetCost - 1\)；
  卖出成本基础使用 §15.3.1 **AvgCost**；公司行动按 trade_lot → parent 比例分摊（§15.5.3）；
  研究截止时若仍有 `RECOGNIZED` stock receivable 未结算：其终值按 §15.5.2 计入 `TotalNetProceeds`（与 Gate 8A 可评估），**不得**另标 `NON_EVALUABLE_EXECUTION`（该码仅用于 `SYNTHETIC_TERMINAL_MARK`）。
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v15.0"）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v15(theme, D, ...)`：
...
- **StockPositionLock（v15）：** 只要股票满足任一：`CurrentShares>0` **或** 存在开放买入母订单 **或** 存在未完成卖出母订单，则其他 episode **禁止**对该股票创建新 ParentOrder（登记 `REJECT_DUPLICATE_STOCK`）。同日竞争按 §15.1 排序，仅最优者可创建唯一 ParentOrder；其余不创建 ReservedCash、不消耗入场权（有效期内若 lock 全释放仍可重试）。原 parent 的 D+2/D+3 剩余重试**不**视为新 parent。
  **显式 owner 字段（强制）：** `lock_owner_parent_order_id`、`lock_owner_episode_id`、`acquired_at`、`release_reason`。09:35 检查：若 `lock_owner_parent_order_id == current.parent_order_id` → **允许**该母订单继续执行/重试；否则若 lock 被占用 → `REJECT_DUPLICATE_STOCK`。禁止因“存在开放买单”而拒绝 lock owner 自身。这落实“首次入场后不加仓”与单一 origin 归属。
...
**分红税：** 税率 = `tax_rate(holding_days, board, tax_rule_version)`，PIT 取 `payment_date` 可知版本；缺省在 payment 开盘前对 **entitlement lot** 预扣；仅当税表要求卖出追缴时登记 `DIV_TAX_CLAWBACK`。
...
### 15.5.1 CorporateActionEntitlementLot（v15 唯一）
...
### 15.5.2 Receivable 估值与税（v15 唯一）

**现金股息税口径（二选一已冻结为本款）：**

    CashReceivable = receivable_cash_gross   # 税前总额
    TaxPayable     = expected_tax            # 预期税款

**禁止**再对 CashReceivable 做 `(1-tax)` 折现同时又生成 TaxPayable（双计税）。`expected_tax` 使用 **ex-date 当日 `INPUT_SNAPSHOT_CUTOFF` 前可知**的 `tax_rule_version`；**禁止**用 payment_date 后来才可知的税制回写 ex-date NAV。若 payment 前税制变更：当日向前 remasure `TaxPayable` / receivable，登记 `TAX_RULE_REMEASURE`，不得回写历史日。

|类型|RECOGNIZED 起估值|
|---|---|
|CashReceivable|`receivable_cash_gross`（毛额）；配套 `TaxPayable=expected_tax`|
|StockReceivableValue|每日收盘 **mark-to-market**：`receivable_stock_shares × P_close,t`；证券为 successor 时用 successor 收盘价；停牌用最近收盘；缺失 → INVALID_DATA。ex-date 当日初值可用除权参考价，此后每日按收盘重估|
...
- **版本 α-wealth：** 正式 PASS 预算按**实际解封 HOLDOUT 的版本序号** k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)。**HOLDOUT 触碰日志（v15 唯一声明）：** v1–v14 均未解封 HOLDOUT，已消耗集合为空；**本 Canonical `SFL-1_v15.0` 为 k=1，\(\alpha_v=0.025\)**。后续任一新 SPEC 若解封 HOLDOUT 必须取下一 k，禁止重置；不得并行登记多个“当前 k=1”；
...
|`TheoreticalOrderIntent`|**凡**已完成 `IntendedTargetValue` 计算的对象**必须**物化（含 `REJECT_DUPLICATE_STOCK`、`REJECT_MIN_WEIGHT`、`REJECT_PORTFOLIO_FULL` 及后续执行拒绝）；禁止由实现者选择是否保留|Gate 8B / θ_ADV|
|`ParentOrder`|取得 StockPositionLock 并创建的正式母订单|Gate 8A、ParentFillRate|

**Population 纪律（强制）：** 完成 TargetValue 计算 ⇒ 必须创建 Intent；未创建 Intent 的路径不得声称进入 Gate 8B/θADV。禁止“若保留 / 若不保留则排除”的自由裁量。

### Gate 8A：Conditional Execution Court（正式主 estimand）

- **Population_fill：** 有效期内至少一笔非零成交的 `ParentOrder`；
- **Outcome：** NetAlpha20 = §0.3 `TotalNetProceeds` 口径；
- **Null / MES：** 见附录 B；进入 §18.0 IUT 主终局；
- **禁止**把未成交记 0 后并入本样本。

### Gate 8B：Execution Policy Diagnostic（强制政策诊断，非第二主 Gate）

- **Population_intent：** **全部** `TheoreticalOrderIntent`；未创建 Parent 或未成交 ⇒ `PolicyReturn=0`；
- **Outcome：** `PolicyReturn(s) = RealizedNetPnL(s) / IntendedTargetValue(s)`（已创建 Parent 时 IntendedTargetValue ≡ ParentOrderTargetValue）；
  RealizedNetPnL 使用与 §0.3 同一 `TotalNetProceeds` 函数；
  **禁止**把部分成交上的 NetAlpha20 直接当作 PolicyReturn；
- `ParentFillRate` 分母 = 全部正式 `ParentOrder`；
- 正式汇总：`PolicyNetAlpha20 =` Intent 等权均值；与 ParentFillRate 同列首页（§20.6）；
...
1. 隔离账户固定 `IntendedTargetValue`；
2. **忽略**生产组合 `Cash` / `GrossRoom` / `ThemeRoom` / `StockRoom` / **StockPositionLock**；actual 与 no-gap 的**唯一**差异是是否施加 `REJECT_GAP`；其余市场执行、涨跌停、参与率、费用同构；
3. 成交用 TotalNetProceeds 口径 NetAlpha20，未成交为 0；
4. \(\theta_{ADV,s}=Outcome_{no\_gap}(s)-Outcome_{actual}(s)\)；正式 estimand = Intent 等权 \(\theta_{ADV}\)；非劣界 1 个百分点；
5. **禁止**完整生产组合逐信号 rerun（`DIAGNOSTIC_PORTFOLIO_RERUN`）；
6. guard PASS：\(\theta_{ADV}\) 单侧 \(1-\alpha_v\) 上界 < 0.01；
7. guard FAIL：Bonferroni 单侧 \(1-0.05/(9\cdot m)\)（m=2）下界 > 0.01 → `FAIL_ADVERSE_SELECTION`；
8. 其余 `INCONCLUSIVE_ADVERSE_SELECTION` → Gate 8 INCONCLUSIVE；
9. **Gate 8 IUT** =（8A NetAlpha20）与（\(-\theta_{ADV}\)）；8B 不进 IUT。
...
**科学问题选择（v15 Canonical）：ParentOrder 配对退出增量。** 不构造共享有限现金组合，不回答端到端资金政策。
...
**推断装置（v15）：** parent 层观测；ID1 = persistent component；ID2 = 该 parent 首笔 fill 月；主推断 = 题材簇×月 CGM；稳健性 = persistent-component 整块 bootstrap（seed 表 `G9_COMP_BOOT`，默认 5,000，可升 50,000）。

**未清算配对资格（v15 强制）：** 正式 outcome 截止时，若 dynamic arm 与 fixed20 arm **任一**仍存在未清算仓位，或任一侧带 `SYNTHETIC_TERMINAL_MARK`，则该 pair 记 `G9_NON_EVALUABLE_EXECUTION`，**整对剔除**（两臂均不进 θExitMean/θExitComp）。**禁止**单侧 synthetic mark、单侧估值收益、或无限等待仍计入样本数。必须强制报告 `UnresolvedRate_dynamic` 与 `UnresolvedRate_fixed20`；若动态臂未清算率显著低于固定臂，登记 `G9_SELECTION_UNRESOLVED`（诊断，不自动改写 PASS，但阻断宣称“样本对退出政策无选择偏倚”）。
...
### P10_NULL_EPISODE（v15 Stable Episode Generator）
...
年度覆盖（v15 theme-day + episode-day 公式）：令

    RealThemeDayKey = (real_theme_id, D)
    RealEvaluableThemeDays_y = # unique RealThemeDayKey 于年 y 且该题材日可评估
    PseudoEpisodeDays_y = Σ_e（e 在年 y 内且 P10_EVALUABLE_EPI(e)=1 的可评估运行交易日数）
    CoveredRealEvaluableThemeDays_y = # unique RealThemeDayKey，存在可评估 P10 episode e
      满足 e.real_theme_id = key.real_theme_id 且 e 在 D 日运行

要求：

    若 RealEvaluableThemeDays_y = 0 → 直接 `P10_INSUFFICIENT_YEAR`，且**不**检验该年 AnnualFalseEpisodes
    否则：
      PseudoEpisodeDays_y / RealEvaluableThemeDays_y >= 40
      且 CoveredRealEvaluableThemeDays_y / RealEvaluableThemeDays_y >= 0.90

否则 `P10_INSUFFICIENT_YEAR`（40 = 每真实可评估题材日最低可评估伪 episode-日暴露，= 200 controls × 0.20，冻结于 §27）。**禁止**按自然交易日去重覆盖（同日题材 A 覆盖不使题材 B 被覆盖）；**禁止**用新 spawn 数惩罚长寿 episode。达标前不得报告 AnnualFalseEpisodes、不得用于 Phase-I/漂移。
...
**真实交易日 D 的唯一顺序（v13）：**
    ...
    D 15:00以后（信号日）
      ...
      ├─ load_or_build_nulls_v15；Gate1 冻 DISCOVERY_RESEARCH_* 
...
# 25. 核心伪代码
    ...
    for D in trading_days:
        ...
        set_commission_reserves_from_execution_allocated(eligible_buy_orders)
        reallocate_cash_at_0935(eligible_buy_orders, PARAMS)
        # 静态约束：执行器只接收 eligible；rejected fill_qty 必须为 0
        ...
        assert_execution_allocated_le_reserved(eligible_buy_orders)
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v15(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v15(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                ...
                m["nulls"] = load_or_build_nulls_v15(
...
|版本 α-wealth|0.05×2^{-k}（v15: k=1, α_v=0.025；HOLDOUT 从未消耗）|—|治理|—|
...
本规格的 Canonical 纪律：
    ...
    TotalNetProceeds 仅 RECOGNIZED 终值；CashReceivable 毛额 + TaxPayable；
    StockReceivable 每日 MTM；ex 不增实际股份；P10 RealThemeDayKey；
    ExecutionAllocatedBudget 不含 FreeCash；Provisional→Final AllIn 只一轮；
    Intent 强制保留；θADV 忽略 StockPositionLock；G9 未清算整对剔除；
    lock_owner 字段；odd-lot 卖出分支；α-wealth v15 k=1；路径 /sfl1_v15/。
...
|9 退出|Track S；固定买入 tape；ParentOrder 配对|parent 首笔 fill 月|θExitMean 与 θExitComp（component 等权公式）|任一≤0|各0.01 / 0|persistent簇×月 CGM；稳健性 G9_COMP_BOOT|固定顺序第9；m=2 IUT|仅Gate1–8 PASS后；HOLDOUT一次|禁用 ΔSharpe/21日块；PASS 仍需动态 §20.5(2–8)|
...
**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v15 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] TotalNetProceeds 状态过滤；CashReceivable 毛额+TaxPayable；StockReceivable 每日 MTM；ex 不增股份
- [ ] P10 RealThemeDayKey；% real-theme-days；denominator=0 → INSUFFICIENT_YEAR
- [ ] ExecutionAllocatedBudget 无 FreeCash；Provisional→Final AllIn；CommissionTopUp
- [ ] Intent 强制物化；θADV 忽略 rooms/StockPositionLock；仅隔离 REJECT_GAP
- [ ] Gate9 G9_NON_EVALUABLE_EXECUTION 整对剔除；两臂未清算率报告
- [ ] lock_owner_* 字段；odd-lot 卖出分支不做 100 取整；eligible_buy_orders 静态检查
- [ ] α-wealth：v15 k=1；路径 /sfl1_v15/；spec_version=SFL-1_v15.0
...
## 执行与治理
- [ ] 账本恒等式零失败
- [ ] Run-in 20日完成，并覆盖黄金路径：同日题材A有P10而B无 → B 不计入 Covered；RealEvaluableThemeDays=0 → INSUFFICIENT_YEAR；ex 日 StockReceivable 不增 SellableQty、payment 才增股；CashReceivable 毛额+TaxPayable 无双计税；LEGAL_ONLY 不进 TotalNetProceeds；Reserved=60万且 FreeCash=0 时仍可执行；Provisional VWAP all-in → 一次 Final；REJECT_MIN_WEIGHT/DUPLICATE 必须进 Intent；θADV 忽略 StockPositionLock；lock owner 09:35 不自拒；dynamic 清算而 fixed20 odd-lot pending → G9 整对剔除；odd-lot 卖出参与量不做100取整
