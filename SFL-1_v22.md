...
**简称：** SFL-TDCRMC
**版本：** v22.0 Monetary Quantization Final Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v22 Monetary Quantization Final Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为 Monetary Quantization Final Closure（SettledEstimatedTax 人民币分整数化、disposed baseline 的 floor 分配、remaining 尾差守恒、禁止浮点与未定义舍入）；不新增题材指标、角色、状态、对照、Gate 或退出腿。已 PASS 的 proportional split 结构、TaxTrueUp 过账/幂等及更早闭包维持不变。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v22.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
- **总回报（v22 唯一，Gate 8A/8B/9 共用）：**
  \(BuyNetCost\) = 生命周期实际净买入成本（冲击单次计入）；
  **`UnsettledEntitlementTerminalValue_p`（仅 RECOGNIZED，净额唯一公式；v19 parent-specific）：**
  \[
  \sum_{e\in\mathrm{CashRecog}_p}(\mathrm{CashReceivable}_{e,p}-\mathrm{TaxPayable}_{e,p})
  +\sum_{e\in\mathrm{StockRecog}_p}\mathrm{StockReceivableValue}_{e,p}
  +\sum_{e\in\mathrm{RightsRecog}_p}\mathrm{RightsReceivableValue}_{e,p}
  \]
  其中 `CashReceivable_(e,p)` / `TaxPayable_(e,p)` 必须按 §15.5.3 由 tax-slice 的 `parent_order_id` 归集；**禁止**用 entitlement 总净额按 `source_trade_lot_ids` 股数再摊；
  `LEGAL_ONLY`→0；`SETTLED` receivable terminal→0；
  **科目边界（强制，防双计）：**
  `SellNetProceeds_p` = parent 归属的**全部**实际股份市场卖出净所得（**包括** SETTLED stock entitlement 生成后卖出的股份）；
  `SettledCorporateActionCash_p` = **仅**非市场卖出现金流：现金股息（按 `SettledDividendCash_(e,p)`）、退市现金、cash-in-lieu、其他 PIT 法定现金结算；
  **禁止**将 SETTLED 股票权益股份的市场卖出款再次计入 `SettledCorporateActionCash`；
  \(TotalNetProceeds_p = SellNetProceeds_p + SettledCorporateActionCash_p + UnsettledEntitlementTerminalValue_p - TaxTrueUp_p\)；
  **`TaxTrueUp_p`** = Σ 归属 parent p 的 slice 级 `TaxTrueUp_(e,s)`（§15.5.2）：正值为 clawback（减收益），负值为 reversal/refund（加收益）；禁止只实现正向追缴；禁止与已结算 `SettledEstimatedTax` 双减；
  **现金过账（强制）：** 每个 disposed slice 的 true-up 在卖出成交后原子执行 `Cash_fen -= TaxTrueUp_fen_(e,s)`（负值即增加 Cash），并计入当日 NAV / FreeCash；金额单位为人民币分（§15.5.2）；**禁止**仅在研究 outcome 调整而不改 Cash；**禁止**将 TaxTrueUp 再计入 `SettledCorporateActionCash_p`（防双计）；幂等键与 POSTED 状态见 §15.5.2；
  \(RealizedNetPnL = TotalNetProceeds - BuyNetCost\)；
  \(R^{net} = TotalNetProceeds / BuyNetCost - 1\)；
  卖出成本基础使用 §15.3.1 **AvgCost**；分摊见 §15.5.3；
  研究截止时仍有 RECOGNIZED stock receivable：按上式净终值计入（Gate 8A 可评估）；`SYNTHETIC_TERMINAL_MARK` 仍 → `NON_EVALUABLE_EXECUTION`。
...
2. **同日可执行对照（v13 唯一）：** 对照篮子**仅从真实信号的 \(\tau_{entry}\) 当日**开始尝试买入与流式参与；在真实 \(\tau_{entry}\) 之前的任何尝试日，对照只做 09:35 可成交性诊断登记（`CONTROL_FILLABLE_BEFORE_ENTRY`），**禁止**提前建仓或扣减生命周期预算；
3. 自真实 \(\tau_{entry}\) 起：对照成员按 §14.1 人民币母订单完整算法执行；若真实信号在 \(\tau_{entry}\) 后仍有 D+2/D+3 追补尝试，对照在相同尝试日同构重试并继承 LifecycleRemainingBudget；对照不得在真实未尝试日自行成交；
4. **篮子 TargetValue：** 整个对照篮子总目标金额 = 配对真实信号的单笔 `ParentOrderTargetValue`；成员等权拆分人民币；禁止每只对照各获一份；
5. 全部拒绝判定同构适用；
6. REJECT_GAP：与真实信号相同状态、对照股自身板块的同一阈值表；
7. 决策被拒成员从篮子剔除且额度不重分；
8. **资本同构（v13 条件成交主终点）：** 真实侧与对照侧均按**实际成交净成本**归一化收益；未成交现金**不进入**收益分母（仅影响 \(C^{valid}\)/FillRate 诊断）。篮子收益 =
   \(\sum_j(\text{成交净成本}_j\times R^{net}_j)\,/\,\sum_j\text{成交净成本}_j\)（仅对成交成员 j）；**禁止**用 ParentOrderTargetValue 稀释对照而真实侧不稀释；
9. 卖出：与真实信号同一卖出日（共享退出时钟），§14.3 + T+1 + §14.4 同构；持有计数自真实 \(\tau_{entry}\) 起；
10. \(C^{valid}\) = 成交成员非零的篮子；\(|C^{valid}|<100\) → episode 层主终点 `NON_EVALUABLE`；
11. **禁止**“对照提前买入却共享处理侧退出时钟”的实现。
...
### 3.5.1 FrozenMetricBackcast（v13；确认量尺永冻）
...
**三时钟分离（时区均为 Asia/Shanghai）：**
    ...
    PREOPEN_ACTION_CUTOFF(D) = PreviousTradingDay(D) 20:00

- D 日信号链只能读取 \(available\_at \le\) `INPUT_SNAPSHOT_CUTOFF` 的记录；20:00 后到达的记录 `available_at` 一律记为下一交易日，不得进入 D 日任何计算；
- **公司行动 / 开盘前账本（v17 强制）：** D 日开盘前生效的 entitlement 确认、税率、股份调整、StockPositionLock 迁移，只能读取 `available_at ≤ PREOPEN_ACTION_CUTOFF(D)`；**禁止**用 D 日 20:00 `INPUT_SNAPSHOT_CUTOFF` 信息处理 D 日开盘前账本（否则构成时点未来信息）；
- 输入快照在 20:00 冻结并哈希；计算必须在 `COMPUTE_DEADLINE` 前完成（超时处置见 §8.3.9）；信号在 `ARCHIVE_DEADLINE` 前只增不改归档；
- 时钟顺序为不变量：快照冻结后的任何数据到达都不得触发当日重算。两个实现对同一到达序列必须产生相同输入集。
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v22.0"）
...
## 6.10 TradingDays 持有期算子（全文唯一；v18/v19）
...
起点取得日不计、终点计入；统一交易所日历；单券停牌不剔除日历交易日；细则与税款调用见 §15.5.2。禁止另定义并行持有期计数。
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v22(theme, D, ...)`：
...
**NON_EVALUABLE 冻结范围（v14 唯一）：** 题材日为 `NON_EVALUABLE`（§8.3.4/8.3.5/§9.2.3）时：

- **冻结：** IES / IDS / GCC / RMI / CapacityBreadth / HHI 驱动的**新**正常状态转移，以及一切新入场信号；
- **不冻结（必须继续）：** 固定20日入口退出；Leg C；已触发退出腿的升级；`LACDecay=1` → COLLAPSE（Leg X）；停牌与执行状态更新；既有卖单与升级（§14.3）；T+1；安全 Kill；账本恒等式；
- 登记 `NON_EVALUABLE` 事件。伪代码 `update_desired_exit_state_only` **必须**消费时间型退出与已触发腿，不得仅消费 LAC。
...
## 12.3.1 ConfirmedEventFlag（v13 吸收事实）

一旦 \(\tau_{confirm}\) 在归档临时量尺下成立：

    ConfirmedEventFlag_u := 1

此后该标志**永久为 1**，不得因 FrozenMetricBackcast、量尺切换或后续 IDS 斜率变化撤销。

**确认事实 ≠ 每日扩张健康：**

- `ConfirmedEventFlag` = 历史事件（吸收）；
- `CurrentExpansionHealth` = 确认后用冻结 \(B^E/L^E\) + EPISODE_NULL 计算的当日 IES/IDS/GCC/RMI/Breadth。

## 12.4 EARLY_CAPACITY

    ConfirmedEventFlag = 1
    GCC^θ 启动（§11.3，level 尺度；冻结量尺）
    CapacityBreadth^E >= 0.40
    RMI^E >= 0 或处于Co-expansion
    未过度拥挤（GCC^θ <= 0.85）

**禁止**在 EARLY_CAPACITY（及之后状态）每日重判“CONFIRMED_EXPANSION 前三行 / 确认 IDS 斜率”；确认斜率只用于创建 `ConfirmedEventFlag`。允许入场。

## 12.5 HEALTHY_TRANSFER

    ConfirmedEventFlag = 1
    冻结量尺下：IES >= 0.80 且 IDS^θ 较过去3日最高值下降不超过0.10
    GCC^θ 启动
    RMI^E > 0
    Δ3 EMA3(CapacityBreadth^E) > 0

允许入场或继续持有。确认后 IDS/IES 历史窗不足时，用已冻结量尺下可得日计算；不得回拼临时 DAILY_NULL 确认历史。
...
- **开盘涨停拒单（v13 唯一）：** 当且仅当 09:30—09:34 五分钟**始终**满足 \(Low_m = up\_limit\)（连续无量/无开板封死）→ `UNFILLED_LIMIT`；若 09:30 以涨停价开盘但 09:30—09:34 任一分钟出现 \(Low_m < up\_limit\)（已开板），则**不得**因此拒单，应从开板后首个可参与分钟起流式参与；
...
**人民币母订单与流式参与（v13 唯一；09:35–10:00，共 25 根分钟线）：**
母订单以人民币为意图单位，字段：

    ParentOrderTargetValue
    LifecycleRemainingBudget   # 生命周期剩余预算（跨日）
    IntradaySpendableBudget    # 当日可花费 = Lifecycle − CommissionReserve
    CommissionReserve
    CumLifecycleNetBuyCost     # 累计实际净买入成本（含日终佣金）
    signal_id / parent_order_id

**禁止**把 TargetValue 固定为不可变 TargetQty 作为跨日母目标。

**预算对象拆分（v15）：**

1. 创建时：`LifecycleRemainingBudget = ParentOrderTargetValue`；`CumLifecycleNetBuyCost = 0`；
2. 每个执行日 09:35 完成拒绝判定与再分配后，该订单已获配现金唯一由 `ReservedCashAfter0935` 代表（已从 FreeCash 扣除）。定义：
   `ExecutionAllocatedBudget_t = min(LifecycleRemainingBudget, ReservedCashAfter0935, CurrentStockRoom, CurrentThemeRoom, CurrentGrossRoom, CurrentCapacityRoom)`（金额口径）；
   **禁止**将 `CurrentFreeCash`（= Cash − ΣReserved）放入同一 min——否则充分预留订单预算被压为 0。
   组合恒等式：`Σ ReservedCashAfter0935 ≤ Cash`。若需把尚未预留的 FreeCash 追加给订单，必须**先**完成追加分配写入该订单 ReservedCash，再计算本式。
3. `CommissionReserve = minimum_commission_cny if ExecutionAllocatedBudget_t>0 else 0`；
   `IntradaySpendableBudget = ExecutionAllocatedBudget_t - CommissionReserve`（≤0 → 当日不可买，`BUDGET_RESERVE_BLOCK`）；
4. **LifecycleRemainingBudget 是剩余意图，不是当日可执行金额**；当日成交上限由 `ExecutionAllocatedBudget_t` 控制；
5. 预留不侵蚀生命周期预算：无成交日终释放 Reserve 时 Lifecycle 不变；
6. 日终：`DayOrderValue` = 当日执行价名义之和；`ProvisionalCommission` = 已逐分钟计提比例佣金之和；
   `DayCommission = max(DayOrderValue×commission_rate, minimum_commission_cny if DayOrderValue>0 else 0)`；
   `CommissionTopUp = max(DayCommission - ProvisionalCommission, 0)`（**只补正差额，禁止再加一整份 DayCommission**）；
   无成交则佣金=0；更新 `CumLifecycleNetBuyCost` 与 `LifecycleRemainingBudget`；
7. **CommissionTailAdjust：** 若 TopUp 后超 ParentTarget，仅减最后一分钟 fill 并重算该分钟一次；禁止回改更早分钟；
8. 跨日重试继承 Lifecycle；次日从真实剩余意图与新获配预算重新预留；
9. 预算闭包：生命周期净买入成本 ≤ ParentOrderTargetValue。

每分钟 m（顺序冲击 + 单次计入 + provisional→final all-in，**只一轮**）：

令 `variable_fee_rates = commission_rate + transfer_fee_rate + exchange_fee_rate + buy_stamp_tax_rate`（当日 `fee_schedule.yaml`）。

1. 若 \(Low_m=up\_limit\) → 可参与=0；否则 `particip_qty = floor(0.05×vol_m/100)×100`；
2. **ProvisionalAllInUnitCost（v15 唯一初值）：**
   `ProvisionalAllInUnitCost_m = VWAP_m × (1 + variable_fee_rates)`（**无冲击**；禁止用尚未存在的 \(P^{ex}\)）；
3. `qty0 = min(particip_qty, floor(IntradaySpendableBudget / ProvisionalAllInUnitCost_m / 100)×100)`（VWAP=0 或 provisional=0 → 不成交）；
   `TentativeValue_m = qty0 × VWAP_m`；
4. `CumOrderValue_{m-}` = 当日至上一分钟累计执行价名义；`OrderValue_m^{impact}=CumOrderValue_{m-}+TentativeValue_m`；
5. `ImpactBps_m` 按 §14.4；买入 \(P^{ex}_m=VWAP_m×(1+ImpactBps_m/10^4)\)；
6. **FinalAllInUnitCost_m = \(P^{ex}_m\) × (1 + variable_fee_rates)**；
   `max_qty = floor(IntradaySpendableBudget / FinalAllInUnitCost_m / 100)×100`；`fill_qty=min(max_qty, particip_qty)`；
   **禁止**在本分钟对 Impact/Pex/AllIn 继续迭代；
7. 成交记账价=\(P^{ex}_m\)；再加比例佣金与法定税费；**Impact Single-Count**；
8. `IntradaySpendableBudget -=` 该分钟净成本；不得为负；
9. 至 10:00 或 IntradaySpendableBudget < 一手 FinalAllIn；全部分钟可参与0 → UNFILLED_LIMIT；
10. 部分完成 → PARTIALLY_FILLED / EXPIRED_REMAINDER，跨日继承生命周期预算；有效期尽 → 余额作废。
...
3. 14:30–15:00 逐分钟参与：分钟 \(High_m=down\_limit\) 可参与量为 0，否则：
   - **常规 round-lot：** `participation_qty = floor(0.05×vol_m/100)×100`；
   - **最终 odd-lot 清算分支（v15）：** 若 `final_liquidation=1` 且 `0 < remaining_qty < 100`，则
     `participation_qty = min(remaining_qty, floor(0.05×vol_m))`（**禁止**再向下取整到 100；若共享通用 round-lot 函数必须绕过）；
   成交数量 ≤ 剩余可卖；**成交记账价 =** \(VWAP_m×(1-ImpactBps_m/10^4)\)（冲击单次计入，与 §14.1/§14.4 同构）；
...
- **StockPositionLock（v18）：** 只要证券（或其 `SecurityIdentityLockGroup` 内任一 alias）满足任一：
  `CurrentShares>0` **或** 开放买入母订单 **或** 未完成卖出母订单 **或** `OutstandingShareGeneratingEntitlement>0`，
  则其他 episode **禁止**在该 LockGroup 任一 alias 上创建新 ParentOrder（`REJECT_DUPLICATE_STOCK`）。
  **`OutstandingShareGeneratingEntitlement`** = 会转化为实际股份的权益：`LEGAL_ONLY` STOCK_DIVIDEND/TRANSFER、`RECOGNIZED` StockReceivable、未结算换股/successor share entitlement。**现金股息不占用**本锁。
  同日竞争按 §15.1；仅最优者成为 owner；loser 不创建 ReservedCash、不消耗入场权；D+2/D+3 重试不视为新 parent。
  **显式 owner：** `lock_owner_parent_order_id`、`lock_owner_episode_id`、`acquired_at`、`release_reason`。09:35：owner 自身允许继续；他人占用 → `REJECT_DUPLICATE_STOCK`。
  **LockGroup 形成时点（唯一）：** `LockGroupFormationPreOpen` = 公司行动记录首次同时满足：
  (i) `available_at ≤ PREOPEN_ACTION_CUTOFF(D)`；
  (ii) `successor_security_id` 非空且已法定识别；
  的交易日 D 之开盘前。自该时点起 predecessor 与 successor **立即**共享同一 LockGroup（禁止等到 effective 才形成）。
  **CODE_CHANGE / ABSORPTION（原子迁移）：** effective 开盘前将 predecessor 的持仓、entitlement、成本、PendingSell、**StockPositionLock 与全部 owner 字段**一次性迁移至 `successor_security_id`。
  **Owner collision 政策（方案 B，强制）：** LockGroup 形成时，若跨 alias 已存在 **≥2** 个不同的 `lock_owner_parent_order_id`（或 successor 在 CODE_CHANGE 下已有独立 owner/持仓），登记 `CORP_ACTION_LOCK_COLLISION` 并判 **`LEDGER_INVALID`**——该次运行不得产生正式 Gate 判词；**禁止**实现者选择保留哪一个 owner、合并 origin 或临时 multi-owner 子账。无碰撞时单 owner 随迁移原子转移到 successor。
  **释放条件：** LockGroup 内全部 alias 均无 CurrentShares、OpenBuy、PendingSell、OutstandingShareGeneratingEntitlement。
  **`release_reason` 枚举：** `RELEASE_NO_FILL_EXPIRED` | `RELEASE_FULL_EXIT` | `RELEASE_ENTITLEMENT_VOID` | `RELEASE_LEGAL_CASH_SETTLEMENT` | `RELEASE_CORPORATE_SUCCESSOR_CLOSED` | `RELEASE_CANCELLED_BEFORE_FILL`。
- 组合已满时后续信号进入有效期重试，不跨有效期排队。

**题材簇上限时点语义（v14 唯一）：** ThemeRoom 在**入场分配时**按当日 §5.3 连通分量计算（entry-time constraint），并按 origin 永久归属。若持仓后因连通分量动态合并导致某 persistent/当日簇合计暴露 >20%：
...
CashReceivable_t
+
StockReceivableValue_t
+
\sum_i PositionValue_{i,t}
-
TaxPayable_t
\]
...
**FreeCash 只含已结算 Cash，禁止包含任何 receivable。** `ReservedCash` 是现金子账，不重复计入 NAV。每个交易日收盘后运行恒等式检查，容差为 NAV 的 \(10^{-10}\)；失败记 `LEDGER_INVALID`，该次运行不得产生正式判词。

必须保存订单状态机：

    CREATED → RESERVED → SUBMITTED
      → FILLED / PARTIALLY_FILLED / REJECTED / EXPIRED / CANCELLED / CANCELLED_UPGRADE
    PARTIALLY_FILLED → (LifecycleRemainingBudget 重试) SUBMITTED / EXPIRED

每位证券持仓子账必须含：`SettledShares`、`UnsettledBuyShares`、`SellableQty`、`PendingSellReserved`、`AvgCost`、`trade_lots`、FIFO `tax_lots`、`entitlements`。母订单必须含：`ParentOrderTargetValue`、`LifecycleRemainingBudget`、`ExecutionAllocatedBudget`、`IntradaySpendableBudget`、`CommissionReserve`、`CumLifecycleNetBuyCost`、`signal_id`、`parent_order_id`。TheoreticalOrderIntent 另含 `IntendedTargetValue`（完成 TargetValue 计算后、StockPositionLock 竞争前）。

### 15.3.1 Lot 会计唯一口径（v14）
...
|`AvgCost`|组合 PnL / \(R^{net}\) 的卖出成本基础|**移动平均成本（非市值）：** 买入时 \(\mathrm{NewAvgCost}=(OldShares\times OldAvgCost + NewFillNetCost)/(OldShares+NewShares)\)；卖出按 AvgCost 结转；**禁止**用“旧市值/当前价”更新成本；**禁止**用 FIFO 作为 \(R^{net}\) 成本|
|`tax_lots`|分红税持有期限与税务成本基础|FIFO；字段 `{lot_id, shares, cost_basis, acquire_date, settle_state}`；**部分卖出必须拆分**为 disposed/remaining slices（§15.5.2），不得仅改一个 disposal_date|

**现金分红与经济成本（v14）：** 组合 `AvgCost` **不因** CASH_DIVIDEND 改变。record date 创建 entitlement（§15.5.1）；payment date 增加税后现金。税务成本基础若制度要求调整，**只**写入 `tax_lots.cost_basis`，公式按 `tax_rule_version` 冻结表，不得暗改 AvgCost。

**分红税（v22 唯一算法，覆盖 §15.5.2）：** 税率 = `tax_rate(holding_days, board, tax_rule_version)`。holding_days、parent 税额、`TaxTrueUp`、`SettledEstimatedTax` 人民币分 + `floor(T_fen×q/Q)` 拆分见 §15.5.2–15.5.3。

**零碎股与最终清算（v15）：** 送转/拆合后允许非 100 股整数记账；常规卖出对 100 向下取整。若退出腿要求清仓且 `0 < SellableQty < 100`：允许整笔 odd-lot 挂出；执行器必须走 §14.3 odd-lot 分支（不做 100 取整，仍受 `floor(0.05×vol_m)` 限制）。**禁止**按收盘价/跌停价做虚拟现金变现。若连续跌停/停牌无法成交：继续持仓，按 `min(close, down_limit)`（停牌用最近收盘）计入 NAV，保持 pending liquidation；**只有**实际市场成交、官方退市现金、正式换股或其他 PIT 法定结算才能清零。研究窗强制估值仅可标记 `SYNTHETIC_TERMINAL_MARK`，且该 parent 在 Gate 8A 记 `NON_EVALUABLE_EXECUTION`；Gate 9 配对见 §18 `G9_NON_EVALUABLE_EXECUTION`（整对剔除）。

每次转移保存时间、原因、前后现金、预留、仓位、可卖库存与槽位快照哈希。

## 15.5 公司行动与证券身份账本（v14→v15）
...
### 15.5.1 CorporateActionEntitlementLot（v22 唯一）

在 `record_date` 收盘后（`available_at` 已满足）按**当时有权持仓**冻结**法律**不可变权益，字段：

    entitlement_id, security_id, action_type, record_date,
    entitled_shares, source_trade_lot_ids, parent_order_id_alloc,
    source_tax_lot_alloc,   # 见下；record 后不可改
    receivable_cash_gross, receivable_stock, rights_receivable,
    payment_or_effective_date, tax_basis_note, accounting_status

**`source_tax_lot_alloc`（record date 冻结，不可变）：**
...
`entitled_shares` 之和必须等于 entitlement 总股数。后续部分卖出**不得**改写该数组；税款按 §15.5.2 的 tax slice 在该冻结映射上演化。禁止仅用 `source_trade_lot_ids` 在任意时点重推 tax-lot 对应关系。

**三时点状态机（v15 唯一）：**

1. **LegalEntitlementAtRecordDate：** 仅冻结法律 entitlement；`accounting_status=LEGAL_ONLY`；**不**增加 NAV 独立资产；**不**增加 Shares / SellableQty；
2. **AccountingRecognitionAtExDate：** ex-date（或 effective）开盘前：原持仓进入除权价格制度；`accounting_status=RECOGNIZED`；确认 receivable 进入 NAV；**股票权益在此阶段仍是 StockReceivable，不得直接增加实际股份/SellableQty**；
3. **SettlementAtPaymentDate：** payment / listing / 法定到账日开盘前：receivable 转为 `Cash` / 实际 `Shares`；`accounting_status=SETTLED`；此时才增加 SellableQty（并遵守 T+1）。

规则：

1. 权益由 record_date 持仓决定；其后卖出仍保留；其后买入不产生；
2. 结算日**只**结算已冻结 entitlement；
3. **FreeCash 永不含 receivable**；
4. 迟到数据于 `available_at` 后首个交易日开盘前补建/补确认，登记 `CORP_ACTION_LATE`。

### 15.5.2 Receivable 估值与税（v22 唯一）

**现金股息税口径：**

    CashReceivable = receivable_cash_gross
    TaxPayable     = expected_tax   # 必须由下款逐 tax-slice 求和

**禁止**对 CashReceivable 再做 `(1-tax)` 折现同时又记 TaxPayable。

**`TradingDays(a,b)` 唯一算子（全 SPEC 税务持有期共用）：**

    TradingDays(a, b)
    = |{ d ∈ 统一A股交易所交易日历 : a < d ≤ b }|

规则：起点 `a`（取得日）**不计**；终点 `b`（处置日或估值日）**计入**；使用交易所统一交易日历（非单券停牌日表）；日历上的交易日即使该券停牌仍计入；`a≥b` → 0。禁止另选“两端都计/都不计”实现。

**Tax slice 生命周期（强制）：**

1. record date：由 `source_tax_lot_alloc` 为每个 `(entitlement_id, tax_lot_id)` 生成初始 slice：
   `{slice_id, entitlement_id, tax_lot_id, shares, acquire_date, parent_order_id, disposal_date=null, SettledEstimatedTax=null, trueup_status=null}`；
2. **每次部分卖出（shares + SettledEstimatedTax 同步拆分，v22）：** 将被卖出股数从 remaining/parent slice 拆出 `disposed slice`（写入 `disposal_date=实际卖出日`）与 `remaining slice`（`disposal_date=null`）；禁止用单一 lot 级 `actual_disposal_date` 代表全部份额；
   **若父 slice `trueup_status=PENDING` 且已有 `SettledEstimatedTax`（payment 后部分卖出）：** 必须按 §15.5.2「SettledEstimatedTax basis split」同步拆分估计税基线，不得只拆 shares；
3. TaxPayable 只对 slices 求和，不用未拆分 lot 日期。

**`PlannedExitDate_arm(t)`（PIT 唯一，v19）：**

    fixed20 arm / 入口验证生产路径：
      PlannedExitDate_fixed20(t) ≡ 入场时预注册的固定20日退出信号日（不变）

    dynamic arm：
      若截至 t 尚无已归档正式动态退出信号：
        PlannedExitDate_dynamic(t) = PlannedExitDate_fixed20   # 唯一 fallback
      若截至 t 已有已归档动态退出信号日 τ_dyn（≤t）：
        PlannedExitDate_dynamic(t) = τ_dyn
      **禁止**使用 t 之后才发生的动态退出回填 t 日 expected_tax（look-ahead）

生产组合：入口验证阶段用 fixed20；动态退出阶段用当日 PIT 的 `PlannedExitDate_dynamic(t)`。

**逐 tax-slice `expected_tax`：**

对 entitlement e 的每个 slice s（shares>0）：

    GrossDividend_(e,s)
    = receivable_cash_gross_e × shares_s / total_entitled_shares_e

    if disposal_date_s is not null:
        ExpectedHoldingDays_(e,s,t)
        = TradingDays(acquire_date_s, disposal_date_s)
    else:
        holding_days_as_of_t,s = TradingDays(acquire_date_s, t)
        HoldingDaysToPlannedExit_s
        = TradingDays(acquire_date_s, PlannedExitDate_arm(t))
        ExpectedHoldingDays_(e,s,t)
        = max(holding_days_as_of_t,s, HoldingDaysToPlannedExit_s)

    Tax_(e,s,t)
    = GrossDividend_(e,s) × tax_rate(ExpectedHoldingDays_(e,s,t), board, tax_rule_version_asof_t)

    TaxPayable_(e,t) = Σ_s Tax_(e,s,t)

**禁止**最早取得日、加权平均日、parent 平均日、`tax_rate(加权平均持有日)`。

**开盘前税制版本 PIT：**

    tax_rule_version_asof_t
    = latest version with available_at ≤ PREOPEN_ACTION_CUTOFF(t)   # §4.3

**禁止**使用 t 日 `INPUT_SNAPSHOT_CUTOFF`（t 20:00）处理 t 日开盘前 TaxPayable。

**重估与 payment / 双向 true-up（v22 唯一）：**

定义最终真实税（税制版本永远随持有期终点日）：
    ...
    = GrossDividend_(e,s)
      × tax_rate(
          TradingDays(acquire_date_s, disposal_date_s),
          board,
          tax_rule_version_asof_disposal
        )
    # tax_rule_version_asof_disposal = PREOPEN_ACTION_CUTOFF(disposal_date) 可知版本

1. **每日开盘前（payment 前）：** 逐 slice 重算 `Tax_(e,s,t)`；档位变化 → `TAX_HOLDING_REMEASURE`；税制变化 → `TAX_RULE_REMEASURE`；
2. **payment 开盘前结算（按 slice 分支，税制版本不得混用）：**

    if disposal_date_s is not null and disposal_date_s < payment_date:
        # disposal-before-payment：holding 与税制版本均冻结在 disposal
        SettledTaxAtPayment_(e,s) = FinalActualTax_(e,s)
        SettledEstimatedTax_(e,s) = SettledTaxAtPayment_(e,s)
        TaxTrueUp_(e,s) = 0
        trueup_status_s = NOT_REQUIRED
        # **禁止**再用 tax_rule_version_asof_payment 覆盖本 slice
    else:
        # 尚未处置：按 payment 日 PIT holding / 税制估计
        SettledTaxAtPayment_(e,s) = Tax_(e,s,payment)   # 用 tax_rule_version_asof_payment
        SettledEstimatedTax_(e,s) = SettledTaxAtPayment_(e,s)
        trueup_status_s = PENDING   # 待后续实际处置 true-up

    SettledDividendCash_(e,p)
    = CashReceivable_(e,p) - Σ_{s: parent_order_id=p} SettledTaxAtPayment_(e,s)

   清零组合层 CashReceivable / TaxPayable；**保留**各 slice 的 `SettledEstimatedTax_(e,s)` / `trueup_status` 直至 true-up 完成或 NOT_REQUIRED；
3. **payment 之后的实际处置日（双向 true-up + 现金过账）：**
   对每个新 disposed slice（`trueup_status=PENDING`）：
   `TaxTrueUp_(e,s) = FinalActualTax_(e,s) - SettledEstimatedTax_(e,s)`；
   `TaxTrueUp > 0` → `DIV_TAX_CLAWBACK`；`< 0` → `DIV_TAX_REVERSAL`；`=0` → 无现金变动但仍标记 POSTED；
   **强制现金过账：** `Cash_delta = -TaxTrueUp_(e,s)`（计入 FreeCash / 当日 NAV）；不得只改研究 outcome；
   parent 级 `TaxTrueUp_p = Σ TaxTrueUp_(e,s)` 进入 §0.3 `TotalNetProceeds`；
   **禁止**把 TaxTrueUp 再写入 `SettledCorporateActionCash_p`。

**SettledEstimatedTax 货币单位与量化（v22 唯一）：**

正式账本单位：**人民币分，非负整数**（记 `_fen`）。禁止以元浮点、未定义定点或“实现自选有理数→分”路径作为正式存储。

    QuantizeTaxToFen(x_yuan)
    = floor(x_yuan × 100)
    # 中间量用任意精度有理数/十进制；禁止 IEEE754 浮点参与该转换

payment 写入 `SettledEstimatedTax` 时必须先量化：

    SettledEstimatedTax_fen
    = QuantizeTaxToFen(SettledTaxAtPayment_yuan)

计算 `TaxTrueUp` / 现金过账时，`FinalActualTax` 同样先 `QuantizeTaxToFen`，再：

    TaxTrueUp_fen = FinalActualTax_fen - SettledEstimatedTax_fen   # 有符号整数分
    Cash_fen     -= TaxTrueUp_fen

**禁止** half-up / half-even / truncate-toward-zero / 银行家舍入 / 配置文件另选舍入模式。

**SettledEstimatedTax basis split（v22 唯一；payment 后部分卖出强制）：**

设拆分前父 slice（`trueup_status=PENDING`）为：

    Q     = shares_parent_before          # 正整数股
    T_fen = SettledEstimatedTax_parent_before_fen   # 非负整数分
    q     = 本次卖出股数（0 < q ≤ Q）

则同步生成（整数算术，禁止浮点）：

    SettledEstimatedTax_disposed_fen
    = floor(T_fen × q / Q)      # 任意精度整数乘法后向下取整

    SettledEstimatedTax_remaining_fen
    = T_fen - SettledEstimatedTax_disposed_fen

若 `q = Q`：`disposed_fen = T_fen`，`remaining_fen = 0`（最终清算取得全部剩余 baseline）。

守恒约束（每次拆分强制断言）：

    disposed_fen + remaining_fen = T_fen

全生命周期断言：

    Σ disposed baseline_fen + 当前 remaining baseline_fen
    = payment 时原始 SettledEstimatedTax_fen

两个 child 均继承：`entitlement_id`、`tax_lot_id`、`acquire_date`、`parent_order_id`、payment 税元数据；`trueup_status=PENDING`。
disposed child：`TaxTrueUp_fen = FinalActualTax_fen - SettledEstimatedTax_disposed_fen`；
remaining child 保留 `SettledEstimatedTax_remaining_fen`，供后续部分卖出继续拆分。

**禁止**实现：half-up/half-even 到分、将全部 T 放入 disposed、将全部 T 留给 remaining（除非 q=Q）、或 disposed 基线强制为 0。

若拆分发生在 payment **之前**（尚无 SettledEstimatedTax）：只拆 shares / disposal_date；估计税仍按 payment 日分支写入并 `QuantizeTaxToFen`（§上款）。

**TaxTrueUp 幂等与事件键（强制）：**

    TaxTrueUpEventKey = (entitlement_id, disposed_slice_id, disposal_fill_id)

每个 EventKey 至多过账一次。slice 字段至少含：

    trueup_status ∈ {NOT_REQUIRED, PENDING, POSTED}
    trueup_posted_at, trueup_amount, tax_rule_version_used, ledger_entry_id

重放/审计重跑若 EventKey 已 POSTED → 跳过，不得重复 `Cash -= TaxTrueUp`。

**卖出成交后唯一顺序（与 §26 伪代码同构）：**

    update_trade_positions_and_sell_cash(sell_fills)
    → split_tax_slices_and_settled_tax_basis_by_sell_fills(sell_fills)
      # 拆 shares；写 disposal_date；若 PENDING：disposed_fen=floor(T_fen×q/Q)；remaining=T−disposed；守恒
    → compute_final_actual_tax_and_trueup(disposed_slices)
    → post_tax_trueup_cash_once(trueup_events)     # Cash -= TaxTrueUp；幂等
    → attribute_trueup_to_parent(trueup_events)
    → assert_ledger_identities()

|类型|RECOGNIZED 起估值|
|---|---|
|CashReceivable|`receivable_cash_gross`（毛额）；配套逐 slice / parent `TaxPayable`|
|StockReceivableValue|每日收盘 **mark-to-market**：`receivable_stock_shares × P_close,t`；successor 用其收盘；停牌用最近收盘；缺失 → INVALID_DATA；ex-date 初值可用除权参考价|
|RightsReceivable|NEVER_EXERCISE：估值恒为 0|

禁止用 record-date 含权收盘价同时给 PositionValue 与 CashReceivable 计价。

### 15.5.3 Parent / lot 分摊（v22：税额按 tax-slice parent 归集）

**现金股息 / receivable 税额（强制实现 B）：**

    CashReceivable_(e,p)
    = Σ_{s: parent_order_id=p} GrossDividend_(e,s)

    TaxPayable_(e,p,t)
    = Σ_{s: parent_order_id=p} Tax_(e,s,t)

    RecognizedCashTerminalValue_(e,p,t)
    = CashReceivable_(e,p) - TaxPayable_(e,p,t)

    SettledDividendCash_(e,p)
    = CashReceivable_(e,p) - Σ_{s: parent_order_id=p} SettledTaxAtPayment_(e,s)
    # SettledTaxAtPayment：disposal<payment 用 FinalActualTax（disposal 税制）；否则用 Tax_(e,s,payment)

**禁止**先汇总 entitlement 总净额再按 `source_trade_lot_ids` 股数比例平均分摊税额或净股息。`source_tax_lot_alloc.parent_order_id` → tax slice → parent 是税额唯一路径。
**禁止**对已处置 slice 使用 payment-date 税制版本覆盖 `FinalActualTax`。

**非税公司行动数量/股份终值（按冻结 tax-lot 股数，非 trade-lot 税额路径）：**

    shares_(e,p) = Σ_{s: parent_order_id=p} shares_s
    StockReceivableValue_(e,p) = StockReceivableValue_e × shares_(e,p) / total_entitled_shares_e
    RightsReceivableValue_(e,p) / 退市现金_(e,p) 同理按 shares_(e,p) 归集

不得用 `source_trade_lot_ids` 比例覆盖已由 tax-slice 决定的税额；非税数量归集也优先 `source_tax_lot_alloc` 股数。

|action_type|账本效果|
|---|---|
|CASH_DIVIDEND|record：LEGAL_ONLY；ex：RECOGNIZED CashReceivable+TaxPayable（parent 分摊见上）；payment：SETTLED 按 SettledDividendCash_(e,p)（disposal<payment 用 FinalActualTax）；保留 SettledEstimatedTax；payment 后 true-up 过账 Cash；AvgCost 不变|
|STOCK_DIVIDEND / TRANSFER|record：LEGAL_ONLY；ex：RECOGNIZED StockReceivableValue（**不**增实际股份）；payment/listing：SETTLED 增加实际股份；总成本不变；零碎股见 §15.3.1|
|SPLIT / REVERSE_SPLIT|effective 开盘前：股数 × ratio；AvgCost 按倒数；总成本不变（拆合直接改 Position，不经 receivable）|
|RIGHTS_ISSUE|record：rights_receivable；**NEVER_EXERCISE**：到期作废，不扣现金不增股|
|CODE_CHANGE / ABSORPTION|自 `LockGroupFormationPreOpen` 起共享 LockGroup；effective 开盘前原子迁移持仓/entitlement/成本/PendingSell/lock+owner；形成时若跨 alias 已有 ≥2 个不同 owner → `CORP_ACTION_LOCK_COLLISION`/`LEDGER_INVALID`；见 §15.0|
|DELISTING / LONG_SUSPENSION_EXIT|按 `delisting_cash_value` 变现；股数清零；未结算 entitlement 按事实表处理|

DG16：漏建 entitlement、漏结算、错误按 trade-lot 均摊税额、或破坏 NAV → `LEDGER_INVALID`。
...
- **版本 α-wealth：** 正式 PASS 预算按**实际解封 HOLDOUT 的版本序号** k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)。**HOLDOUT 触碰日志（v22 唯一声明）：** v1–v21 均未解封 HOLDOUT，已消耗集合为空；**本 Canonical `SFL-1_v22.0` 为 k=1，\(\alpha_v=0.025\)**。后续任一新 SPEC 若解封 HOLDOUT 必须取下一 k，禁止重置；不得并行登记多个“当前 k=1”；
...
1. 买入：按 §14.1 人民币母订单 + 分钟流式参与（含 REJECT_*、部分成交、跨日 LifecycleRemainingBudget 补单，有效期同生产 D+1:D+3）；
...
加入分钟执行、涨跌停、未成交和全部成本后，按下列法庭对象报告；不得混成同一 CI。

### 正式对象分层（v16 唯一）

|对象|定义|用途|
|---|---|---|
|`SignalCandidate`|Gate 6/7 排序后、完成资格检查、**尚未**算出 IntendedTargetValue 的候选|排序与诊断|
|`TheoreticalOrderIntent`|**凡**已完成 `IntendedTargetValue` 计算的对象**必须**物化（含零金额、`REJECT_DUPLICATE_STOCK`、`REJECT_MIN_WEIGHT`、`REJECT_PORTFOLIO_FULL` 及后续执行拒绝）|Gate 8B / θ_ADV|
|`ParentOrder`|取得 StockPositionLock 并创建的正式母订单|Gate 8A、ParentFillRate|

**Population 纪律：** 完成 TargetValue 计算 ⇒ 必须创建 Intent（含 `IntendedTargetValue=0`）。

### Gate 8A：Conditional Execution Court（正式主 estimand）

- **Population_fill：** 有效期内至少一笔非零成交的 `ParentOrder`；
- **Outcome：** NetAlpha20 = §0.3 `R^{net}`（TotalNetProceeds 口径）；
- **Null / MES：** 见附录 B；进入 §18.0 IUT 主终局；
- **禁止**把未成交记 0 后并入本样本。

### Gate 8B：Execution Policy Diagnostic（强制政策诊断，非第二主 Gate）

- **Population_intent：** **全部** `TheoreticalOrderIntent`（含零金额）；
- **Outcome（零分母闭合）：**
  `RealizedNetPnL(s) = TotalNetProceeds(s) - BuyNetCost(s)`（未成交：两者为 0 ⇒ PnL=0）；
  若 `IntendedTargetValue(s) = 0`：**强制** `PolicyReturn(s)=0`（禁止 0/0）；
  否则 `PolicyReturn(s) = RealizedNetPnL(s) / IntendedTargetValue(s)`；
  已创建 Parent 时 IntendedTargetValue ≡ ParentOrderTargetValue；
  **禁止**把部分成交上的 NetAlpha20 直接当作 PolicyReturn；
- `ParentFillRate` 分母 = 全部正式 `ParentOrder`；
- 正式汇总：`PolicyNetAlpha20 =` Intent 等权均值（零金额 Intent 以 0 贡献，完整反映无空间放弃交易）；与 ParentFillRate 同列首页；
- **不得**替代 Gate 8A；标记 `DIAGNOSTIC_POLICY_RETURN`；
- 若 8A PASS 但 FillRate 过低或 PolicyNetAlpha20 显著恶化，登记 `POLICY_IMPLAUSIBLE`。

### 强制逆向选择对照（隔离研究账户；与 8A/8B 并行）

对每个 `TheoreticalOrderIntent` s（**纯 REJECT_GAP 隔离**）：

1. 隔离账户固定 `IntendedTargetValue`（可为 0）；
2. **忽略**生产组合 `Cash` / rooms / **StockPositionLock**；actual 与 no-gap 唯一差异为 `REJECT_GAP`；
3. 若 `IntendedTargetValue=0`：强制 `Outcome_actual=Outcome_no_gap=0` ⇒ \(\theta_{ADV,s}=0\)；
4. 否则成交用 §0.3 `R^{net}`，未成交为 0；\(\theta_{ADV,s}=Outcome_{no\_gap}-Outcome_{actual}\)；
5. 正式 estimand = Intent 等权 \(\theta_{ADV}\)；非劣界 1 个百分点；
6. **禁止**完整生产组合逐信号 rerun（`DIAGNOSTIC_PORTFOLIO_RERUN`）；
7. guard PASS：\(\theta_{ADV}\) 单侧 \(1-\alpha_v\) 上界 < 0.01；
8. guard FAIL：Bonferroni 单侧 \(1-0.05/(9\cdot m)\)（m=2）下界 > 0.01 → `FAIL_ADVERSE_SELECTION`；
9. 其余 `INCONCLUSIVE_ADVERSE_SELECTION` → Gate 8 INCONCLUSIVE；
10. **Gate 8 IUT** =（8A NetAlpha20）与（\(-\theta_{ADV}\)）；8B 不进 IUT。

FAIL：

    FAIL_EXECUTION

唯一失败码优先级：若逆向选择 guard FAIL → `FAIL_ADVERSE_SELECTION`；否则若 8A FAIL → `FAIL_EXECUTION`。任一共同主 estimand INCONCLUSIVE 且无 FAIL 时 Gate 8 为 INCONCLUSIVE。

## Gate 9：退出增量

动态退出相对固定20日持有是否改善配对 parent 净收益增量。

**科学问题选择（v17 Canonical）：PairResolvableByCutoff 上 parent 配对退出增量。** 不构造共享有限现金组合，不回答端到端资金政策。

**外生冻结入场 tape：**

1. Tape 来源：隔离研究账户上固定20日入口政策的生产入场成交序列；哈希冻结后方可开庭；
2. 行字段：`signal_id, parent_order_id, attempt_date, fill_date, stock, fill_qty, fill_net_cost, origin_episode_id, original_target_value, persistent_component_id, ...`；
3. **配对单元：** 每个 `parent_order_id` 一条观测；
4. **Outcome：**
   `ExitInc_p = R^{net,dynamic}_p - R^{net,fixed20}_p`，
   两侧 \(R^{net}\) 均用 §0.3 `TotalNetProceeds`；共享同一买入 tape，仅退出规则不同；T+1 与费用同构；
5. 正式共同主 estimand（MES_pass 各 = 0.01，MES_fail 各 = 0）：
   - \(\theta_{ExitMean} = (1/N)\sum_p ExitInc_p\)（parent 等权）；
   - **\(\theta_{ExitComp}\) 唯一公式（component 等权）：** 令 G = persistent overlap component 数，component g 含 \(n_g\) 个 parent，
     \[
     \theta_{ExitComp}
     =
     (1/G)\sum_g\Big[(1/n_g)\sum_{p\in g} ExitInc_p\Big]
     \]
     实现时保留 parent 行并赋权 \(w_p = 1/(G\times n_g)\)；**禁止**先把 component 压成单行再虚构月份；
6. **禁止** `DeployedCapitalBase = max ConcurrentGrossNotional` 作为主现金；仅诊断；
7. **禁止**隐含融资而不改用 TWR；端到端对照标记 `DIAGNOSTIC_E2E_POLICY`，不进 Gate 9 主终局。

**推断装置（v17）：** parent 层观测；ID1 = persistent component；ID2 = 该 parent 首笔 fill 月；主推断 = 题材簇×月 CGM；稳健性 = `G9_COMP_BOOT`（默认 5,000，可升 50,000）。

**`G9_OUTCOME_CUTOFF_p`（预注册，两臂共用，结果前冻结）：**

    G9_OUTCOME_CUTOFF_p
    = min(
        TradingDayAdd(parent_first_fill_date, G9_MAX_RESOLVE_TRADING_DAYS),
        partition_end_date
      )

其中 `G9_MAX_RESOLVE_TRADING_DAYS = 90`（§27）。**禁止**随结果延长、禁止实现者另选 cutoff、禁止两臂使用不同截止日。

**Population（正式收窄）：** `PairResolvableByCutoff` = 在 `G9_OUTCOME_CUTOFF_p` 前，dynamic 与 fixed20 **两臂均完成真实清算**（无未清算仓位、无 `SYNTHETIC_TERMINAL_MARK`）的 parent。θExitMean / θExitComp **仅**在此 Population 上定义。

**未清算配对：** 截止时任一侧未清算或带 synthetic mark → `G9_NON_EVALUABLE_EXECUTION`，整对剔除。禁止单侧估值、禁止无限等待计入样本。

**H-EXIT 判词域（强制）：** 正式 PASS 仅声称“在两臂均于 cutoff 前真实清算的 parent 中，dynamic 相对 fixed20 的条件退出增量”；**禁止**外推至全部 parent。

**Unresolved selection（v17 对称配对 guard）：** 对全部 tape parent p 定义配对变量
`Z_p = I(unresolved_dynamic,p) - I(unresolved_fixed20,p)`，
`ΔU = mean(Z_p)`（= UnresolvedRate_dynamic − UnresolvedRate_fixed20）。
**主推断装置（唯一）：** parent 层；ID1=persistent component；ID2=parent 首笔 fill 月；与 Gate 9 主法庭同构的 **CGM** 生成双侧置信区间 `CI_{1-α_v}(ΔU)`（禁止独立二项、禁止未声明方法）。
**正式通过条件（必须同时满足）：**
1. `CI_{1-α_v}(ΔU) ⊂ [-G9_UNRESOLVED_DIFF_TOL, +G9_UNRESOLVED_DIFF_TOL]`（默认 ±0.05）——即 UCB≤+tol **且** LCB≥−tol（两方向均检验）；
2. `max(UnresolvedRate_dynamic, UnresolvedRate_fixed20) ≤ G9_UNRESOLVED_ABS_TOL`（默认 0.20）。
任一不满足 → 登记 `G9_SELECTION_UNRESOLVED` 且 Gate 9 **必须**为 `INCONCLUSIVE`（不得 PASS）。否则若 Population 内 IUT PASS 且无 FAIL → 可 PASS（仍受 §20.5 动态护栏）。
...
### P10_NULL_EPISODE（v16 Stable Episode Generator）

P10 不得以每日新建随机 control 当作跨日题材。spawn 后必须形成**稳定伪 episode**。

**Identity key：**

    (spec_version, real_theme_id, fold, control_index, per_control_spawn_index)

|字段|定义|
|---|---|
|`P10_NULL_EPISODE_ID`|`Hash(spec_version,"P10EPI",real_theme_id,fold,control_index,per_control_spawn_index)`|
|`discovered_at`|spawn 日|
|`fold`|spawn 时冻结，生命周期不变|
|`base_membership`|spawn 日成员集合，**此后永久冻结**直至关闭|
|`base_fingerprint`|spawn 日 fingerprint|
|`τ_fake_confirm` / `main_signal_used` / `closed_at` / `close_code`|同前|

**稳定运行（v14 核心，取代日度 Jaccard continuation）：**

1. 仅当某 `(real_theme_id, control_index)` **无开放**伪 episode，且 cooldown 满足时，才允许 spawn；
2. spawn 日：从当日 DAILY_NULL 对齐矩阵取 `Members(c,D)` 冻为 `base_membership`；`fold = Hash(spec_version,real_theme_id,control_index,discovered_at) mod 5`（**禁止**再用含变动 D 的 fold 公式改变已 spawn episode）；
3. `per_control_spawn_index` 在 `(real_theme_id, fold, control_index)` 内从 0 严格递增；同日多 control spawn 按 `control_index` 升序分配；
4. **spawn 之后直至正常关闭：** 每日直接用冻结 `base_membership` 更新市场统计与状态机，**禁止**每日重抽 control c 再与 base 比 Jaccard；**禁止**因日度随机 membership 变化而 SUPERSEDE；
5. 关闭码仅来自 §12.10 同构路径或 `CLOSED_SUPERSEDED`（仅当显式更换 control 族时；常规日更不得 supersede）；
6. cooldown：同 `(real_theme_id,fold,control_index)` 关闭后 `D ≥ closed_at + 5` 才可 respawn；
7. 新的日度 DAILY_NULL control 只用于：**无开放 episode 且 cooldown 结束**时的新 spawn 候选，不得扰动已开放伪 episode。
...
FalseSignalRate(W)
=
\frac{\sum_{e:\,overlap(W)} P10\_EVALUABLE\_EPI(e)\times P10\_FAKE\_SIGNAL\_EPI(e)}
{\sum_{e:\,overlap(W)} P10\_EVALUABLE\_EPI(e)}
\]

分子**只**计 evaluable∩fake；禁止非 evaluable episode 进入分子。
...
=
\#\{\,e:\,P10\_EVALUABLE\_EPI(e)=1,\ P10\_FAKE\_SIGNAL\_EPI(e)=1,\ discovered\_at\in y\,\}
\]

按**唯一可评估伪 episode**计数；禁止把每日 recipient 隐含当成新 episode。板块/规模分层规则同前（按 spawn 日真实题材属性）。

年度覆盖（v16：episode-day 严格绑定真实可评估 theme-day）：令

    RealThemeDayKey = (real_theme_id, D)
    R_y = { (u,D) : 真实题材 u 在 D 为真实可评估题材日，且 D∈y }
    RealEvaluableThemeDays_y = |R_y|

    P10_EVALUABLE_EPI_DAY(e,D) = 1{
      e 在 D 运行
      AND D 日 reference≥100
      AND D 日 SMD 通过
      AND D 日测量链无缺失
    }

    PseudoEpisodeDays_y
    = Σ_{(u,D)∈R_y} Σ_{e: e.real_theme_id=u} P10_EVALUABLE_EPI_DAY(e,D)

    CoveredRealEvaluableThemeDays_y
    = Σ_{(u,D)∈R_y} 1{ Σ_{e: e.real_theme_id=u} P10_EVALUABLE_EPI_DAY(e,D) > 0 }

**禁止**把非 R_y 上的 P10 运行日计入 PseudoEpisodeDays（即使 episode 仍在生命周期内可评估）。`P10_EVALUABLE_EPI(e)` 仍为 episode 层仪器指标，**不得**替代日级 `P10_EVALUABLE_EPI_DAY`。

要求：

    若 RealEvaluableThemeDays_y = 0 → `P10_INSUFFICIENT_YEAR`，且不检验该年 AnnualFalseEpisodes
    否则：
      PseudoEpisodeDays_y / RealEvaluableThemeDays_y >= 40
      且 CoveredRealEvaluableThemeDays_y / RealEvaluableThemeDays_y >= 0.90

否则 `P10_INSUFFICIENT_YEAR`（40 = 每真实可评估题材日平均可评估伪 episode 暴露下限）。同日题材 A 覆盖不使 B 被覆盖；禁止用 spawn 数惩罚长寿 episode。达标前不得报告 AnnualFalseEpisodes、不得用于 Phase-I/漂移。
...
1. **审计抽样 fold（生成第二层 reference 时）：** 对每个已 spawn 伪 episode 使用其**冻结** `fold`；新 spawn 候选的 fold 按 §19 Identity Protocol 在 discovered_at 冻结；禁止对开放 episode 按变动 D 重算 fold；
...
4. 每个 `P10_NULL_EPISODE_ID` 在其生命周期中恰好作为假真实题材一次（允许同 `control_index` 在 cooldown 后 respawn 形成新 episode 并再次审计）；五 fold 结果合并前按 `(control_index, per_control_spawn_index)` 稳定排序；
...
**正式主装置：** 题材簇 × 自然月双向聚类稳健标准误，由其生成 Gate 主效果 CI 与 p 值。Gate 9 稳健性使用 §18 声明的 persistent-component 整块 bootstrap（`G9_COMP_BOOT`），不是组合级 circular block；Gate S 使用月块 bootstrap，属显式例外。
...
|Gate 9 persistent-component bootstrap（稳健性）|`spec_version,"G9_COMP_BOOT",partition_id,rep`|
...
伪题材 donor 复用导致的基准相关：同一 persistent overlap component 内由 ID1 吸收。**跨 component 的高 donor overlap（v13 强制敏感性）：** 若两正式观测的 control 篮子 donor Jaccard 中位数 >0.30，须额外报告 donor-exposure block bootstrap（按最大 donor 连通分量整块抽样）；若该敏感性与主装置“方向相反”（§20.2.4 定义），主 PASS 降级 INCONCLUSIVE。不把 200 个篮子当独立样本。
...
**成交率（v13 主口径 = ParentFillRate）：**

\[
ParentFillRate
=
\frac{\sum_{parent} LifecycleFilledNetValue}
{\sum_{parent} OriginalTargetValue}
\]

每个 `parent_order_id` 分母只计一次 `OriginalTargetValue`；分子为其生命周期实际净买入金额（跨日重试不重复计分母）。**AttemptFillRate**（按尝试日理论子订单）仅诊断，不得替代主口径。下文凡经济门槛、样本充分性、安全 Kill 所称 FillRate，均指 **ParentFillRate**。无理论母订单日记 `NA`，不进入“连续10日”分母。
...
**真实交易日 D 的唯一顺序（v13）：**
    ...
    D 开盘前
      ├─ 结算 record-date entitlement / payment / 股数类公司行动（§15.5）
      ├─ 昨日 UnsettledBuyShares → SettledShares
      ├─ 更新 SellableQty / tax_lots / entitlements
      └─ StockPositionLock 状态载入

    D 09:35—10:00
      ├─ 执行由 D−1（及有效期内更早）生成的买单（连续封死拒单、StockPositionLock、预算拆分）
      ├─ 顺序冲击；日终佣金；仅最后一分钟可 TailAdjust
      └─ 新买 → UnsettledBuyShares

    D 14:30—15:00
      └─ 执行由 D−1 收盘生成的卖单（SellableQty；odd-lot 最终清算）

    D 15:00以后（信号日）
      ├─ 落地日线/分钟线；更新 FeatureStore / DonorIndex
      ├─ 更新 LAC、episode 结构（ThemeRoom 不迁移）
      ├─ load_or_build_nulls_v22；Gate1 冻 DISCOVERY_RESEARCH_* 
      ├─ 未确认：临时角色 + DAILY_NULL；已确认：冻结集合 + EPISODE_NULL
      ├─ 确认日：归档临时量尺判定 τconfirm → ConfirmedEventFlag=1 → 冻结 → FrozenMetricBackcast
      ├─ 确认日刚冻结后：**重载 EPISODE_NULL PseudoThemeStats** 再算 CCS/GCC/状态/信号
      ├─ 状态机（EARLY_CAPACITY 用 ConfirmedEventFlag，不重判确认斜率）
      ├─ 代表生成信号；StockPositionLock 竞争；预留 LifecycleRemainingBudget
      ├─ 更新 desired exit state（风险分支不直接下单）
      ├─ **统一物化**退出母订单一次（幂等 upsert）
      └─ DG/NG/DG16、SLO、漂移、账本检查

    D 21:00 / 21:30
      └─ COMPUTE_TIMEOUT / 归档哈希
...
# 25. 核心伪代码
    ...
    for D in trading_days:
        # ===== D 开盘前 =====
        apply_corporate_actions_pre_open(D)  # entitlement settle + stock adjust
        roll_yesterday_unsettled_to_settled(D)
        refresh_sellable_qty_tax_lots_entitlements(D)

        # ===== D 09:35 执行昨日及有效期买单 =====
        buy_orders = open_buy_orders_executable_on(D)  # created on D-1.. with validity
        reject_and_release_reserves(buy_orders)  # gap/limit/StockPositionLock(lock_owner exempt)
        eligible_buy_orders = [o for o in buy_orders if o.accepted]
        # 顺序强制：先再分配 → ReservedCashAfter0935 → ExecutionAllocatedBudget → 佣金 → 日内预算 → 执行
        reallocate_cash_at_0935(eligible_buy_orders, PARAMS)
        compute_execution_allocated_budget(eligible_buy_orders)  # uses ReservedCashAfter0935; no FreeCash
        set_commission_reserves(eligible_buy_orders)
        compute_intraday_spendable_budget(eligible_buy_orders)
        buy_fills = execute_streaming_participation_sequential_impact(eligible_buy_orders)
        settle_day_commission_topup_tail_adjust_last_minute_only(buy_fills)
        update_positions_unsettled(buy_fills)
        assert_ledger_identities()
        assert_all(o.fill_qty == 0 for o in buy_orders if not o.accepted)
        assert_all(o.ExecutionAllocatedBudget <= o.ReservedCashAfter0935 for o in eligible_buy_orders)
        assert_receivable_not_in_freecash()

        # ===== D 14:30 执行昨日卖单（含 TaxTrueUp 现金过账，顺序强制）=====
        sell_orders = open_sell_orders_executable_on(D)
        sell_fills = execute_streaming_sell_orders_1430(sell_orders)  # SellableQty; odd-lot final
        update_trade_positions_and_sell_cash(sell_fills)  # 卖出款+股份；不含 TaxTrueUp
        disposed_tax_slices = split_tax_slices_and_settled_tax_basis_by_sell_fills(sell_fills)
        # PENDING：disposed_fen=floor(T_fen×q/Q)；remaining_fen=T_fen-disposed_fen；整数分守恒
        trueup_events = compute_final_actual_tax_and_trueup(disposed_tax_slices)
        # Final/Settled 均 QuantizeTaxToFen；TaxTrueUp_fen=Final_fen-Settled_fen
        post_tax_trueup_cash_once(trueup_events)  # Cash_fen -= TaxTrueUp_fen；EventKey 幂等；已 POSTED 跳过
        attribute_trueup_to_parent(trueup_events)  # TaxTrueUp_p；不入 SettledCorporateActionCash
        assert_ledger_identities()

        # ===== D 收盘后：度量与生成 D+1 订单 =====
        stock_features = update_stock_day_feature_store(D)
        donor_index = build_daily_donor_index(stock_features, D)
        themes = load_base_universe_asof(D - 1) | themes_of(open_episodes)

        update_live_active_core(open_episodes, D)
        update_episode_structure(open_episodes, D)  # ThemeRoom 不迁移
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v22(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v22(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)

            if not pass_theme_gate(base, D) or len(nulls) < 100 or not pass_balance_gate(nulls):
                update_desired_exit_state_only(theme, lac_health, D)  # 不下单
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                role_cohort.ConfirmedEventFlag = 1
                frozen_metric_backcast(role_cohort, D)
                # A-18：确认日必须改用 EPISODE_NULL stats
                m["nulls"] = load_or_build_nulls_v22(
                    theme, D, mode="EPISODE_NULL", donor_index=donor_index)
                m["pseudo_stats"] = update_pseudo_theme_stats(m["nulls"], D)
            ...
            rmi = (role_migration_index(role_cohort.B_E, role_cohort.L_E,
                                       role_cohort.K_E, D)
                   if role_cohort.is_frozen else None)

            state = lifecycle_state_once_per_theme(
                theme,
                confirmed_event_flag=role_cohort.ConfirmedEventFlag,
                ies=m["ies"], ids_theme=m["ids_theme"],
                gcc_theme=gcc_theme, rmi=rmi,
                lac_decay=lac_health.get(theme).decay_if_held,
                # EARLY_CAPACITY 不重判确认斜率
                recheck_confirm_slope=False,
                priority=PARAMS.state_priority,
            )
            update_desired_exit_state_only(theme, state, lac_health, D)
            ...
            if theme in representatives and first_entry_window(theme, state):
                ...
                for stock in capacity_trade:
                    ...
                    if pass_candidate_entry(
                        ...
                        emit_signal_candidate(stock, theme, valid_until=D + 3)

        # StockPositionLock：全局排序后唯一母订单；持仓期间禁第二 parent
        intents = build_theoretical_order_intents(signal_candidates, PARAMS)
        signals = select_parent_orders_with_stock_position_lock(intents, PARAMS)
        reserve_lifecycle_budgets(signals, retries, PARAMS)

        # A-19：全部退出订单只物化一次（幂等）
        materialize_exit_orders_idempotent(open_episodes, D)
...
|P10 年度有效日覆盖率|90|% real-theme-days|工程闸门|95%报告|
|P10 episode-day/真实评估日下限|40|episode-日/题材日|工程闸门（=200×0.20）|—|
...
|Gate 9 MES_pass（θExitMean / θExitComp）|0.01 / 0.01|parent ExitInc|经济先验|减半/加倍报告|
|Gate 6 近邻池/臂内只数|5 / 2|只|工程|—|
|Gate 2/7 匹配卡尺|VALIDATION 配对距离90%分位|马氏|工程|85% / 95%|
|Gate 9 component bootstrap|persistent-component / 5,000（可升50,000）|块 / 次|推断稳健性|—|
|Gate 9 G9_MAX_RESOLVE_TRADING_DAYS|90|交易日|工程冻结|60 / 120 报告|
|Gate 9 G9_UNRESOLVED_DIFF_TOL|0.05|率差|工程先验|0.03 / 0.10|
|Gate 9 G9_UNRESOLVED_ABS_TOL|0.20|率|工程先验|0.10 / 0.30|
|P10 绝对预算 率/年化|5% / 20|— / episode|经济先验|3%/10；8%/40|
|版本 α-wealth|0.05×2^{-k}（v22: k=1, α_v=0.025；HOLDOUT 从未消耗）|—|治理|—|
...
本规格的 Canonical 纪律：
    ...
    SettledEstimatedTax 人民币分；disposed_fen=floor(T_fen×q/Q)；禁浮点/half-up/half-even；
    α-wealth v22 k=1；路径 /sfl1_v22/。
...
|统一算子|§6|收益、行业收益、TurnZ、CLV、Active、Pctl、EMA、Hash/seed、TradingDays、缺失窗口|
...
|入场、执行与账本|§13–§16|ExecutionAllocatedBudget、AllInUnitCost、entitlement 三时点、TaxTrueUp 过账/幂等、SettledEstimatedTax 分整数+floor 拆分、odd-lot 真实清算、StockPositionLock|
...
|8A 执行主|Population_fill（非零成交 ParentOrder）|理论信号日|Conditional NetAlpha20（TotalNetProceeds）|≤0|0.01 / 0|persistent簇×月 CGM|与 θ_ADV 组成第8门 IUT（m=2）|仅Gate1–7 PASS后；HOLDOUT一次|正式主；禁与8B混CI|
|8B 政策诊断|TheoreticalOrderIntent|理论信号日|PolicyNetAlpha20（未成交=0）|—|无独立 MES（强制报告）|—|不进 IUT / 不单独 PASS|与8A同开庭强制输出|DIAGNOSTIC_POLICY_RETURN|
|8 逆向选择|TheoreticalOrderIntent；隔离账户|理论信号日|\(-\theta_{ADV}\)|上界≥0.01 为逆|−0.01；fail 下界>0.01|persistent簇×月 CGM|并入第8门 IUT|同 Gate8|禁组合干扰|
|9 退出|Track S；固定买入 tape；PairResolvableByCutoff|parent 首笔 fill 月|θExitMean 与 θExitComp（条件域）|任一≤0|各0.01 / 0|persistent簇×月 CGM；G9_COMP_BOOT|固定顺序第9；m=2 IUT|仅Gate1–8 PASS后；HOLDOUT一次|cutoff=min(fill+90,partition_end)；对称 CI(ΔU)⊂[±tol]；禁外推全部 parent|
...
**index_date 规则（v14）：** `ID2 = YYYY-MM(index_date)`，必须回引本表，不得另选。

**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v22 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
|目标权重、StockPositionLock、ThemeRoom、T+1、trade/tax/entitlement lot、AvgCost、ExecutionAllocatedBudget|§15|§15.0/15.2–15.5|
|LAC 五变量、LACDecay|§3.3|§3.3|
|FutureAttention/FutureCapacity、θ^ROLE|§17.2|§17.2|
|AUC、Whipsaw5、ΔStartRate/ΔCDS/ΔPriceShare、GrossAlpha20、θ_ADV、ExitInc|§17.1–17.4、§18|§17 通用观测规则|
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] SettledEstimatedTax/FinalActualTax/TaxTrueUp 正式单位=人民币分整数；QuantizeTaxToFen=floor(x×100)
- [ ] disposed_fen=floor(T_fen×q/Q)；remaining_fen=T_fen−disposed_fen；q=Q→disposed=T；禁浮点
- [ ] 禁 half-up/half-even/truncate/配置另选舍入；局部与全生命周期分整数守恒断言
- [ ] 维持：proportional split 结构；Cash_fen-=TaxTrueUp_fen；EventKey 幂等；disposal<payment
- [ ] α-wealth：v22 k=1；路径 /sfl1_v22/；spec_version=SFL-1_v22.0
- [ ] 不存在未解析参数；`params.yaml` lint 通过
...
## 执行与治理
- [ ] 账本恒等式零失败
- [ ] Run-in 20日完成，并覆盖黄金路径：T_fen=1、Q=2、q=1 → disposed_fen=floor(1×1/2)=0、remaining_fen=1（禁 half-up 得1/0）；T_fen=100、Q=1000、q=300 → disposed=30、remaining=70；q=Q 时 disposed=T；浮点/half-even 静态禁检；Cash_fen-=TaxTrueUp_fen；维持 EventKey 重放幂等
