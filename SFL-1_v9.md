...
**简称：** SFL-TDCRMC
**版本：** v9.0 Final Interface Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v9 Final Interface Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
1. **唯一规范源：** 本文件是 SFL-1 v9.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v9.0"）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v9(theme, D, ...)`：
...
- **共同主 estimand（唯一正式 IUT）：** \(\beta_3\)（\(\widetilde{CCS}\times\widetilde{GCC^{-i}}\)）按 Gate 5 MES/CI 与 §18.0 裁决；
- **稳健性 guardrail（非 IUT）：** （i）CCS_level 顶四分位毛超额 − 容量匹配随机组 > 0；（ii）四分位毛超额对 CCS_level 分位呈非严格单调不减。若正式 IUT PASS 但任一 guardrail 失败 → Gate 5 降级为 INCONCLUSIVE；guardrail 不单独 FAIL；
- **强制诊断（仅报告，不影响判词）：** \(GCC^{\theta}\) 版本回归的交互项方向。
...
- **版本 α-wealth：** SFL-1 项目全生命周期的正式 PASS 预算按解封 HOLDOUT 的版本序号 k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)（\(\sum_{k=1}^{\infty}\alpha_v(k)=0.05\)，故项目级错误 PASS 概率 ≤5%）。**v9 登记 k=1，\(\alpha_v=0.025\)**；v1–v8 均未解封 HOLDOUT，不消耗预算（登记于触碰日志）。后续任何新 SPEC 版本自动取下一序号，禁止重置；
...
**Gate 7 独立生成器（v9；禁止回引 §17.1 / Gate 2）：**

- **事件时点 S：** episode 唯一主信号日 = 该 episode 首次开主信号窗之日（§13.1）；
- **处理组：** S 日首次开主信号窗的真实 episode；
- **独立控制登记簿：** `Gate7_ControlRegistry` 与 `Gate2_ControlRegistry` **完全分离**。“已使用”仅指曾作为**控制**被登记；处理角色不写入任一 ControlRegistry，故 Gate 2 处理题材仍可成为 Gate 7 控制（若满足风险集）；
- **控制风险集：** S−1 已存在、已确认（\(\tau_{confirm}\le S-1\)）、按**其自身原生产冻结 cohort** 在 S 日尚未进入 EARLY_CAPACITY / HEALTHY_TRANSFER、与处理题材 Jaccard < 0.60、与**其他**进行中 episode \(e\neq\)候选控制 的 Jaccard < 0.60（**不**对候选控制自身计算 J=1 自排除）、且不在 `Gate7_ControlRegistry` 中的真实题材；
- **匹配顺序：** 按交易日升序，日内按处理 theme_id 字典序；每成功配对将控制写入 `Gate7_ControlRegistry`（跨日不复用）；
- **匹配变量：** 只允许 S−1 及以前数据；马氏距离、卡尺同 §17.1；
- **控制角色（同构冻结，v9）：** 控制必须使用其自身原有 \(B^E/L^E/K^E\) 与 EPISODE_NULL，**禁止**在 S 日重新任命 Temporary Cohort。EntryScore / CONF / RMI / GCC 一律用控制原冻结制度在 S−1 及以前可算值；容量启动判定亦仅用原生产状态机路径；
- **共同支持：** 处理与控制双方均须满足 Gate 6 的 Eligible≥2 且 NonSelectedEligible≥2；任一方不满足 → 该配对 `OUT_OF_SUPPORT_G7` 剔除；
- **列与账本：** 四格使用 Gate 6 机制口径假想账本与资本规则，不占用生产资金/槽位。
...
**外生冻结入场 tape（v9 唯一生成器）：**

1. **Tape 来源预注册为：** 在隔离研究账户上运行**固定 20 日入口政策**（§16.1）所产生的生产入场成交序列；生成时强制使用固定退出，**禁止**用动态退出政策生成 tape；生成完成并哈希冻结后，方可开庭 Gate 9；
2. tape 字段：`(date, stock, fill_qty, fill_net_cost, origin_episode_id, target_value)`；
3. 两条影子组合**无条件接受同一 tape**，只允许退出规则不同；
4. **隔离资本（等号定义）：** \(InitialCapital = \sum_{\text{tape rows}} target\_value\)（恰好等于，禁止“≥”倍数稀释收益率量纲）；不受 §15 题材簇槽位/60% 总仓硬约束绑定；收益为对该 InitialCapital 的净值路径；禁止声称该装置即完整有限资金生产政策；
5. 生产影子账户可另报端到端动态 vs 固定政策，标记 `DIAGNOSTIC_E2E_POLICY`，不得替代 Gate 9。
...
1. **题材簇 episode 块 bootstrap：** 以 episode 为块、按聚类 ID1（见 §20.2.1）有放回抽取等数量 episode，重算该 Gate 点估计；seed 见 §20.2.3；5,000 次（可升 50,000）。
2. **匹配集合内随机化检验（通用，非仅 P5）：** 在保持处理/控制配对结构下，以 \(Seed=Hash(spec_version,"RAND",gate_id,partition_id,rep)\) 置换处理标签（或 Gate 规定的角色/臂标签），重算效应；真实效应须 ≥ 置换分布的 \(1-\alpha_v\) 分位，否则记该稳健性失败。
...
# 24. 每日生产流程
    ...
    D 15:00以后
      ...
      ├─ load_or_build_nulls_v9；Gate1 日另冻 DISCOVERY_RESEARCH_NULL
      ...
      ├─ 全部 open episode 退出腿（含升级）；代表生成信号与初始预留
...
# 25. 核心伪代码
    ...
    for D in trading_days:
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v9(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v9(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
    ...
    fills = execute_streaming_participation(orders)
...
|版本 α-wealth|0.05×2^{-k}（v9: k=1, α_v=0.025）|—|治理|—|
...
|5 时钟|Gate 4 PASS 域；predictor=\(\tau_{confirm}\)；股×episode|\(\beta_3\)（\(\widetilde{CCS}\times\widetilde{GCC^{-i}}\) at \(\tau_{confirm}\)）|≤0|0.005 / 0|persistent簇×月 CGM|固定顺序第5，α=α_v|仅Gate1–4 PASS后；HOLDOUT一次|顶组/单调=guardrail；GCC^θ=诊断|
|6 选择|Track S；Eligible≥2 且 NonSelectedEligible≥2（episode 等权）|EntryScore前2 − MatchedRandom_Eligible（§18 Gate 6）|≤0|1% / 0|persistent簇×月 CGM|固定顺序第6，α=α_v|仅Gate1–5 PASS后；HOLDOUT一次|不足→OUT_OF_SUPPORT_G6|
|7 交互|Track S；主信号日 S 配对；控制用原 \(B^E/L^E/K^E\)；双方 Gate6 共同支持|\(\Delta_{INT}\)|≤0|1% / 0|persistent簇×月 CGM|固定顺序第7，α=α_v|仅Gate1–6 PASS后；HOLDOUT一次|独立 Gate7_ControlRegistry|
|8 执行|有效期内实际成交信号|NetAlpha20 与 \(-\theta_{ADV}\)|任一不达标|0.01/−0.01；fail 0/−0.01|persistent簇×月 CGM|固定顺序第8；m=2 IUT|仅Gate1–7 PASS后；HOLDOUT一次|两者均PASS才PASS；逆向选择按 §18 Gate 8；其余INCONCLUSIVE|
|9 退出|Track S；固定20日政策生成的冻结 tape；InitialCapital=Σtarget_value|动态−固定 Sharpe差、累计净超额差|任一≤0|ΔSharpe 0.10、ΔCumExcess 0.02；fail均0|同步21日 circular block bootstrap（§20.2.3）|固定顺序第9；m=2 IUT|仅Gate1–8 PASS后；HOLDOUT一次|纯退出增量；非端到端有限资金|
...
**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v9 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
- [ ] 连通分量排除 = §5.3；伪代码更新全部 open episode
- [ ] Gate 5 predictor_date、Gate 9 tape、Kill 风险止损口径已实现
- [ ] 不存在未解析参数
