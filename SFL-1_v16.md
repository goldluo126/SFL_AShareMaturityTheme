...
**简称：** SFL-TDCRMC
**版本：** v16.0 Final Support–Censoring Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v16 Final Support–Censoring Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为 Final Support–Censoring Closure（P10 episode-day 支持域、expected_tax、receivable 净终值、ExecutionAllocatedBudget 伪代码顺序、zero-target Intent、Gate9 cutoff 与 unresolved selection、entitlement-aware StockPositionLock）；不新增题材指标、角色、状态、对照、Gate 或退出腿。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v16.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
- **总回报（v16 唯一，Gate 8A/8B/9 共用）：**
  ...
  **`UnsettledEntitlementTerminalValue_p`（仅 RECOGNIZED，净额唯一公式）：**
  ...
  \]
  `LEGAL_ONLY`→0；`SETTLED` receivable terminal→0（只走实际 Cash/股份处置）；
  \(TotalNetProceeds\) = 卖出净所得 + SETTLED 公司行动实际处置所得 + `UnsettledEntitlementTerminalValue` − `DIV_TAX_CLAWBACK`；
  **`DIV_TAX_CLAWBACK`** 仅减**尚未**计入当前 `TaxPayable` 的后续追缴；禁止与 TaxPayable 双减；
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v16.0"）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v16(theme, D, ...)`：
...
- **StockPositionLock（v16）：** 只要股票满足任一：
  `CurrentShares>0` **或** 开放买入母订单 **或** 未完成卖出母订单 **或** `OutstandingShareGeneratingEntitlement>0`，
  则其他 episode **禁止**创建新 ParentOrder（`REJECT_DUPLICATE_STOCK`）。
  **`OutstandingShareGeneratingEntitlement`** = 该证券上仍存在会转化为实际股份的权益：`LEGAL_ONLY` STOCK_DIVIDEND/TRANSFER、`RECOGNIZED` StockReceivable、未结算换股/successor share entitlement。**现金股息 entitlement 不占用**本锁。
  同日竞争按 §15.1，仅最优者创建 ParentOrder 并成为 owner；loser 不创建 ReservedCash、不消耗入场权。原 parent D+2/D+3 重试不视为新 parent。
  ...
  **释放条件（全部为 false 才可释放）：** CurrentShares=0 且无 OpenBuy 且无 PendingSell 且无 OutstandingShareGeneratingEntitlement。
  **`release_reason` 枚举（禁止自由文本）：** `RELEASE_NO_FILL_EXPIRED` | `RELEASE_FULL_EXIT` | `RELEASE_ENTITLEMENT_VOID` | `RELEASE_LEGAL_CASH_SETTLEMENT` | `RELEASE_CORPORATE_SUCCESSOR_CLOSED` | `RELEASE_CANCELLED_BEFORE_FILL`。
  share-generating entitlement 存续期间 lock owner 保持为原 parent，直至权益作废、法定现金结算、或转股后真实清算完毕。
...
**分红税（v16 唯一算法，覆盖 §15.5.2）：** 税率 = `tax_rate(holding_days, board, tax_rule_version)`。holding_days 与重估见 §15.5.2；仅当税表要求**额外**卖出追缴且该金额尚未计入 `TaxPayable` 时登记 `DIV_TAX_CLAWBACK`。
...
### 15.5.1 CorporateActionEntitlementLot（v16 唯一）
...
### 15.5.2 Receivable 估值与税（v16 唯一）

**现金股息税口径（v16 唯一）：**

    CashReceivable = receivable_cash_gross
    TaxPayable     = expected_tax

**禁止**对 CashReceivable 再做 `(1-tax)` 折现同时又记 TaxPayable。

**`expected_tax` holding_days 输入（强制唯一）：**

    HoldingDaysToFixed20Exit
    = 自 acquire_date（FIFO tax_lot / trade_lot 加权最早取得日）计至入口验证固定20日退出信号日的持有交易日数

    ExpectedHoldingDays_ex
    = max(holding_days_as_of_ex_open, HoldingDaysToFixed20Exit)

    TaxPayable_ex
    = receivable_cash_gross × tax_rate(ExpectedHoldingDays_ex, board, tax_rule_version_ex)

其中 `tax_rule_version_ex` = **ex-date 当日 `INPUT_SNAPSHOT_CUTOFF` 前可知**版本；**禁止**用 payment 后才可知版本回写 ex-date。

**重估算法（唯一）：**

1. **每日开盘前（payment 前）：** 用 `holding_days_as_of_today` 与当日可知 `tax_rule_version` 重算 `TaxPayable`；若仅因持有天数档位变化 → `TAX_HOLDING_REMEASURE`；若因税制版本变化 → `TAX_RULE_REMEASURE`；只向前调整，不回写历史日；
2. **payment 开盘前结算：** `TaxPayable_payment = receivable_cash_gross × tax_rate(holding_days_as_of_payment, board, tax_rule_version_payment)`（payment 日 `INPUT_SNAPSHOT_CUTOFF` 前可知版本）；结算税后现金 = `CashReceivable - TaxPayable_payment`；清零 receivable 与 TaxPayable；
3. **`DIV_TAX_CLAWBACK`：** 仅结算后税表要求的**额外**追缴，且不得重复扣已结算 TaxPayable。

|类型|RECOGNIZED 起估值|
|---|---|
|CashReceivable|`receivable_cash_gross`（毛额）；配套 `TaxPayable` 按上款|
...
|CODE_CHANGE / ABSORPTION|开盘前映射 successor；持仓、entitlement、成本整体迁移|
...
- **版本 α-wealth：** 正式 PASS 预算按**实际解封 HOLDOUT 的版本序号** k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)。**HOLDOUT 触碰日志（v16 唯一声明）：** v1–v15 均未解封 HOLDOUT，已消耗集合为空；**本 Canonical `SFL-1_v16.0` 为 k=1，\(\alpha_v=0.025\)**。后续任一新 SPEC 若解封 HOLDOUT 必须取下一 k，禁止重置；不得并行登记多个“当前 k=1”；
...
**科学问题选择（v16 Canonical）：PairResolvableByCutoff 上 parent 配对退出增量。** 不构造共享有限现金组合，不回答端到端资金政策。
...
**推断装置（v16）：** parent 层观测；ID1 = persistent component；ID2 = 该 parent 首笔 fill 月；主推断 = 题材簇×月 CGM；稳健性 = `G9_COMP_BOOT`（默认 5,000，可升 50,000）。
...
**Unresolved selection（不仅诊断）：** 强制报告 `UnresolvedRate_dynamic`、`UnresolvedRate_fixed20`（分母=全部 tape parent）。令
`ΔU = UnresolvedRate_fixed20 - UnresolvedRate_dynamic`。
若 `ΔU` 的单侧 \(1-\alpha_v\) 上界 > `G9_UNRESOLVED_DIFF_TOL`（§27，默认 0.05），或任一侧 unresolved rate > `G9_UNRESOLVED_ABS_TOL`（默认 0.20）：登记 `G9_SELECTION_UNRESOLVED` 且 Gate 9 **必须**为 `INCONCLUSIVE`（不得 PASS）。否则若 Population 内 IUT PASS 且无 FAIL → 可 PASS（仍受 §20.5 动态护栏约束）。
...
**真实交易日 D 的唯一顺序（v13）：**
    ...
    D 15:00以后（信号日）
      ...
      ├─ load_or_build_nulls_v16；Gate1 冻 DISCOVERY_RESEARCH_* 
    ...
    for D in trading_days:
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v16(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v16(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                ...
                m["nulls"] = load_or_build_nulls_v16(
...
|版本 α-wealth|0.05×2^{-k}（v16: k=1, α_v=0.025；HOLDOUT 从未消耗）|—|治理|—|
...
本规格的 Canonical 纪律：
    ...
    P10_EVALUABLE_EPI_DAY 绑定 R_y；expected_tax 唯一 holding_days；
    UnsettledEntitlementTerminalValue=净应收；share-entitlement 占用 StockPositionLock；
    09:35：reallocate→ExecutionAllocatedBudget→佣金→执行；零金额 Intent PolicyReturn=0；
    G9_OUTCOME_CUTOFF=min(fill+90,partition_end)；PairResolvableByCutoff；ΔU超限 INCONCLUSIVE；
    release_reason 枚举；α-wealth v16 k=1；路径 /sfl1_v16/。
...
|9 退出|Track S；固定买入 tape；PairResolvableByCutoff|parent 首笔 fill 月|θExitMean 与 θExitComp（条件域）|任一≤0|各0.01 / 0|persistent簇×月 CGM；G9_COMP_BOOT|固定顺序第9；m=2 IUT|仅Gate1–8 PASS后；HOLDOUT一次|cutoff=min(fill+90,partition_end)；ΔU超限→INCONCLUSIVE；禁外推全部 parent|
...
**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v16 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] P10_EVALUABLE_EPI_DAY 仅在 R_y 上求和；RealThemeDayKey；denom=0
- [ ] expected_tax=ExpectedHoldingDays_ex；每日/税制重估；terminal=CashRecv−TaxPay+StockRecv
- [ ] StockPositionLock 含 OutstandingShareGeneratingEntitlement；release_reason 枚举
- [ ] 伪代码：reallocate→ExecutionAllocatedBudget→佣金→执行；零金额 Intent PolicyReturn=0
- [ ] G9_OUTCOME_CUTOFF；PairResolvableByCutoff；ΔU/绝对未清算超限→INCONCLUSIVE
- [ ] Provisional→Final AllIn；eligible_buy_orders；odd-lot 分支
- [ ] α-wealth：v16 k=1；路径 /sfl1_v16/；spec_version=SFL-1_v16.0
...
## 执行与治理
- [ ] 账本恒等式零失败
- [ ] Run-in 20日完成，并覆盖黄金路径：P10 在非可评估 theme-day 运行日不计入 PseudoEpisodeDays；ExpectedHoldingDays_ex=max(当前,固定20)；payment 前 holding 档位变化触发 TAX_HOLDING_REMEASURE；terminal 用 CashRecv−TaxPay；卖光股份后仍有 StockReceivable → lock 不释放；09:35 先 reallocate 再算预算；IntendedTargetValue=0 → PolicyReturn=0；fill+90 前未清算 → G9_NON_EVALUABLE；ΔU>0.05 → Gate9 INCONCLUSIVE；release_reason 仅枚举值
