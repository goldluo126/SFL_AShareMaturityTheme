...
**简称：** SFL-TDCRMC
**版本：** v20.0 Tax-Posting Final Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v20 Tax-Posting Final Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为 Tax-Posting Final Closure（disposal-before-payment 税制版本优先、TaxTrueUp 现金过账与幂等）；不新增题材指标、角色、状态、对照、Gate 或退出腿。已 PASS 的 parent-specific tax allocation、PlannedExitDate_arm 及更早闭包维持不变。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v20.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v20.0"）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v20(theme, D, ...)`：
...
**分红税（v20 唯一算法，覆盖 §15.5.2）：** 税率 = `tax_rate(holding_days, board, tax_rule_version)`。holding_days、`PlannedExitDate_arm`、parent 级税额、disposal-before-payment 税制版本、`TaxTrueUp` 现金过账与幂等见 §15.5.2–15.5.3。
...
### 15.5.1 CorporateActionEntitlementLot（v20 唯一）
...
1. record date：由 `source_tax_lot_alloc` 为每个 `(entitlement_id, tax_lot_id)` 生成初始 slice：
   `{slice_id, entitlement_id, tax_lot_id, shares, acquire_date, parent_order_id, disposal_date=null}`；
2. **每次部分卖出：** 将被卖出股数从 remaining slice 拆出 `disposed slice`（写入 `disposal_date=实际卖出日`）与 `remaining slice`（`disposal_date=null`）；禁止用单一 lot 级 `actual_disposal_date` 代表全部份额；
...
**卖出成交后唯一顺序（与 §26 伪代码同构）：**
    ...
    → split_tax_slices_by_sell_fills(sell_fills)   # 写 disposal_date
...
### 15.5.3 Parent / lot 分摊（v20：税额按 tax-slice parent 归集）
...
- **版本 α-wealth：** 正式 PASS 预算按**实际解封 HOLDOUT 的版本序号** k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)。**HOLDOUT 触碰日志（v20 唯一声明）：** v1–v19 均未解封 HOLDOUT，已消耗集合为空；**本 Canonical `SFL-1_v20.0` 为 k=1，\(\alpha_v=0.025\)**。后续任一新 SPEC 若解封 HOLDOUT 必须取下一 k，禁止重置；不得并行登记多个“当前 k=1”；
...
**真实交易日 D 的唯一顺序（v13）：**
    ...
    D 15:00以后（信号日）
      ...
      ├─ load_or_build_nulls_v20；Gate1 冻 DISCOVERY_RESEARCH_* 
...
# 25. 核心伪代码
    ...
    for D in trading_days:
        ...
        disposed_tax_slices = split_tax_slices_by_sell_fills(sell_fills)  # disposal_date
        trueup_events = compute_final_actual_tax_and_trueup(disposed_tax_slices)
        # disposal<payment → TaxTrueUp=0 / NOT_REQUIRED；else TaxTrueUp=Final-SettledEstimated
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v20(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v20(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                ...
                m["nulls"] = load_or_build_nulls_v20(
...
|版本 α-wealth|0.05×2^{-k}（v20: k=1, α_v=0.025；HOLDOUT 从未消耗）|—|治理|—|
...
本规格的 Canonical 纪律：
    ...
    disposal<payment 用 FinalActualTax（disposal 税制）；TaxTrueUp 过账 Cash 且 EventKey 幂等；
    α-wealth v20 k=1；路径 /sfl1_v20/。
...
|入场、执行与账本|§13–§16|ExecutionAllocatedBudget、AllInUnitCost、entitlement 三时点、tax-slice parent 税额、PlannedExitDate_arm、TaxTrueUp 现金过账/幂等、odd-lot 真实清算、StockPositionLock|
...
**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v20 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] disposal<payment：SettledTaxAtPayment=FinalActualTax（disposal 税制）；TaxTrueUp=0；禁 payment 税制覆盖
- [ ] 卖出后顺序：split slices → FinalActualTax → TaxTrueUp → Cash-=TaxTrueUp → parent → POSTED → assert
- [ ] TaxTrueUpEventKey 幂等；trueup_status∈{NOT_REQUIRED,PENDING,POSTED}；禁入 SettledCorporateActionCash
- [ ] 维持：parent tax 归集；PlannedExitDate_arm；slices；TradingDays；LockGroup；collision
- [ ] α-wealth：v20 k=1；路径 /sfl1_v20/；spec_version=SFL-1_v20.0
...
## 执行与治理
- [ ] 账本恒等式零失败
- [ ] Run-in 20日完成，并覆盖黄金路径：disposal 日税制V1=20%、payment 前改V2=10% → SettledDividendCash 按80（禁90）且 TaxTrueUp=0；payment 后处置 true-up：Cash-=TaxTrueUp 且 NAV 同步；同一 EventKey 重放不重复过账；TaxTrueUp 不进入 SettledCorporateActionCash；维持 100/80 parent 分摊与 PlannedExitDate fallback
