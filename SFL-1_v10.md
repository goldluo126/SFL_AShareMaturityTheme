...
**简称：** SFL-TDCRMC
**版本：** v10.0 Execution–Measurement Final Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v10 Execution–Measurement Final Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
1. **唯一规范源：** 本文件是 SFL-1 v10.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
2. 假想买入：按 §14.1 人民币母订单完整算法执行；**补单生命周期与配对真实信号同构**——真实信号在 D+1/D+2/D+3 的每一次尝试日，对照成员在相同尝试日重新做 09:35 判定并继承 RemainingTargetValue；对照不得在真实未尝试日自行成交；若真实首日未成交而对照可成交，对照仍须等待真实 \(\tau_{entry}\) 对齐后的持有时钟，但其成交路径按自身尝试结果记账；
3. **篮子 TargetValue：** 整个对照篮子总目标金额 = 配对真实信号的单笔 `ParentOrderTargetValue`；成员等权拆分人民币；禁止每只对照各获一份；
4. 全部拒绝判定同构适用；
5. REJECT_GAP：与真实信号相同状态、对照股自身板块的同一阈值表；
6. 决策被拒成员从篮子剔除且额度不重分；部分成交按实际金额计权；未成交为现金收益 0；篮子收益 = 成交成员金额加权；
7. 卖出：与真实信号同一卖出日（共享退出时钟），§14.3 + T+1 + §14.4 同构；
8. \(C^{valid}\) = 成交成员非零的篮子；\(|C^{valid}|<100\) → episode 层主终点 `NON_EVALUABLE`。
...
### 3.5.1 FrozenMetricBackcast（v10 唯一）

在 \(\tau_{confirm}\) 日完成 \(B^E/L^E/K^E\) 与 EPISODE_NULL 冻结后，对所有需要历史差分/均值的冻结变量，**一律**用该冻结 cohort 与 EPISODE_NULL，对所需历史窗口做 PIT 回算：

- 覆盖：\(GCC^{\theta/-i}\)、\(GCCSlope\)、\(RMI^E\)、\(CapacityBreadth^E\)、\(HHI^E\)、\(\Delta_3 EMA_3(\cdot)\)、确认斜率所用 IDS 历史等；
- 回算窗口：至少覆盖该变量定义所需的最长历史（通常 D−3:D−1 或 EMA 初始化窗）；
- **禁止**将确认前临时 B/L/K + DAILY_NULL 的序列与确认后冻结量尺拼接；
...
则 \(u_2\) 在账务上合并入 \(u_1\)（v10 measurement 不合并）：

- 保留 \(u_1\) 与 \(u_2\) 各自的 episode_id、冻结角色、EPISODE_NULL 与状态机；\(u_2\) 登记 `MERGED_INTO = u_1`（账务指针）；
- Entry Cohort / \(B^E/L^E/K^E\) **各自保持冻结存档，不得取并集后重算 level 变量**；
- \(u_2\) 持仓在组合账本上并入 \(u_1\) 的 ThemeRoom 记账单元，但退出仍由各 origin 状态机触发，只卖出该 origin 持仓；
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v10.0"）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v10(theme, D, ...)`：
...
**人民币母订单与流式参与（v10 唯一；09:35–10:00，共 25 根分钟线）：**

母订单以人民币为意图单位，字段：

    ParentOrderTargetValue   # 再分配后的 TargetValue
    RemainingTargetValue     # 09:35 初值 = ParentOrderTargetValue；跨日继承
...
2. 预计含冲击成交价 \(P^{ex}_m = VWAP_m × (1 + ImpactBps_m/10^4)\)（Impact 按本节将成交的子金额迭代一次：先用无冲击价估计数量，再代入 §14.4；若 VWAP=0 则该分钟不成交）；
3. `max_qty_by_value = floor(RemainingTargetValue / P^{ex}_m / 100)×100`；
4. `fill_qty = min(max_qty_by_value, particip_qty)`；成交价 = 该分钟 VWAP，再按 §14.4 计费用与冲击；
5. `RemainingTargetValue -=` 该分钟实际净买入成本（含费用与冲击）；不得为负（若因取整将越过则减少 fill_qty）；
6. 累计至 10:00 或 RemainingTargetValue < 一手最低金额；
7. 全部分钟可参与为 0 → UNFILLED_LIMIT；
8. 部分完成 → `PARTIALLY_FILLED`，当日 `EXPIRED_REMAINDER`，**跨日重试继承 RemainingTargetValue（人民币）**，重试日 09:35 重新做拒绝判定；禁止把首日股数冻结为跨日目标；
9. 有效期尽仍有剩余 → 余额作废。
...
2. **T+1 可卖库存（v10 唯一）：**
   ...
   \]
   `SettledShares` = 截至上一交易日收盘已完成买入结算的股数（当日新买 `UnsettledBuyShares` 于次一交易日开盘转入 Settled）；卖出订单只能消费 `SellableQty`；计划卖出超过可卖时只下达可卖部分，差额顺延并登记 `T1_UNSELLABLE`；
3. 14:30–15:00 逐分钟参与：分钟 \(High_m=down\_limit\) 可参与量为 0，否则成交量 × 5%；成交数量 ≤ 剩余可卖；成交价为该分钟 VWAP；
...
- 无合格信号时持有现金；
- 不强制满仓；
- 同一股票属于多个题材时只持有一份；**其全部权重唯一计入 origin episode（首个持有它的 episode）入场时所属题材簇的 ThemeRoom**；多题材成员身份逐日登记披露；
- 组合已满时后续信号进入有效期重试，不跨有效期排队。

**题材簇上限时点语义（v10 唯一）：** ThemeRoom 在**入场分配时**按当日 §5.3 连通分量计算（entry-time constraint）。若持仓后因连通分量动态合并导致某 persistent/当日簇合计暴露 >20%：
...
每位证券持仓子账必须含：`SettledShares`、`UnsettledBuyShares`、`SellableQty`、`PendingSellReserved`、`AvgCost`。母订单必须含：`ParentOrderTargetValue`、`RemainingTargetValue`、`signal_id`、`parent_order_id`。

每次转移保存时间、原因、前后现金、预留、仓位、可卖库存与槽位快照哈希。

## 15.5 公司行动与证券身份账本（v10）

正式交易账本必须消费 PIT `corporate_actions` 事实表（字段：`security_id`、`effective_date`、`ex_date`、`record_date`、`action_type`、`cash_amount_per_share`、`stock_ratio`、`split_ratio`、`rights_price`、`tax_rule_version`、`successor_security_id`、`delisting_cash_value`、`available_at`）。研究复权价（§4 DG07）**不得**替代本账本。
...
全部公司行动于 `available_at ≤ INPUT_SNAPSHOT_CUTOFF` 的交易日收盘后过账；迟到数据进入下一交易日。DG16：公司行动漏过账或破坏 NAV 恒等式 → `LEDGER_INVALID`。
...
- **Gate 1 / H-DISC：** 真实 D−1 候选题材的未来5日 NewJoin AUC − 其 `DISCOVERY_RESEARCH_NULL_D`（§8.3.3）冻结伪题材路径 AUC，按 Gate 1 MES/CI 裁决；
...
**强制逆向选择对照（v10 信号级配对）：** 对每个理论信号 s 构造一对反事实，避免 rejected 与 later-filled 组重叠：

- `Outcome_actual(s)` = 实际规则（含 REJECT_GAP）下该信号的最终 NetAlpha20；若最终完全未成交则为 0（现金）；
- `Outcome_no_gap(s)` = 仅移除 REJECT_GAP、其余规则不变（仍含涨停/停牌/容量/组合拒绝与流式参与）下的 NetAlpha20；
- 定义 \(\theta_{ADV,s}=Outcome_{no\_gap}(s)-Outcome_{actual}(s)\)；正式 estimand 为信号等权均值 \(\theta_{ADV}\)；非劣界 1 个百分点；
- later-filled 信号只产生一个配对差，不得同时进入“拒绝组”与“成交组”；
- guard PASS：\(\theta_{ADV}\) 的单侧 \(1-\alpha_v\) 上界 < 0.01；
- guard FAIL：Bonferroni 单侧 \(1-0.05/(9\cdot m)\)（m=2）下界 > 0.01 → `FAIL_ADVERSE_SELECTION`；
- 其余 `INCONCLUSIVE_ADVERSE_SELECTION`，Gate 8 整体 INCONCLUSIVE；
- Gate 8 共同主 estimand = 执行 NetAlpha20 与 \(-\theta_{ADV}\)，§18.0 IUT；VALIDATION 与 HOLDOUT 各报告一次。
...
3. **InitialCapital（等号）：** 对每个 `parent_order_id` **只计一次** `original_target_value` 后求和；禁止对多日部分成交行重复累加 target_value；
...
### P10_NULL_EPISODE（v10 唯一对象）

P10 不得以每日新建 `(c,D)` 当作独立完整题材路径。必须构造稳定伪 episode：

|字段|定义|
|---|---|
|`P10_NULL_EPISODE_ID`|`Hash(spec_version,"P10EPI",real_theme_id,fold,spawn_index)` 派生的稳定 ID|
|`discovered_at`|伪题材被纳入发现审计的起始交易日|
|`base_membership`|discovered_at 日按 §8.3 对齐矩阵冻结的成员（此后确认前不得每日重抽成员）|
|确认前 null|对该伪 episode 使用固定 reference fold 的 DAILY 统计更新，**membership 不变**|
|`τ_fake_confirm`|链内首次满足 τ_confirm 三条件之日；当日冻结伪角色与伪 EPISODE_NULL|
|`main_signal_used`|每伪 episode 最多一次主信号（布尔）|
|`closed_at`|按 §12.10 同构关闭码关闭|

**生成：** 对每个真实可评估题材日的 cross-fit control，仅在其作为“新发现”生成时 spawn 一个 P10_NULL_EPISODE；后续交易日推进同一 episode 状态机，不得把相邻日新 `(c,D)` 计为新假 episode。
...
1. **persistent-component 块 bootstrap（v10）：** 以聚类 ID1（persistent overlap component）为**整块**有放回抽样；抽中某 component 则其内全部 episode 一并进入；抽取次数使 episode 总数期望等于原样本；禁止逐 episode 独立有放回破坏簇内依赖。seed 见 §20.2.3；5,000 次（可升 50,000）。
...
3. **周聚类敏感性：** 将 ID2 由自然月改为信号日历周，其余同主装置 CGM，重算 CI。
...
# 25. 核心伪代码
    ...
    for D in trading_days:
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v10(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v10(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
            ...
            # Gate1 样本日：另冻结 DISCOVERY_RESEARCH_NULL（§8.3.3-1b）
            maybe_freeze_discovery_research_null(theme, D, nulls)
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                frozen_metric_backcast(role_cohort, D)  # §3.5.1
    ...
    # D+1 09:35
    reject_and_release_reserves(orders)
    reallocate_cash_at_0935(eligible_orders, PARAMS)
    fills = execute_streaming_participation(orders)  # RemainingTargetValue
    update_reservations_and_positions(fills)
    roll_unsettled_to_settled_next_open()            # T+1
    apply_corporate_actions_pit(D)                   # §15.5
    assert_ledger_identities()
...
本规格的 Canonical 纪律：
    ...
    账务合并不得暗含 measurement/null 合并；
    人民币母订单、T+1 可卖库存与公司行动必须进入正式账本。
...
|入场、执行与账本|§13–§16|人民币母订单、T+1、公司行动、部分成交、预留、退出|
...
|---|---|---|---|---|---|---|---|---|
|1 发现|Track S；共同支持域内 D−1 候选题材日|未来5日 NewJoin AUC：真实−DISCOVERY_RESEARCH_NULL_D|≤0|0.05 / 0|persistent簇×月 CGM|固定顺序第1，α=α_v|HOLDOUT一次；FORWARD仅Kill|null 成员/L 于 D 冻结|
|2 确认|Track S；确认处理 vs §17.1 匹配控制（Control Measurement Cohort）|Whipsaw率：控制−处理|≤0|0.10 / 0|persistent簇×月 CGM|固定顺序第2，α=α_v|仅Gate1 PASS后；HOLDOUT一次|Gate2_ControlRegistry 独立；平衡失败整样本无效|
|3 角色可分离|已确认且 \(L^E/K^E\) 均非空的 episode（episode 等权）|\(\theta^{ROLE}_A,\theta^{ROLE}_C\)|任一≤0|各0.05 / 0|persistent簇×月 CGM|固定顺序第3；m=2 IUT|仅Gate1–2 PASS后；HOLDOUT一次|两者均PASS才PASS；任一FAIL才FAIL；其余INCONCLUSIVE|
|4 迁移|Gate 3 PASS 域内已确认 episode（episode 等权）|\(\Delta StartRate_5,\Delta CDS_5,\Delta PriceShare_5\)|任一≤0|0.10/0.05/0.05；fail均0|persistent簇×月 CGM|固定顺序第4；m=3 IUT|仅Gate1–3 PASS后；HOLDOUT一次|三者均PASS才PASS；任一FAIL才FAIL；其余INCONCLUSIVE|
|5 时钟|Gate 4 PASS 域；predictor=\(\tau_{confirm}\)|\(\beta_3\) at \(\tau_{confirm}\)|≤0|0.005 / 0|persistent簇×月 CGM|固定顺序第5，α=α_v|仅Gate1–4 PASS后；HOLDOUT一次|顶组/单调/Slope>0子样本=guardrail|
|6 选择|Track S；Eligible≥2 且 NonSelectedEligible≥2|\(R^{arm1}_{net}-R^{arm2}_{net}\)（§18 完整净执行）|≤0|1% / 0|persistent簇×月 CGM|固定顺序第6，α=α_v|仅Gate1–5 PASS后；HOLDOUT一次|毛口径仅诊断|
|7 交互|Track S；匹配≤S−1；四臂信号=S；执行=S+1；NoCapacityStartThroughS|\(\Delta_{INT}\) 基于 \(R^{arm}_{net}\)|≤0|1% / 0|persistent簇×月 CGM|固定顺序第7，α=α_v|仅Gate1–6 PASS后；HOLDOUT一次|处理/控制同日选择器|
|8 执行|理论信号（含未成交）|NetAlpha20 与信号级配对 \(-\theta_{ADV}\)|任一不达标|0.01/−0.01；fail 0/−0.01|persistent簇×月 CGM|固定顺序第8；m=2 IUT|仅Gate1–7 PASS后；HOLDOUT一次|no-gap vs actual 配对；禁组重叠|
|9 退出|Track S；固定20日 tape（母订单字段）；InitialCapital=Σ_parent original_target|动态−固定 Sharpe差、累计净超额差|任一≤0|ΔSharpe 0.10、ΔCumExcess 0.02；fail均0|同步21日 block bootstrap|固定顺序第9；m=2 IUT|仅Gate1–8 PASS后；HOLDOUT一次|PASS 仍需动态 §20.5(2–8)|
|S 叠加|端到端 overlay 完整政策 vs 无叠加完整政策（允许入口/退出序列不同）|叠加−无叠加的Sharpe差|≤0|0.10 / 0|月块|独立 Gate S 家族 Holm FWER α_v|HOLDOUT一次；FORWARD仅Kill|不影响主终局；FAIL/INCONCLUSIVE 不进入生产|

**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v10 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
|null 生成器、load-or-build、DISCOVERY_RESEARCH_NULL、SMD、NG、DAG/SLO|§8.3|§8.3.1–8.3.9|
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] FrozenMetricBackcast；合并 measurement 不合并；P10_NULL_EPISODE
- [ ] 人民币母订单、T+1 SellableQty、公司行动账本
- [ ] Gate7 同日信号；Gate6/7 R_arm_net；Gate8 配对 θ_ADV；Gate9 母订单 tape
- [ ] §20.5 公式与终局分支（Gate9 PASS + 动态 guardrail FAIL → PASS_ENTRY_ONLY）
- [ ] 不存在未解析参数
