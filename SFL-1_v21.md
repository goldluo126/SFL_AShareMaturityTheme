...
**简称：** SFL-TDCRMC
**版本：** v21.0 Tax-Basis Split Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v21 Tax-Basis Split Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为 Tax-Basis Split Closure（payment 后 PENDING slice 部分卖出时 SettledEstimatedTax 按股数比例拆分与金额守恒）；不新增题材指标、角色、状态、对照、Gate 或退出腿。已 PASS 的 disposal-before-payment 税制版本、TaxTrueUp 现金过账/幂等及更早闭包维持不变。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v21.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
- **总回报（v21 唯一，Gate 8A/8B/9 共用）：**
  ...
  **现金过账（强制）：** 每个 disposed slice 的 true-up 在卖出成交后原子执行 `Cash -= TaxTrueUp_(e,s)`（负值即增加 Cash），并计入当日 NAV / FreeCash；**禁止**仅在研究 outcome 调整而不改 Cash；**禁止**将 TaxTrueUp 再计入 `SettledCorporateActionCash_p`（防双计）；幂等键与 POSTED 状态见 §15.5.2；
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v21.0"）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v21(theme, D, ...)`：
...
**分红税（v21 唯一算法，覆盖 §15.5.2）：** 税率 = `tax_rate(holding_days, board, tax_rule_version)`。holding_days、`PlannedExitDate_arm`、parent 级税额、disposal-before-payment 税制版本、`TaxTrueUp` 现金过账/幂等、payment 后 `SettledEstimatedTax` 按股拆分见 §15.5.2–15.5.3。
...
### 15.5.1 CorporateActionEntitlementLot（v21 唯一）
...
2. **每次部分卖出（shares + SettledEstimatedTax 同步拆分，v21）：** 将被卖出股数从 remaining/parent slice 拆出 `disposed slice`（写入 `disposal_date=实际卖出日`）与 `remaining slice`（`disposal_date=null`）；禁止用单一 lot 级 `actual_disposal_date` 代表全部份额；
...
**SettledEstimatedTax basis split（v21 唯一；payment 后部分卖出强制）：**

设拆分前父 slice（`trueup_status=PENDING`）为：

    Q = shares_parent_before
    T = SettledEstimatedTax_parent_before
    q = 本次卖出股数（0 < q ≤ Q）

则同步生成：

    SettledEstimatedTax_disposed
    = T × q / Q

    SettledEstimatedTax_remaining
    = T - SettledEstimatedTax_disposed

守恒约束（强制）：

    SettledEstimatedTax_disposed + SettledEstimatedTax_remaining
    = SettledEstimatedTax_parent_before

两个 child 均继承：`entitlement_id`、`tax_lot_id`、`acquire_date`、`parent_order_id`、payment 税元数据；`trueup_status=PENDING`。
disposed child 再算 `TaxTrueUp = FinalActualTax_disposed - SettledEstimatedTax_disposed`；
remaining child 保留 `SettledEstimatedTax_remaining`，供后续部分卖出继续拆分。

**金额精度与尾差（强制）：** 使用冻结定点金额精度或精确有理数。若必须分币舍入：disposed 按冻结舍入规则计算；**全部舍入尾差保留在 remaining**；最终清算时最后一个 disposed child 获得全部剩余 baseline。禁止各 child 独立四舍五入导致 `Σ child SettledEstimatedTax ≠ payment 时原始 SettledEstimatedTax`。

**禁止**实现：将全部 T 放入 disposed、将全部 T 留给 remaining、或 disposed 基线=0。

若拆分发生在 payment **之前**（尚无 SettledEstimatedTax）：只拆 shares / disposal_date；估计税仍按 payment 日分支写入（§上款）。
...
**卖出成交后唯一顺序（与 §26 伪代码同构）：**
    ...
    → split_tax_slices_and_settled_tax_basis_by_sell_fills(sell_fills)
      # 拆 shares；写 disposal_date；若 PENDING 则按股数比例拆 SettledEstimatedTax 并守恒检查
...
### 15.5.3 Parent / lot 分摊（v21：税额按 tax-slice parent 归集）
...
- **版本 α-wealth：** 正式 PASS 预算按**实际解封 HOLDOUT 的版本序号** k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)。**HOLDOUT 触碰日志（v21 唯一声明）：** v1–v20 均未解封 HOLDOUT，已消耗集合为空；**本 Canonical `SFL-1_v21.0` 为 k=1，\(\alpha_v=0.025\)**。后续任一新 SPEC 若解封 HOLDOUT 必须取下一 k，禁止重置；不得并行登记多个“当前 k=1”；
...
**真实交易日 D 的唯一顺序（v13）：**
    ...
    D 15:00以后（信号日）
      ...
      ├─ load_or_build_nulls_v21；Gate1 冻 DISCOVERY_RESEARCH_* 
...
# 25. 核心伪代码
    ...
    for D in trading_days:
        ...
        # PENDING：SettledEstimatedTax_disposed = T×q/Q；remaining = T - disposed；守恒检查
        trueup_events = compute_final_actual_tax_and_trueup(disposed_tax_slices)
        # disposal<payment → TaxTrueUp=0 / NOT_REQUIRED；else TaxTrueUp=Final-SettledEstimated(child)
        post_tax_trueup_cash_once(trueup_events)  # Cash -= TaxTrueUp；EventKey 幂等；已 POSTED 跳过
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v21(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v21(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                ...
                m["nulls"] = load_or_build_nulls_v21(
...
|版本 α-wealth|0.05×2^{-k}（v21: k=1, α_v=0.025；HOLDOUT 从未消耗）|—|治理|—|
...
本规格的 Canonical 纪律：
    ...
    payment 后 PENDING 部分卖出：SettledEstimatedTax 按股比例拆分且守恒；尾差留 remaining；
    α-wealth v21 k=1；路径 /sfl1_v21/。
...
|入场、执行与账本|§13–§16|ExecutionAllocatedBudget、AllInUnitCost、entitlement 三时点、tax-slice parent 税额、PlannedExitDate_arm、TaxTrueUp 过账/幂等、SettledEstimatedTax 按股拆分、odd-lot 真实清算、StockPositionLock|
...
**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v21 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] payment 后 PENDING 部分卖出：SettledEstimatedTax_disposed=T×q/Q；remaining=T−disposed；守恒
- [ ] 舍入尾差留 remaining；末次清算获得全部剩余 baseline；禁独立四舍五入破坏守恒
- [ ] split_tax_slices_and_settled_tax_basis_by_sell_fills 同步拆 shares+税基线
- [ ] 维持：disposal<payment 税制；Cash-=TaxTrueUp；EventKey 幂等；parent tax；PlannedExitDate
- [ ] α-wealth：v21 k=1；路径 /sfl1_v21/；spec_version=SFL-1_v21.0
...
## 执行与治理
- [ ] 账本恒等式零失败
- [ ] Run-in 20日完成，并覆盖黄金路径：payment 后 1000股 SettledEstimatedTax=100，卖出300 → disposed基线30、remaining70；FinalActualTax_disposed=60 → TaxTrueUp=+30、Cash-=30；再卖700 时 remaining 基线严格=70且全生命周期 ΣSettledEstimatedTax child=100；禁全部基线给 disposed/remaining/0 的静态检查；维持 disposal<payment 税制与 EventKey 重放幂等
