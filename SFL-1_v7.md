...
1. **唯一规范源：** 本文件是 SFL-1 v7.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
2. 假想买入：与真实信号同一 \(\tau_{entry}\)（真实信号首次非零成交日），按 §14.1 完整算法执行（09:35 决策 + 09:30–09:34 gap + 逐分钟 5% 参与流式成交），假想目标金额与真实信号的 TargetValue 相同；
3. 全部拒绝判定同构适用（UNFILLED_SUSPENSION / UNFILLED_LIMIT / REJECT_CAPACITY）；
4. REJECT_GAP：对照股使用与配对真实信号**相同状态**、**对照股自身所属交易板块**的同一张阈值表（§14.1）判定；
5. 决策被拒的对照股从该篮子剔除；参与成交为部分的按实际成交金额计权；篮子收益 = 成交成员按成交金额加权；成交成员为零的篮子从平均中剔除；
6. 卖出：与真实信号同一卖出日，按 §14.3 同构流式参与执行；成本规则同构（§14.4）；
...
### 8.3.3 生成算法（确定性）与两阶段生命周期（v7 修复 A-04）
...
4. P10 中假真实题材（control）的角色锚点 = 其在完整链中的**假确认日**（链内首次满足 \(\tau_{confirm}\) 三条件之日），同构冻结其伪角色。
...
4. **NullMembership：** 确认前每题材日一次（DAILY_NULL）、确认日一次并冻结整个 episode（EPISODE_NULL，§8.3.3）；生成最多200个伪题材的 `recipient_member → donor_member` 对齐矩阵；IES、IDS、CCS、GCC、Gate 对照和 P10 必须共享对应阶段的冻结矩阵，不得各自重新抽样。
...
## 12.10 episode 关闭路径穷举（v7 新增，A-03）
...
**v7 执行时钟不变量（B-02 修复）：** 任何交易决策只能使用其决策时点前已经完整可见的行情；禁止使用尚未结束窗口的 VWAP、成交量或任何聚合量决定同一窗口内的成交。v6 以 09:35–10:00 完整 VWAP 决定是否在同一窗口按该 VWAP 成交，属执行时钟穿越，已废止。
...
其中 Q0.85 按 §6.7 唯一定义。阈值分布使用与本节相同的 g 定义，取自历史同状态（EARLY_CAPACITY / HEALTHY_TRANSFER 分别统计）、同交易板块信号的条件跳空经验分布，随样本滚动更新并逐日归档版本。
...
- 无合格信号时持有现金；
- 不强制满仓；
- 同一股票属于多个题材时只持有一份；**其全部权重唯一计入 origin episode（首个持有它的 episode）所属题材簇的 20% 上限与暴露归因，其他题材簇的 ThemeRoom 不重复计入**；episode 合并时归属随账务转入承接 episode；多题材成员身份逐日登记披露（v7 修复 A-09）；
...
- **距离与算法：** 7 维马氏距离（当日全体风险集题材协方差，对角加 \(10^{-6}\)），对每个处理题材取最近 1 个，处理题材按 theme_id 字典序依次匹配，**同日无放回**；控制题材跨日可复用；
- **卡尺：** 马氏距离 ≤ VALIDATION 区全部配对距离经验分布的 90% 分位（冻结）；超出 → 该处理题材记 `UNMATCHED` 剔除，剔除率入报告；
- **平衡审计：** 配对后全协变量 |SMD| ≤ 0.25（等权口径，公式同 §8.3.5），超限维度披露，任一超限该配对无效；
- **依赖：** 配对差为观测单位，聚类按 §20.2。

### 主指标（AUC 唯一定义，v7 修复 A-07）
...
### 两个共同主 estimand（episode 等权，v7 修复 A-07）
...
**主时钟使用候选剔除版 \(GCC^{-i}\)（v7 修复：容量组仅3只时候选可经 \(GCC^{\theta}\) 抬高自身题材时钟）：**
...
- **版本 α-wealth（v7 新增，B-06）：** SFL-1 项目全生命周期的正式 PASS 预算按解封 HOLDOUT 的版本序号 k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)（\(\sum_k\alpha_v<0.05\)）。**v7 登记 k=1，\(\alpha_v=0.025\)**；v1–v6 均未解封 HOLDOUT，不消耗预算（登记于触碰日志）。后续任何新 SPEC 版本自动取下一序号，禁止重置；
...
- **FAIL 与项目级 false-death 预算（v7 新增，B-06）：** 对反向命题 \(\theta_{g,k}<MES^{fail}_{g,k}\)，每个 elementary 反向检验使用单侧置信度 \(1-0.05/(9\cdot m_g)\) 的 Bonferroni 同时上界；任一共同主 estimand 的该上界低于其 \(MES^{fail}\) 才 FAIL。由此单版本内"至少一个真机制被错误 FAIL"的概率 ≤5%（false-death FWER 预算，独立于 PASS 预算）；
...
同一题材、同一入场日的四臂比较。**四臂统一机制口径假想账本（v7 闭合 A-06）：** 每臂在入场日以等权 2 只（或声明只数）按 §14.1 完整执行语义假想成交（同拒绝、同参与、同成本），非生产组合账本；生产目标权重版为诊断报告。臂定义：

1. **EntryScore 前 2：** 生产排序（§13.2）取前 2；\(|K^{trade}|<2\) 的题材日剔除并登记；
2. **MatchedRandom 2 只（唯一生成器）：** 在 \(K^{trade}\setminus\){EntryScore 前 2} 中，对 EntryScore 前 2 逐一匹配：以（log自由流通市值, log ADV20）VALIDATION 标准化欧氏距离取最近 5 只，以 \(Seed=Hash(spec\_version,"G6",theme\_id,D,k)\)（§6.9 协议，k=1,2）等概率抽 1，无放回；候选不足 5 取全部；不足 2 只该题材日剔除并登记；
3. **全部可交易容量股等权：** \(K^{trade}\) 全体；
...
- 第一行 = 主信号日的真实 episode；
- 第二行 = **§17.1 控制组生成器输出的"同日未确认真实题材"**（不是 §8.3 伪题材，不是已确认未启动题材）；其"容量候选"按 §9.1 于匹配日在其成员上同构临时构造（无确认冻结），EntryScore 同构计算（其 CONF/RMI 等取自身值）。

**列与账本同构：** 四格全部使用 Gate 6 的机制口径假想账本（等权 2 只、同执行、同成本、同拒绝），共享同一执行语义，不占用生产资金/槽位——Gate 7 是机制交互检验，组合层可实现性由 Gate 8 裁决。
...
动态退出相对固定20日持有改善风险调整后净收益。**两政策使用完全相同的入场信号与入场成交序列，只允许退出规则不同；**否则不构成退出增量识别。走廊测量为强制输入。

**推断装置（v7 闭合 B-05：组合级，替代题材簇×月聚类）：**
...
**Monte Carlo 精度闸门：** bootstrap/随机化检验使用不少于5,000次；若版本阈值 \(\alpha_v\)（或 false-death 阈值、Gate S Holm 阈值）两侧的 Monte Carlo 95% 二项置信区间跨越决策阈值，自动增加到50,000次；仍跨越则 INCONCLUSIVE。
...
上述9个正式 Gate 使用 §18.0 固定顺序 gatekeeping（每 Gate 使用版本预算 \(\alpha_v\)）；每个复合 Gate 使用 IUT；单版本核心家族强 FWER≤\(\alpha_v\)，项目级任一版本错误 PASS 概率 <5%，单版本 false-death FWER≤5%。不得在前序未 PASS 时计算或解释下游正式 p 值。
...
**序贯边界（v7 新增，B-06）：** 绩效 Kill 在 FORWARD 期最多开庭 6 次（每季度一次，自满足 §21.5 起）；每次开庭使用名义单侧水平 \(0.05/6\)（即置信度 \(1-0.05/6\approx99.17\%\) 的上界），Bonferroni 保证整个 FORWARD 期误杀（false-death）概率 ≤5%。触发条件（任一）：

- FORWARD episode 层 NetAlpha20 的单侧 \(1-0.05/6\) 置信上界 < 0；或
- 暴露匹配累计净超额 < −5%，且过去连续两个季度均为负；或
- 成本翻倍压力下累计净超额 < −10%。

（后两条为预注册确定性护栏，不消耗序贯预算。）
...
# 24. 每日生产流程
    ...
    D+1 09:35（决策时点）
      ├─ 用 09:30–09:34 数据完成 gap 与全部拒绝判定
      └─ 新/重试订单按统一排序分配
...
|版本 α-wealth|0.05×2^{-k}（v7: k=1, α_v=0.025）|—|治理|—|
|false-death 预算|每elementary 1−0.05/(9m)|置信度|治理|—|
|绩效 Kill 开庭|6次 / 名义0.05/6|季 / 单侧|治理|—|
...
|2 确认|题材级确认处理 vs §17.1 生成器匹配的未确认真实题材|Whipsaw率：控制−处理|≤0|0.10 / 0|题材簇×月|固定顺序第2，α=α_v|仅Gate1 PASS后；HOLDOUT一次|§18.0；不足→INCONCLUSIVE|
...
|IES、IDS 两级、CONF|§8.4–8.6|§8.4|
|null 生成器、SMD、NG、DAG/SLO|§8.3|§8.3.4–8.3.9|
