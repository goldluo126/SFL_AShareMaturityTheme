...
**简称：** SFL-TDCRMC
**版本：** v11.0 Final Clock–Execution Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v11 Final Clock–Execution Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为时钟—执行—统计接口闭合；不新增题材指标、状态、对照或 Gate。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v11.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
2. **同日可执行对照（v11 唯一）：** 对照篮子**仅从真实信号的 \(\tau_{entry}\) 当日**开始尝试买入与流式参与；在真实 \(\tau_{entry}\) 之前的任何尝试日，对照只做 09:35 可成交性诊断登记（`CONTROL_FILLABLE_BEFORE_ENTRY`），**禁止**提前建仓或累计 RemainingTargetValue 扣减；
...
### 3.5.1 FrozenMetricBackcast（v11 唯一；确认量尺永冻）
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v11.0"）
...
1c. **发现研究真实 cohort（DISCOVERY_RESEARCH_COHORT_D，v11）：** 当题材日 D 进入 Gate 1 / H-DISC 样本时，于 D 日同步冻结真实路径测量制度：`B^{DR}_D` = 当日 Base Universe 快照；`L^{DR}_D` = 当日临时注意力核心；观察集合分母固定为 \(|B^{DR}_D\setminus L^{DR}_D|\)；Gate 1 的 D+1:D+5 真实 NewJoin/IDS AUC **只使用该冻结 cohort**，禁止窗内切换到确认后 \(B^E/L^E\)、禁止消费窗内 vendor 新增成员；与 1b 的 null 路径使用同一冻结制度语义；
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v11(theme, D, ...)`：
...
- **开盘涨停拒单（v11 唯一）：** 当且仅当 09:30—09:34 五分钟**始终**满足 \(Low_m = up\_limit\)（连续无量/无开板封死）→ `UNFILLED_LIMIT`；若 09:30 以涨停价开盘但 09:30—09:34 任一分钟出现 \(Low_m < up\_limit\)（已开板），则**不得**因此拒单，应从开板后首个可参与分钟起流式参与；
...
**人民币母订单与流式参与（v11 唯一；09:35–10:00，共 25 根分钟线）：**
母订单以人民币为意图单位，字段：
...
4. 日终一次结算：令 `DayOrderValue` = 该日子订单全部分钟成交金额之和；`DayCommission = max(DayOrderValue×commission_rate, minimum_commission_cny)`；将已按比例累计扣减的佣金与 `DayCommission` 的差额从 Cash 补扣或释放；若补扣将使净成本超过原 `ParentOrderTargetValue`，则按时间倒序减少最后一笔 fill_qty（100 股步进）直至满足，差额记 `COMMISSION_TAIL_ADJUST`；
...
4. 预计含冲击成交价 \(P^{ex}_m = VWAP_m × (1 + ImpactBps_m/10^4)\)（若 VWAP=0 则该分钟不成交）；用 \(P^{ex}_m\) 重算 `max_qty_by_value` 一次（单次前向修正，不再迭代）；
5. `max_qty_by_value = floor(RemainingTargetValue / P^{ex}_m / 100)×100`；
6. `fill_qty = min(max_qty_by_value, particip_qty)`；成交价 = 该分钟 VWAP，再按 §14.4 计比例费用与本分钟冲击；
7. `RemainingTargetValue -=` 该分钟实际净买入成本（含比例佣金、税费与冲击；不含日终最低佣金差额）；不得为负（若因取整将越过则减少 fill_qty）；
8. 累计至 10:00 或 RemainingTargetValue < 一手最低金额；
9. 全部分钟可参与为 0 → UNFILLED_LIMIT；
10. 部分完成 → `PARTIALLY_FILLED`，当日 `EXPIRED_REMAINDER`，**跨日重试继承 RemainingTargetValue（人民币）**，重试日 09:35 重新做拒绝判定；禁止把首日股数冻结为跨日目标；
11. 有效期尽仍有剩余 → 余额作废。

**"涨停可成交"的唯一判据（v11）：** 分钟线 \(Low_m<up\_limit\) 的分钟才有非零可参与量；开盘涨停后若在 09:30—09:34 已开板，09:35 不得拒单，只能从开板分钟起流式参与；不允许回填为开盘价或窗口 VWAP 成交。
...
\(\sigma_{20}\) 使用日收益率小数尺度；乘以 \(10^4\) 后容量冲击单位为 bps。**顺序冲击 OrderValue（v11）：** 买入流式参与中，第 m 分钟的 `OrderValue` = 截至该分钟（含本分钟拟成交）的累计成交金额；卖出同构。**禁止**用日终总成交额决定盘中分钟的 \(P^{ex}\) 或 fill_qty。日终可另输出“事后全日冲击诊断”，不得回写成交。买入成交价乘以 \(1+ImpactBps/10^4\)，卖出成交价乘以 \(1-ImpactBps/10^4\)。敏感性固定报告 `fixed_slippage_bps ∈ {3,10}` 与 `lambda ∈ {0.25,1.00}`，不得择优替换主值。
...
**题材簇上限时点语义（v11 唯一）：** ThemeRoom 在**入场分配时**按当日 §5.3 连通分量计算（entry-time constraint），并按 origin 永久归属。若持仓后因连通分量动态合并导致某 persistent/当日簇合计暴露 >20%：
...
每位证券持仓子账必须含：`SettledShares`、`UnsettledBuyShares`、`SellableQty`、`PendingSellReserved`、`AvgCost`、FIFO `tax_lots`。母订单必须含：`ParentOrderTargetValue`、`RemainingTargetValue`、`CommissionReserve`、`signal_id`、`parent_order_id`。

每次转移保存时间、原因、前后现金、预留、仓位、可卖库存与槽位快照哈希。

## 15.5 公司行动与证券身份账本（v11）
...
**分红税 lot（v11）：** 每位证券持仓必须维护 FIFO tax lots：`{lot_id, shares, cost_basis, acquire_date, settle_state}`。税后每股现金 = `cash_amount_per_share × (1 - tax_rate(lot, tax_rule_version, payment_date))`。禁止用单一 AvgCost 替代税 lot 计算分红税。

**零碎股（v11）：** 送转/拆合后允许持有非 100 股整数的 `SettledShares` 记账；卖出下单数量必须对 100 股向下取整；零碎股不可卖出，直至公司行动现金兑付或再次行动使其合并为整手；现金兑付按事实表入账。
...
**主 NetAlpha20 样本（v11 唯一，conditional-on-fill）：**

- Population_fill = 有效期内至少一笔非零成交的理论信号；
- 主执行 NetAlpha20 **只在 Population_fill 上**计算（与 §0.3 一致）；
- 完全未成交信号**不**进入主 NetAlpha20；未成交占比、假想收益与 \(\theta_{ADV}\) 另列；
- **禁止**把未成交记 0 后与成交样本混为同一主 MES 样本（那是政策效果，标记 `DIAGNOSTIC_POLICY_RETURN`，不进 Gate 8 主终局）。

**强制逆向选择对照（v11 隔离研究账户信号级配对）：** 对每个理论信号 s：

1. 构造隔离研究账户：固定该信号 `ParentOrderTargetValue`；**不**消费生产组合 `AvailableCash` / `GrossRoom` / `ThemeRoom` / 同日排序槽位；
2. `Outcome_actual(s)` = 在隔离账户上按实际规则（含 REJECT_GAP）得到的最终净结果：成交则用 NetAlpha20，完全未成交则为 0；
3. `Outcome_no_gap(s)` = 同一隔离账户仅移除 REJECT_GAP、其余执行规则不变后的净结果；
4. 定义 \(\theta_{ADV,s}=Outcome_{no\_gap}(s)-Outcome_{actual}(s)\)；正式 estimand 为**全部理论信号**（含未成交）等权均值 \(\theta_{ADV}\)；非劣界 1 个百分点；
5. **禁止**完整生产组合逐信号 rerun 作为 Gate 8 主 guard（组合干扰使“其他信号不变”不成立）；全组合 rerun 仅可作 `DIAGNOSTIC_PORTFOLIO_RERUN`；
6. later-filled 信号只产生一个配对差，不得同时进入“拒绝组”与“成交组”；
7. guard PASS：\(\theta_{ADV}\) 的单侧 \(1-\alpha_v\) 上界 < 0.01；
8. guard FAIL：Bonferroni 单侧 \(1-0.05/(9\cdot m)\)（m=2）下界 > 0.01 → `FAIL_ADVERSE_SELECTION`；
9. 其余 `INCONCLUSIVE_ADVERSE_SELECTION`，Gate 8 整体 INCONCLUSIVE；
10. Gate 8 共同主 estimand = Population_fill 上的执行 NetAlpha20 与全理论信号上的 \(-\theta_{ADV}\)，§18.0 IUT；VALIDATION 与 HOLDOUT 各报告一次。
...
3. **DeployedCapitalBase（v11 唯一资本分母）：**
   - 对 tape 按交易日 t 定义 `ConcurrentGrossNotional_t` = 当日仍持有的全部 tape 仓位按买入净成本计的总名义；
   - `DeployedCapitalBase = max_t ConcurrentGrossNotional_t`；
   - 两条影子组合的初始现金 = `DeployedCapitalBase`；禁止使用“全样本全部 `original_target_value` 之和”作为第一天资本；
   - 仍须对每个 `parent_order_id` 只计一次 `original_target_value` 以生成 tape，但该和只作诊断，不进 Sharpe / ΔCumExcess 分母；
...
**生成 / 新发现 / spawn 算法（v11 唯一）：**

对每个真实可评估题材日 D、每个 cross-fit control `c`：

1. `ControlFingerprint(c,D) = Hash(spec_version,"P10FP",real_theme_id,fold,c, sorted_member_ids_of_control_on_D)`，其中成员为该 control 在 D 日对齐矩阵中的成员集合（升序拼接后再哈希）；
2. 查阅同 `(real_theme_id,fold,c)` 下最近一个仍开放的 `P10_NULL_EPISODE`（若有）；
3. **视为同一伪 episode（不 spawn）：** 存在开放伪 episode，且当日成员集合与该 episode 的 `base_membership` 的 Jaccard 重叠 ≥ 0.80，则推进该 episode 状态机；
4. **新发现并 spawn：** 不存在开放伪 episode，或重叠 < 0.80；若同 fingerprint 的上一伪 episode 已 CLOSED/INVALID，则须满足 cooldown：`D ≥ closed_at + 5` 个交易日，否则当日不 spawn、不计新分母；
5. spawn 时：`spawn_index` 在 `(real_theme_id,fold)` 内按整数从 0 严格递增；写入 `discovered_at=D`、`base_membership`、`base_fingerprint=ControlFingerprint(c,D)`；
6. 禁止把相邻日仅因 `(c,D)` 键变化而计为新假 episode；禁止多个真实题材的相似 control 共用同一 `P10_NULL_EPISODE_ID`。
...
# 24. 每日生产流程
    ...
    D 15:00以后
      ...
      ├─ load_or_build_nulls_v11；Gate1 日另冻 DISCOVERY_RESEARCH_NULL 与 DISCOVERY_RESEARCH_COHORT_D
    ...
    for D in trading_days:
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v11(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v11(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
        ...
        fills = execute_streaming_participation_sequential_impact(orders)
...
本规格的 Canonical 纪律：
    ...
    Gate8 主 NetAlpha20 为 conditional-on-fill；no-gap 必须隔离研究账户；
    Gate9 资本分母为最大并发部署名义，不得用全期订单金额之和稀释。
...
|入场、执行与账本|§13–§16|人民币母订单、顺序冲击、佣金预留、开盘前 T+1/公司行动、部分成交、预留、退出|
...
|8 执行|主 NetAlpha20：Population_fill；θ_ADV：全部理论信号|理论信号日|fill 上 NetAlpha20 与隔离账户配对 \(-\theta_{ADV}\)|任一不达标|0.01/−0.01；fail 0/−0.01|persistent簇×月 CGM|固定顺序第8；m=2 IUT|仅Gate1–7 PASS后；HOLDOUT一次|conditional-on-fill；禁组合干扰|
|9 退出|Track S；固定20日 tape；DeployedCapitalBase=max并发名义|tape 首笔 fill 日所属月用于诊断；主装置仍为21日 block|动态−固定 Sharpe差、累计净超额差|任一≤0|ΔSharpe 0.10、ΔCumExcess 0.02；fail均0|同步21日 block bootstrap|固定顺序第9；m=2 IUT|仅Gate1–8 PASS后；HOLDOUT一次|PASS 仍需动态 §20.5(2–8)|
...
**index_date 规则（v11）：** `ID2 = YYYY-MM(index_date)`，必须回引本表，不得另选。

**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v11 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
|g、流式参与、顺序冲击、佣金预留、部分成交、费用|§14|§14.1/14.3/14.4|
|目标权重、预留、ThemeRoom 归属、T+1、公司行动、tax lots|§15|§15.0/15.2–15.5|
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] 确认量尺永冻；FrozenMetricBackcast 不回写确认 IDS；DISCOVERY_RESEARCH_COHORT_D
- [ ] P10 ControlFingerprint / spawn / cooldown 唯一算法
- [ ] 合并 measurement 不合并且 ThemeRoom 不迁移
- [ ] 对照自真实 τ_entry 起执行；禁止提前建仓共享退出时钟
- [ ] 人民币母订单、顺序冲击、佣金预留、T+1 开盘前 settle、公司行动开盘前+payment_date
- [ ] Gate7 同日信号；Gate6/7 R_arm_net；Gate8 conditional-on-fill + 隔离 no-gap；Gate9 DeployedCapitalBase
- [ ] 附录 B 每 Gate index_date；§20.5 终局分支
- [ ] 不存在未解析参数
