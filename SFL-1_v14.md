...
**简称：** SFL-TDCRMC
**版本：** v14.0 Court–Ledger Consistency Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v14 Court–Ledger Consistency Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为 Court–Ledger Consistency Closure（P10 coverage、entitlement NAV、总回报、StockPositionLock、ExecutionAllocatedBudget、all-in qty、Gate8 Population、Gate9 Canonical、NON_EVALUABLE 时间退出、odd-lot 真实清算）；不新增题材指标、角色、状态、对照、Gate 或退出腿。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v14.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
- **总回报（v14 唯一，Gate 8A/8B/9 共用）：**
  \(BuyNetCost\) = 生命周期实际净买入成本（冲击单次计入）；
  \(TotalNetProceeds\) = 卖出净所得 + 归属于该 parent/trade_lots 的公司行动现金流（已结算）
  + 尚未结算 entitlement 按 §15.5.2 的终值 − 后续税款追缴（`DIV_TAX_CLAWBACK`）；
  \(R^{net} = TotalNetProceeds / BuyNetCost - 1\)；
  卖出成本基础使用 §15.3.1 **AvgCost**；公司行动现金流按 trade_lot → parent 比例分摊（§15.5.3）；
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v14.0"）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v14(theme, D, ...)`：
...
2. 每个执行日 09:35 完成拒绝判定与再分配后，定义：
   `ExecutionAllocatedBudget_t = min(LifecycleRemainingBudget, ReservedCashAfter0935, CurrentStockRoom, CurrentThemeRoom, CurrentGrossRoom, CurrentFreeCash, CurrentCapacityRoom)`（金额口径）；
...
每分钟 m（顺序冲击 + 单次计入 + all-in）：

1. 若 \(Low_m=up\_limit\) → 可参与=0；否则 `particip_qty = floor(0.05×vol_m/100)×100`；
2. **AllInUnitCost（v14 唯一）：**
   `AllInUnitCost_m = P^{ex}_m × (1 + commission_rate + transfer_fee_rate + exchange_fee_rate + buy_stamp_tax_rate)`，
   其中买入 \(P^{ex}_m\) 由下款冲击公式给出；费率取当日 `fee_schedule.yaml`；
3. **TentativeValue：** 先取 `qty0 = min(particip_qty, floor(IntradaySpendableBudget / AllInUnitCost_m / 100)×100)`（VWAP=0 或 AllIn=0 → 不成交）；
   `TentativeValue_m = qty0 × VWAP_m`；
4. `CumOrderValue_{m-}` = 当日至上一分钟累计执行价名义；`OrderValue_m^{impact}=CumOrderValue_{m-}+TentativeValue_m`；
5. `ImpactBps_m` 按 §14.4；买入 \(P^{ex}_m=VWAP_m×(1+ImpactBps_m/10^4)\)；重算 AllInUnitCost_m；
6. `max_qty = floor(IntradaySpendableBudget / AllInUnitCost_m / 100)×100`；`fill_qty=min(max_qty, particip_qty)`；
7. 成交记账价=\(P^{ex}_m\)；再加比例佣金与法定税费；**Impact Single-Count**；
8. `IntradaySpendableBudget -=` 该分钟净成本；不得为负；
9. 至 10:00 或 IntradaySpendableBudget < 一手 all-in；全部分钟可参与0 → UNFILLED_LIMIT；
...
- **StockPositionLock（v14）：** 只要股票满足任一：`CurrentShares>0` **或** 存在开放买入母订单 **或** 存在未完成卖出母订单，则其他 episode **禁止**对该股票创建新 ParentOrder（登记 `REJECT_DUPLICATE_STOCK`）。同日竞争按 §15.1 排序，仅最优者可创建唯一 ParentOrder；其余不创建 ReservedCash、不消耗入场权（有效期内若 lock 全释放仍可重试）。原 parent 的 D+2/D+3 剩余重试**不**视为新 parent。这落实“首次入场后不加仓”与单一 origin 归属；禁止对合并持仓创建第二 parent。
...
**零碎股与最终清算（v14）：** 送转/拆合后允许非 100 股整数记账；常规卖出对 100 向下取整。若退出腿要求清仓且 `0 < SellableQty < 100`：允许整笔 odd-lot 挂出（不受 100 取整），但仍须有真实可参与成交量。**禁止**按收盘价/跌停价做虚拟现金变现。若连续跌停/停牌无法成交：继续持仓，按 `min(close, down_limit)`（停牌用最近收盘）计入 NAV，保持 pending liquidation；**只有**实际市场成交、官方退市现金、正式换股或其他 PIT 法定结算才能清零。研究窗强制估值仅可标记 `SYNTHETIC_TERMINAL_MARK`，且该 parent 在 Gate 8A 记 `NON_EVALUABLE_EXECUTION`，不得当作真实成交。

每次转移保存时间、原因、前后现金、预留、仓位、可卖库存与槽位快照哈希。

## 15.5 公司行动与证券身份账本（v14）
...
### 15.5.1 CorporateActionEntitlementLot（v14 唯一）
...
1. **LegalEntitlementAtRecordDate：** 仅冻结法律 entitlement；`accounting_status=LEGAL_ONLY`；**不**增加 NAV 独立资产（避免含权价与 receivable 双计）；
2. **AccountingRecognitionAtExDate：** ex-date（或 effective）开盘前：原持仓进入除权价格制度；同时 `accounting_status=RECOGNIZED`，确认 `CashReceivable` / `StockReceivableValue` 进入 NAV；
3. **SettlementAtPaymentDate：** payment/effective 结算日开盘前：receivable 转为 `Cash` / 股份；`accounting_status=SETTLED`。
...
### 15.5.2 Receivable 估值（唯一）

|类型|RECOGNIZED 起估值|
|---|---|
|CashReceivable|`receivable_cash_gross × (1 - expected_tax_rate)`，税率按 tax_rule_version 在 payment_date 可知版本；无 payment_date → INVALID_DATA|
|StockReceivableValue|`receivable_stock_shares × P_ref`；`P_ref` = 除权参考价（若可得）否则 successor/本证券 ex-date 开盘前可交易参考价；缺失 → INVALID_DATA|
...
|action_type|账本效果|
|---|---|
|CASH_DIVIDEND|record：创建 cash entitlement；payment：结算税后现金；AvgCost 不变|
|STOCK_DIVIDEND / TRANSFER|record：stock receivable；ex 开盘前到账股数；总成本不变；零碎股见 §15.3.1|
|SPLIT / REVERSE_SPLIT|effective 开盘前：股数 × ratio；AvgCost 按倒数；总成本不变|
...
税后每股现金 = `cash_amount_per_share × (1 - tax_rate(entitlement_lot, tax_rule_version, payment_date))`。
...
- **版本 α-wealth：** 正式 PASS 预算按**实际解封 HOLDOUT 的版本序号** k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)。**HOLDOUT 触碰日志（v14 唯一声明）：** v1–v13 均未解封 HOLDOUT，已消耗集合为空；**本 Canonical `SFL-1_v14.0` 为 k=1，\(\alpha_v=0.025\)**。后续任一新 SPEC 若解封 HOLDOUT 必须取下一 k，禁止重置；不得并行登记多个“当前 k=1”；
...
|对象|定义|用途|
|---|---|---|
|`SignalCandidate`|Gate 6/7 排序后、完成资格检查的候选|排序与诊断|
|`TheoreticalOrderIntent`|已完成 `IntendedTargetValue` 计算的意图（含 StockPositionLock 竞争失败者，若保留 `IntendedTargetValue`）|Gate 8B / θ_ADV 外推域|
|`ParentOrder`|取得 StockPositionLock 并创建的正式母订单|Gate 8A 成交样本、ParentFillRate|

**Population 纪律：** 不得将“全部理论信号”含糊用于无 `IntendedTargetValue` 的 `REJECT_MIN_WEIGHT` 候选。Gate 8 正式外推到 **post-TargetValue intents**；若某拒绝路径不保留 Intent，则明确排除并登记。
...
- **Population_intent：** 全部 `TheoreticalOrderIntent`（含未创建 Parent 的 lock 失败者，若其 `IntendedTargetValue` 已计算；未成交 Intent 的 PnL=0）；
- **Outcome：** `PolicyReturn(s) = RealizedNetPnL(s) / IntendedTargetValue(s)`（已创建 Parent 时 IntendedTargetValue ≡ ParentOrderTargetValue）；
  RealizedNetPnL 使用与 §0.3 同一 `TotalNetProceeds` 函数；未成交 ⇒ 0；
  **禁止**把部分成交上的 NetAlpha20 直接当作 PolicyReturn；
- `ParentFillRate` 分母 = 全部正式 `ParentOrder`（不是 pre-lock SignalCandidate）；
...
对每个 `TheoreticalOrderIntent` s：

1. 隔离账户固定 `IntendedTargetValue`；不消费生产组合 rooms；
2. `Outcome_actual(s)` / `Outcome_no_gap(s)` 同构；成交用 TotalNetProceeds 口径 NetAlpha20，未成交为 0；
3. \(\theta_{ADV,s}=Outcome_{no\_gap}(s)-Outcome_{actual}(s)\)；正式 estimand = Intent 等权 \(\theta_{ADV}\)；非劣界 1 个百分点；
4. **禁止**完整生产组合逐信号 rerun（`DIAGNOSTIC_PORTFOLIO_RERUN`）；
5. guard PASS：\(\theta_{ADV}\) 单侧 \(1-\alpha_v\) 上界 < 0.01；
6. guard FAIL：Bonferroni 单侧 \(1-0.05/(9\cdot m)\)（m=2）下界 > 0.01 → `FAIL_ADVERSE_SELECTION`；
7. 其余 `INCONCLUSIVE_ADVERSE_SELECTION` → Gate 8 INCONCLUSIVE；
8. **Gate 8 IUT** =（8A NetAlpha20）与（\(-\theta_{ADV}\)）；8B 不进 IUT。
...
**科学问题选择（v14 Canonical）：ParentOrder 配对退出增量。** 不构造共享有限现金组合，不回答端到端资金政策。
...
**推断装置（v14 唯一）：** parent 层观测；ID1 = persistent component；ID2 = 该 parent 首笔 fill 月；主推断 = 题材簇×月 CGM；稳健性 = persistent-component 整块 bootstrap（seed 表 `G9_COMP_BOOT`，默认 5,000，可升 50,000）。**删除**旧 ΔSharpe / ΔCumExcess / 21 日 circular block bootstrap 作为 Gate 9 主法庭。
...
### P10_NULL_EPISODE（v14 Stable Episode Generator）
...
年度覆盖（v14 episode-day 公式）：令

    RealEvaluableThemeDays_y = 年 y 真实可评估题材日数
    PseudoEpisodeDays_y = Σ_e（e 在年 y 内且 P10_EVALUABLE_EPI(e)=1 的可评估运行交易日数）
    CoveredRealEvaluableThemeDays_y = 年 y 中至少被一个可评估伪 episode 覆盖的真实可评估题材日数
...
否则 `P10_INSUFFICIENT_YEAR`（40 = 每真实评估题材日最低可评估伪 episode-日暴露，= 200 controls × 最低存活率先验 0.20，冻结于 §27）。**禁止**用新 spawn episode 数惩罚长寿稳定 episode。达标前不得报告 AnnualFalseEpisodes、不得用于 Phase-I/漂移。
...
**真实交易日 D 的唯一顺序（v13）：**
    ...
    D 15:00以后（信号日）
      ...
      ├─ load_or_build_nulls_v14；Gate1 冻 DISCOVERY_RESEARCH_* 
...
# 25. 核心伪代码
    ...
    for D in trading_days:
        ...
        reject_and_release_reserves(buy_orders)  # gap/limit/StockPositionLock
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v14(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v14(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                ...
                m["nulls"] = load_or_build_nulls_v14(
...
|版本 α-wealth|0.05×2^{-k}（v14: k=1, α_v=0.025；HOLDOUT 从未消耗）|—|治理|—|
...
本规格的 Canonical 纪律：
    ...
    ConfirmedEventFlag；TotalNetProceeds 含公司行动；entitlement 三时点 NAV；
    P10 episode-day coverage；每 P10_NULL_EPISODE_ID 审计一次；
    ExecutionAllocatedBudget + AllInUnitCost；CommissionTopUp 只补差额；
    StockPositionLock；Gate8 三层 Population；Gate9 θExitMean/Comp Canonical；
    NON_EVALUABLE 继续时间退出；odd-lot 禁止虚拟变现；
    伪代码只执行 eligible_buy_orders；α-wealth v14 k=1；路径 /sfl1_v14/。
...
**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v14 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] TotalNetProceeds；entitlement 三时点 + NAV 恒等式；FreeCash 不含 receivable
- [ ] P10 episode-day coverage；每 P10_NULL_EPISODE_ID 一次；FSR 仅 evaluable
- [ ] ExecutionAllocatedBudget；AllInUnitCost；CommissionTopUp；StockPositionLock
- [ ] Gate8 SignalCandidate/Intent/Parent；8A/8B/θADV Population 唯一
- [ ] Gate9 θExitMean/θExitComp MES=0.01；G9_COMP_BOOT；§27/附录B/seed 一致
- [ ] NON_EVALUABLE 继续时间退出；odd-lot 无虚拟现金；eligible_buy_orders 静态检查
- [ ] α-wealth：v14 k=1；路径 /sfl1_v14/；spec_version=SFL-1_v14.0
...
## 执行与治理
- [ ] 账本恒等式零失败
- [ ] Run-in 20日完成，并覆盖黄金路径：P10 稳定 episode 连跑10日 coverage 不因未 respawn 失败；同 index respawn 按 episode 计一次；record 建 entitlement 且 ex 前 NAV 不双计；ex 后 receivable 入 NAV 且 FreeCash 不增；Lifecycle=100万但获配60万；一手 all-in 费用超预算；比例佣金后只补最低佣金差额；持仓期间第二题材禁新 parent；REJECT_DUPLICATE_STOCK 的 8B 归属；NON_EVALUABLE 恰逢 Leg C；odd-lot 连跌停不得虚拟变现；θExitComp 大小悬殊 component 权重；eligible_buy_orders 静态检查；§27/附录B/seed Gate9 一致
