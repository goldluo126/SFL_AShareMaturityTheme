...
**简称：** SFL-TDCRMC
**版本：** v8.0 Canonical Consistency Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v8 Consistency Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
1. **唯一规范源：** 本文件是 SFL-1 v8.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
2. 假想买入：与真实信号同一 \(\tau_{entry}\)（真实信号首次非零成交日），按 §14.1 完整算法执行（09:35 决策 + 09:30–09:34 gap + 逐分钟 5% 参与流式成交）；
3. **篮子 TargetValue（v8 唯一）：** 整个对照篮子的总目标金额 = 配对真实信号的单笔 TargetValue；篮子内合格成员按等权人民币拆分（每人 TargetValue/|members|）；**禁止**解释为“每只对照股各获得一份真实 TargetValue”；
4. 全部拒绝判定同构适用（UNFILLED_SUSPENSION / UNFILLED_LIMIT / REJECT_CAPACITY）；
5. REJECT_GAP：对照股使用与配对真实信号**相同状态**、**对照股自身所属交易板块**的同一张阈值表（§14.1）判定；
6. 决策被拒的对照股从该篮子剔除，其未用额度不重新分配给同篮子其他成员；参与成交为部分的按实际成交金额计权；未成交部分视为现金、期内收益为 0；篮子收益 = 成交成员按成交金额加权；成交成员为零的篮子从平均中剔除；
7. 卖出：与真实信号同一卖出日，按 §14.3 同构流式参与执行；成本规则同构（§14.4）；
8. \(C^{valid}\) = 成交成员非零的篮子集合；\(|C^{valid}| < 100\) → 该 episode 的 episode 层主终点记 `NON_EVALUABLE`，登记、不进入主样本、计入 §20.4 样本充分性核查。
...
- 非代表题材：当日不得新确认、不得新开入场信号；已有持仓按自身 origin episode 继续退出；
- 相同股票不重复持仓（§15）；
- 统计推断聚类单元 = 题材簇 × episode（§20.2）；
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v8.0"）
...
1. **确认前逐日 null（DAILY_NULL）：** 题材尚无 episode 时，每个题材日按本节算法生成，seed 含当日 D，只用于 IES、\(IDS^{\theta/-i}\) 与确认判定；伪题材的注意力集合 \(L_c\) 按 §7.3 在其成员上同构临时构造；
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v8(theme, D, ...)`：
...
\]
  ...
  斜率条件所需的 3 日 IDS 历史不足（含 NON_EVALUABLE 缺口）时按不满足。该定义即 §12.3 CONFIRMED_EXPANSION 的前三行；**不包含**容量时钟条件，**不包含**任何候选级（\(IDS^{-i}\)、NewJoinRate、\(|N^{-i}|\)）条件——候选级条件只作用于该候选的入场资格（§8.6、§13.1），不影响 \(\tau_{confirm}\)、Entry Cohort 与 \(B^E/L^E/K^E\) 的冻结日。episode 于 \(\tau_{confirm}\) 日创建，全部冻结集合于当日收盘一次性锁定；
...
**流式参与成交（09:35–10:00，共 25 根分钟线）：**

1. 对每根分钟线 m（按时间顺序）：若该分钟 \(Low_m = up\_limit\)（全程封板）则该分钟可参与量为 0；否则可参与量 = 该分钟成交量 × 5%；
2. 该分钟成交数量 = min(剩余目标数量, 可参与量)，向下取整到 100 股；成交价 = 该分钟 VWAP（Amount/Volume）；
3. 累计至 10:00 或目标完成为止；逐分钟成交只使用该分钟自身数据，不引用任何后续分钟；
4. 全部分钟可参与量为 0 → UNFILLED_LIMIT；
5. 部分完成 → 订单记 `PARTIALLY_FILLED`，未完成余额当日记 `EXPIRED_REMAINDER`，按 §13.3 在剩余有效期内重试（重试日重新做 09:35 决策判定）；
6. 成交成本 = 各成交分钟的数量加权均价，再按 §14.4 加冲击与费用。

**"涨停可成交"的唯一判据（v7 替代 v6"无法证明"表述）：** 分钟线 \(Low_m<up\_limit\) 的分钟才有非零可参与量；开盘涨停后盘中开板，只能从开板分钟起流式参与，不允许回填为开盘价或窗口 VWAP 成交。
...
1. 决策时点 14:30；14:30 前全日一字跌停（\(High=down\_limit\)）或停牌 → 顺延；
2. 14:30–15:00 逐分钟参与：分钟 \(High_m=down\_limit\)（封死跌停）可参与量为 0，否则该分钟成交量 × 5%；成交价为该分钟 VWAP；
3. 未完成余额记 `PARTIALLY_FILLED` 并逐日顺延至首个可执行窗口，顺延期间继续计入净值和风险；
4. 不允许使用窗口结束后才可知的聚合量回填成交。
...
2. 按新腿目标持仓计算剩余应卖数量 = 当前持仓 − 新腿目标持仓（Leg X/B2/C/I 目标为 0；Leg B1 目标为触发前持仓的 50%，若已卖出超过半数则不再新建）；
...
\(\sigma_{20}\) 使用日收益率小数尺度；乘以 \(10^4\) 后容量冲击单位为 bps。**OrderValue 取该日子订单的实际成交金额**（部分成交按实际额计冲击）。买入成交价乘以 \(1+ImpactBps/10^4\)，卖出成交价乘以 \(1-ImpactBps/10^4\)。敏感性固定报告 `fixed_slippage_bps ∈ {3,10}` 与 `lambda ∈ {0.25,1.00}`，不得择优替换主值。
...
- 无合格信号时持有现金；
- 不强制满仓；
- 同一股票属于多个题材时只持有一份；**其全部权重唯一计入 origin episode（首个持有它的 episode）所属题材簇的 20% 上限与暴露归因，其他题材簇的 ThemeRoom 不重复计入**；episode 合并时归属随账务转入承接 episode；多题材成员身份逐日登记披露（v7 修复；v8 维持）；
...
必须保存订单状态机：
    ...
    PARTIALLY_FILLED → (剩余额度重试) SUBMITTED / EXPIRED
...
- **控制复用（v8）：** 同一控制题材在整个 Gate 2 / Gate 7 正式分析样本中**至多使用一次**（跨日亦不可复用）；已被匹配者从后续日风险集永久排除；
...
- `MorningExecutableValueShare5`（v8 公式）：对 \(t=D+1,\ldots,D+5\)，
  \[
  MES5_{i,t}
  =
  \frac{\sum_{m\in\{09:35,\ldots,10:00\}} \mathbf{1}(Low_m < up\_limit)\cdot \min(0.05\cdot vol_m,\ Q)\cdot VWAP_m}
  {ADV20_{i,D}}
  \]
  其中 \(Q\) 为该研究假想订单的目标股数（机制口径下取 \(0.10\times\) 研究隔离 NAV / 开盘价，向下取整到 100 股；与生产 TargetValue 脱钩）；全日停牌日该项为 0。`MorningExecutableValueShare5_i=\sum_t MES5_{i,t}`。
...
- \(\widetilde{CCS}\times\widetilde{GCC^{-i}}\) 交互项按 Gate 5 MES/CI 规则通过；
- 顶组毛超额高于容量匹配随机组；
- 四分位收益具有预期单调性；
- \(GCC^{\theta}\) 诊断回归方向一致（仅报告）。
...
- **版本 α-wealth：** SFL-1 项目全生命周期的正式 PASS 预算按解封 HOLDOUT 的版本序号 k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)（\(\sum_{k=1}^{\infty}\alpha_v(k)=0.05\)，故项目级错误 PASS 概率 ≤5%）。**v8 登记 k=1，\(\alpha_v=0.025\)**；v1–v7 均未解封 HOLDOUT，不消耗预算（登记于触碰日志）。后续任何新 SPEC 版本自动取下一序号，禁止重置；
...
\(GCC^{-i}\) 和 CCS_level 的标准化交互能够预测确认后 20 日机制口径毛超额（§17.4）；必须同时报告启动带 [0.55, 0.85] 的敏感性（带边界 ±0.05 平移）与 \(GCC^{\theta}\) 诊断回归。按附录 B 三态裁决。
...
**共同支持域（v8）：** 正式 Gate 6 样本要求 \(|K^{trade}|\ge 4\)（至少 2 只生产候选 + 至少 2 只非入选容量候选以生成 MatchedRandom）。\(2\le|K^{trade}|<4\) 的 episode 登记 `OUT_OF_SUPPORT_G6`，不进入正式判词，不得外推。

**四臂统一机制口径假想账本（v8 资本规则）：**

- 每臂总资本 = 该 episode 真实生产信号若以 EntryScore 前 2 等权执行时的合计 TargetValue（若真实只成交 1 只，则用两只理论 TargetValue 之和）；四臂总资本相同；
- 臂内成员等权拆分人民币；未成交部分为现金、期内收益 0；
- 2 名臂：至少 1 只非零成交则该臂有效；全容量臂：至少 1 只非零成交有效；无效臂的 episode 整条剔除并登记；
- 按 §14.1 完整执行语义假想成交（同拒绝、同参与、同成本），非生产组合账本；生产目标权重版为诊断报告。

臂定义：

1. **EntryScore 前 2：** 生产排序（§13.2）取前 2；
2. **MatchedRandom 2 只（唯一生成器）：** 在 \(K^{trade}\setminus\){EntryScore 前 2} 中，对 EntryScore 前 2 逐一匹配：以（log自由流通市值, log ADV20）VALIDATION 标准化欧氏距离取最近 5 只，以 \(Seed=Hash(spec\_version,"G6",theme\_id,D,k)\)（§6.9 协议，k=1,2）等概率抽 1，无放回；候选不足 5 取全部；
3. **全部可交易容量股等权：** \(K^{trade}\) 全体（总资本仍与臂 1 相同，按成员数等权拆分）；
4. **题材内 RS 前 2：** \(RS_i=\sum_{t=D-19}^{D}x_{i,t}\)，\(K^{trade}\) 内最高 2 只，并列按股票代码升序。
...
（臂 3、4 为强制对照报告。）每 episode 一个观测、episode 等权，依赖按 §20.2。
...
**Gate 7 独立生成器（v8；禁止回引 §17.1 / Gate 2）：**

- **事件时点 S：** episode 唯一主信号日 = 该 episode 首次开主信号窗之日（§13.1）；
- **处理组：** S 日首次开主信号窗的真实 episode；
- **控制风险集：** S−1 已存在、已确认（有 \(\tau_{confirm}\le S-1\)）、S 日尚未进入 EARLY_CAPACITY / HEALTHY_TRANSFER（尚未容量启动）、与处理题材及进行中 episode Jaccard < 0.60、且尚未被 Gate 2/7 正式样本使用过的真实题材；
- **匹配变量：** 只允许 S−1 及以前数据（成员数、板块构成、log 市值/ADV20 中位数、前20日等权收益与波动、Active 率）；马氏距离、theme_id 序、无放回、卡尺同 §17.1；
- **控制角色（Temporary Cohort at S）：** 在 S 日一次性冻结控制题材的 \(B^{T},L^{T},K^{T}\) 与研究用 `Null^{T}`（§8.3 算法，seed 含 S）；前三日 RMI / GCCSlope 用该 Temporary Cohort 在 S−3:S−1 回算（回算期成员固定为 Temporary Cohort，不得每日重选）；EntryScore / CONF / RMI 均用控制自身 Temporary 值；S 日之后不再更新角色冻结；
- **列与账本：** 四格使用 Gate 6 机制口径假想账本与资本规则（处理臂用真实 \(K^{trade}\)；控制臂用 Temporary \(K^{T,trade}\)），不占用生产资金/槽位。
...
**强制逆向选择对照：** 被 REJECT_GAP 拒绝的信号，假想忽略该拒绝、按 §14.1 流式参与算法在拒绝日成交，计算其 NetAlpha20 并与实际成交组比较：
...
1. 在看任何退出结果前，用生产信号规则生成并冻结入场 tape：`(date, stock, fill_qty, fill_net_cost, origin_episode_id)`；
2. 两条影子组合**无条件接受同一 tape**，只允许退出规则不同；
3. 使用事先充足的**隔离研究资本**（初始现金 ≥ tape 全部 TargetValue 之和，且不受 §15 题材簇槽位/60% 总仓硬约束绑定）；禁止声称该装置即完整有限资金生产政策；
4. 生产影子账户可另报端到端动态 vs 固定政策，但标记 `DIAGNOSTIC_E2E_POLICY`，不得替代 Gate 9 正式 estimand。
...
- 聚类 ID1 = §5.3 连通分量题材簇 ID；聚类 ID2 = 信号日所属自然月（YYYY-MM）；
...
- 5,000 次题材簇 episode 块 bootstrap；
- 匹配集合内随机化检验；
- 按信号日历周聚类替代自然月的敏感性。

主装置与稳健性冲突时标 `INFERENCE_CONFLICT` 并首页披露；**若主装置 PASS 而两项稳健性均方向相反，或正式 IUT PASS 而本节/各 Gate 声明的 guardrail 失败，一律降级为 INCONCLUSIVE**。不得以稳健性单独宣布 PASS。
...
**序贯边界（v8 联合误杀预算）：** 绩效 Kill 在 FORWARD 期最多开庭 6 次（每季度一次，自满足 §21.5 起）。三条触发条件共享同一 false-death 预算 5%，按 Bonferroni 拆为每条每庭名义水平 \(0.05/(3\cdot 6)\)。触发条件（任一）：

- FORWARD episode 层 NetAlpha20 的单侧 \(1-0.05/(18)\) 置信上界 < 0；或
- 暴露匹配累计净超额 < −5%，且过去连续两个季度均为负（该条为预注册阈值检验，同样消耗联合预算）；或
- 成本翻倍压力下累计净超额 < −10%（同样消耗联合预算）。

不得再声称后两条“不消耗预算”。联合预算只保证版本内 FORWARD 绩效 Kill 的误杀概率 ≤5%；跨版本不累加控制（见 §18.0）。
...
# 24. 每日生产流程
    ...
    D 15:00以后
      ...
      ├─ 更新 episode 合并/分裂
      ├─ 按 §3.5：未确认题材识别临时注意力角色；已确认题材加载冻结 L^E/K^E/B^E
      ├─ load_or_build_nulls_v8（DAILY_NULL / 首日冻结 EPISODE_NULL / 此后只加载）
      ├─ 更新 PseudoThemeStats（不重建已冻结 membership）
      ├─ 计算题材级 IDS^θ、IES、GCC^θ（确认后成员集只用冻结集合）
      ├─ 连通分量题材簇 + ConfirmQuality 代表去重（§5.3）
      ├─ 用精确 sufficient-statistics LOO 计算 IDS^-i、GCC^-i
      ├─ 计算 RMI^E、CCS_rank/CCS_level、EntryScore
      ├─ 更新唯一状态机（NON_EVALUABLE 不冻结 LACDecay；§12.0）
      ├─ 生成信号与初始预留；更新有效重试信号
      ├─ 计算退出腿（含未完成卖单升级）
    ...
    D+1 09:35（决策时点）
      ├─ 用 09:30–09:34 数据完成 gap 与全部拒绝判定（共享日初阈值版本）
      ├─ 释放被拒预留
    ...
    for D in trading_days:
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v8(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v8(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
                # mode=EPISODE_NULL: load frozen membership; never resample

            if not pass_theme_gate(base, D):
                continue
            if len(nulls) < 100 or not pass_balance_gate(nulls):
                # NON_EVALUABLE: freeze normal transitions; still apply LACDecay
                apply_risk_branches_only(theme, lac_health, D)
                log_non_evaluable(theme, D)
                continue

            pseudo_stats = update_pseudo_theme_stats(nulls, D)  # membership fixed
        ...
        # Pass 2：连通分量簇 + ConfirmQuality 代表（§5.3）
        representatives = select_cluster_representatives(theme_metrics, D)

        for theme in representatives:
            m = theme_metrics[theme]
            ...
            ccs_levels = carrier_scores_level(role_cohort.K_E, m["pseudo_stats"])
        ...
        emit_exit_orders_with_upgrade(open_episodes, D)  # §14.3
        reserve_initial_cash(signals, retries, PARAMS)   # D-evening placeholder
        assert_ledger_identities()
        enforce_compute_slo_or_quarantine(D)
        run_drift_checks()

    # D+1 09:35
    reject_and_release_reserves(orders)                 # gap etc.
...
|版本 α-wealth|0.05×2^{-k}（v8: k=1, α_v=0.025）|—|治理|—|
|false-death 预算|每elementary 1−0.05/(9m)|置信度|治理|—|
|绩效 Kill 开庭|6次 / 每条名义0.05/18（三条件联合）|季 / 单侧|治理|—|
...
|发现、集合与变量 owner|§3、§5|Track S/B 主法庭、B/E/A、§3.5 确认前后映射、题材簇连通分量|
|数据与 PIT|§4|字段、vol=股、分钟 bar 边界、可用时钟、DG01–DG15|
...
|入场、执行与账本|§13–§16|信号权、流式参与执行、部分成交、拒绝、成本、权重、现金预留、退出|
|机制研究与正式 Gate|§17–§20、附录 B|研究设计、CGM 推断、seed payload、三态法庭、MES|
...
|---|---|---|---|---|---|---|---|---|
|1 发现|共同支持域内 D−1 候选题材日|未来5日 NewJoin AUC（§17.1 求和定义）：真实−null|≤0|0.05 / 0|题材簇×月|固定顺序第1，α=α_v|HOLDOUT一次；FORWARD仅Kill|§18.0；不足→INCONCLUSIVE|
|2 确认|Track S；题材级确认处理 vs §17.1 匹配控制（Control Measurement Cohort 冻结）|Whipsaw率：控制−处理|≤0|0.10 / 0|题材簇×月 CGM|固定顺序第2，α=α_v|仅Gate1 PASS后；HOLDOUT一次|控制不跨配对复用；平衡失败整样本无效|
|3 角色可分离|已确认且 \(L^E/K^E\) 均非空的 episode（episode 等权）|\(\theta^{ROLE}_A,\theta^{ROLE}_C\)|任一≤0|各0.05 / 0|题材簇×月|固定顺序第3；m=2 IUT|仅Gate1–2 PASS后；HOLDOUT一次|两者均PASS才PASS；任一FAIL才FAIL；其余INCONCLUSIVE|
|4 迁移|Gate 3 PASS 域内已确认 episode（episode 等权）|\(\Delta StartRate_5,\Delta CDS_5,\Delta PriceShare_5\)|任一≤0|0.10/0.05/0.05；fail均0|题材簇×月|固定顺序第4；m=3 IUT|仅Gate1–3 PASS后；HOLDOUT一次|三者均PASS才PASS；任一FAIL才FAIL；其余INCONCLUSIVE|
|5 时钟|Gate 4 PASS 域内容量候选（股×episode，聚类吸收依赖）|标准化交互 \(\beta_3\)（\(\widetilde{CCS}\times\widetilde{GCC^{-i}}\)，因变量 GrossAlpha20）|≤0|0.005 / 0（每1σ×1σ 20日毛超额）|题材簇×月|固定顺序第5，α=α_v|仅Gate1–4 PASS后；HOLDOUT一次|§18.0；不足→INCONCLUSIVE|
|6 选择|Track S；同题材同日 \(K^{trade}\ge4\)（episode 等权）|EntryScore前2 − MatchedRandom（§18 Gate 6）机制口径 Alpha20|≤0|1% / 0|题材簇×月 CGM|固定顺序第6，α=α_v|仅Gate1–5 PASS后；HOLDOUT一次|§18.0；\(<4\)→OUT_OF_SUPPORT|
|7 交互|Track S；主信号日 S 的处理-控制配对（§18 Gate 7 独立生成器；配对等权）|\(\Delta_{INT}\)|≤0|1% / 0|题材簇×月 CGM|固定顺序第7，α=α_v|仅Gate1–6 PASS后；HOLDOUT一次|§18.0；不足→INCONCLUSIVE|
...
|9 退出|Track S；外生冻结入场 tape + 隔离资本的配对 NAV（§18 Gate 9）|动态−固定 Sharpe差、累计净超额差|任一≤0|ΔSharpe 0.10、ΔCumExcess 0.02；fail均0|同步21日 circular block bootstrap（seed 见 §20.2.3）|固定顺序第9；m=2 IUT|仅Gate1–8 PASS后；HOLDOUT一次|纯退出增量；非端到端有限资金政策|
...
**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v8 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
|null 生成器、load-or-build、SMD、NG、DAG/SLO|§8.3|§8.3.3–8.3.9|
...
|g、流式参与、部分成交、费用、冲击|§14|§14.1/14.3|
|目标权重、预留、ThemeRoom 归属、账本|§15|§15.2–15.3|
