...
**简称：** SFL-TDCRMC
**版本：** v19.0 Parent-Tax Reconciliation Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v19 Parent-Tax Reconciliation Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为 Parent-Tax Reconciliation Closure（parent-specific gross/tax allocation、PIT PlannedExitDate_arm、post-payment bilateral TaxTrueUp）；不新增题材指标、角色、状态、对照、Gate 或退出腿。已 PASS 的 tax slices、TradingDays、LockGroup 形成、owner collision 及更早闭包维持不变。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v19.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v19.0"）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v19(theme, D, ...)`：
...
**分红税（v19 唯一算法，覆盖 §15.5.2）：** 税率 = `tax_rate(holding_days, board, tax_rule_version)`。holding_days、`PlannedExitDate_arm`、parent 级税额与 `TaxTrueUp`（正 clawback / 负 reversal）见 §15.5.2–15.5.3。
...
### 15.5.1 CorporateActionEntitlementLot（v19 唯一）
...
**重估与 payment / 双向 true-up（v19 唯一）：**

1. **每日开盘前（payment 前）：** 逐 slice 重算 `Tax_(e,s,t)`；档位变化 → `TAX_HOLDING_REMEASURE`；税制变化 → `TAX_RULE_REMEASURE`；
2. **payment 开盘前结算：** 对每个 slice 记
   `SettledEstimatedTax_(e,s) = Tax_(e,s,payment)`；
   `SettledDividendCash_(e,p) = CashReceivable_(e,p) - TaxPayable_(e,p,payment)`（parent 公式见 §15.5.3）；
   清零组合层 CashReceivable / TaxPayable，但**保留**各 slice 的 `SettledEstimatedTax_(e,s)` 直至处置 true-up 完成；
3. **实际处置日（payment 后或前均适用最终真实税）：**
   `FinalActualTax_(e,s) = GrossDividend_(e,s) × tax_rate(TradingDays(acquire_date_s, disposal_date_s), board, tax_rule_version_asof_disposal)`
   （disposal 用 `PREOPEN_ACTION_CUTOFF(disposal_date)` 可知税制）；
   若 payment 时尚未处置，则于**实际 disposal 日**计算：
   `TaxTrueUp_(e,s) = FinalActualTax_(e,s) - SettledEstimatedTax_(e,s)`；
   `TaxTrueUp > 0` → `DIV_TAX_CLAWBACK`；`TaxTrueUp < 0` → `DIV_TAX_REVERSAL`（refund）；`=0` → 无调整；
   parent 级 `TaxTrueUp_p = Σ TaxTrueUp_(e,s)` 进入 §0.3 `TotalNetProceeds`；
4. 若 disposal 发生在 payment **之前**：payment 直接按实际 disposal holding 计税，`TaxTrueUp_(e,s)=0`（无预计偏差）。
...
### 15.5.3 Parent / lot 分摊（v19：税额按 tax-slice parent 归集）
...
**现金股息 / receivable 税额（强制实现 B）：**
    ...
    = CashReceivable_(e,p) - TaxPayable_(e,p,payment)
...
|action_type|账本效果|
|---|---|
|CASH_DIVIDEND|record：LEGAL_ONLY；ex：RECOGNIZED CashReceivable+TaxPayable（parent 分摊见上）；payment：SETTLED 按 SettledDividendCash_(e,p)；保留 SettledEstimatedTax 供 true-up；AvgCost 不变|
...
- **版本 α-wealth：** 正式 PASS 预算按**实际解封 HOLDOUT 的版本序号** k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)。**HOLDOUT 触碰日志（v19 唯一声明）：** v1–v18 均未解封 HOLDOUT，已消耗集合为空；**本 Canonical `SFL-1_v19.0` 为 k=1，\(\alpha_v=0.025\)**。后续任一新 SPEC 若解封 HOLDOUT 必须取下一 k，禁止重置；不得并行登记多个“当前 k=1”；
...
**真实交易日 D 的唯一顺序（v13）：**
    ...
    D 15:00以后（信号日）
      ...
      ├─ load_or_build_nulls_v19；Gate1 冻 DISCOVERY_RESEARCH_* 
...
# 25. 核心伪代码
    ...
    for D in trading_days:
        ...
        update_cash_positions_after_sells(sell_fills)
        assert_ledger_identities()
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v19(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v19(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                ...
                m["nulls"] = load_or_build_nulls_v19(
...
|版本 α-wealth|0.05×2^{-k}（v19: k=1, α_v=0.025；HOLDOUT 从未消耗）|—|治理|—|
...
本规格的 Canonical 纪律：
    ...
    parent 级 CashReceivable/TaxPayable 按 tax-slice 归集；PlannedExitDate_arm PIT；
    TaxTrueUp 双向（clawback/reversal）；α-wealth v19 k=1；路径 /sfl1_v19/。
...
|入场、执行与账本|§13–§16|ExecutionAllocatedBudget、AllInUnitCost、entitlement 三时点、tax-slice parent 税额、PlannedExitDate_arm、TaxTrueUp、odd-lot 真实清算、StockPositionLock|
...
**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v19 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] CashReceivable_(e,p)/TaxPayable_(e,p) 按 tax-slice parent；禁止 trade-lot 均摊税额
- [ ] PlannedExitDate_dynamic：未触发→fixed20 fallback；触发后用已归档日；禁 look-ahead
- [ ] SettledEstimatedTax + FinalActualTax → TaxTrueUp（正 clawback / 负 reversal）
- [ ] 维持：source_tax_lot_alloc；slices；TradingDays；LockGroup；collision→LEDGER_INVALID
- [ ] α-wealth：v19 k=1；路径 /sfl1_v19/；spec_version=SFL-1_v19.0
...
## 执行与治理
- [ ] 账本恒等式零失败
- [ ] Run-in 20日完成，并覆盖黄金路径：Parent A 税率0% / Parent B 税率20% → 净股息 100/80 而非均摊90/90；dynamic 未触发时 planned=fixed20；触发后用归档动态日且不回写触发前税额；payment 按20日计税10元后实际40日税0 → DIV_TAX_REVERSAL +10 入 TotalNetProceeds；payment 后短卖导致追加税 → DIV_TAX_CLAWBACK；禁 trade-lot 均摊税额静态检查
