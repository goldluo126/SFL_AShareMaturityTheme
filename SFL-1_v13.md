...
**简称：** SFL-TDCRMC
**版本：** v13.0 Identity–Ledger Final Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v13 Identity–Ledger Final Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为 Identity–Ledger Final Closure（确认事实、P10 稳定伪 episode、母订单预算、公司行动 entitlement、odd-lot、stock-order lock、Gate9 配对退出、伪代码时钟、版本 α/路径）；不新增题材指标、角色、状态、对照、Gate 或退出腿。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v13.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v13.0"）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v13(theme, D, ...)`：
...
2. 每个执行日 09:35：`CommissionReserve = minimum_commission_cny`；
   `IntradaySpendableBudget = LifecycleRemainingBudget - CommissionReserve`（若 ≤0 则当日不可买，登记 `BUDGET_RESERVE_BLOCK`）；
3. **预留不侵蚀生命周期预算：** 无成交日终释放 Reserve 回 FreeCash 时，`LifecycleRemainingBudget` **不变**；
4. 分钟推进只扣 `IntradaySpendableBudget`（比例佣金+税费+冲击执行价）；
5. 日终：`DayOrderValue` = 当日成交执行价名义之和；`DayCommission = max(DayOrderValue×commission_rate, minimum_commission_cny if DayOrderValue>0 else 0)`；
   无成交则 `DayCommission = 0`（不收最低佣金）；
   `CumLifecycleNetBuyCost +=` 当日净买入成本（含最终 DayCommission）；
   `LifecycleRemainingBudget = ParentOrderTargetValue - CumLifecycleNetBuyCost`；
6. **CommissionTailAdjust（v13）：** 若补扣佣金后 `CumLifecycleNetBuyCost > ParentOrderTargetValue`，**仅**减少**最后一分钟** `fill_qty`（100 股步进，可至 0），并**仅对该分钟**用新数量重算一次 TentativeValue → ImpactBps → \(P^{ex}\) → 费用；**禁止**回改更早分钟；仍超限则登记工程失败；
7. 跨日重试：继承 `LifecycleRemainingBudget` / `CumLifecycleNetBuyCost`；次日重新从真实剩余预算预留 CommissionReserve；
8. 预算闭包：生命周期净买入成本 \(\le ParentOrderTargetValue\)。

每分钟 m（顺序冲击 + 单次计入）：

1. 若 \(Low_m=up\_limit\) → 可参与=0；否则 `particip_qty = floor(0.05×vol_m/100)×100`；
2. **TentativeValue 唯一公式（v13）：**
   `qty0 = min(particip_qty, floor(IntradaySpendableBudget / VWAP_m / 100)×100)`（VWAP=0 则不成交）；
   `TentativeValue_m = qty0 × VWAP_m`；
3. `CumOrderValue_{m-}` = 当日至上一分钟累计执行价名义；`OrderValue_m^{impact}=CumOrderValue_{m-}+TentativeValue_m`；
4. `ImpactBps_m` 按 §14.4；买入 \(P^{ex}_m=VWAP_m×(1+ImpactBps_m/10^4)\)；
5. `max_qty = floor(IntradaySpendableBudget / P^{ex}_m / 100)×100`；`fill_qty=min(max_qty, particip_qty)`；
6. 成交记账价=\(P^{ex}_m\)；再加比例佣金与法定税费；**Impact Single-Count**（冲击只进 \(P^{ex}\)）；
7. `IntradaySpendableBudget -=` 该分钟净成本；不得为负；
8. 至 10:00 或 IntradaySpendableBudget < 一手；全部分钟可参与0 → UNFILLED_LIMIT；
9. 部分完成 → PARTIALLY_FILLED / EXPIRED_REMAINDER，跨日继承生命周期预算；
10. 有效期尽 → 余额作废。
...
- **StockOrderLock（v13）：** 任一股票任一时点最多一个开放买入母订单。同日或多题材竞争时按 §15.1 全局排序，最优信号取得 lock 并创建唯一 ParentOrder；其余信号登记 `REJECT_DUPLICATE_STOCK`，**不**创建 ReservedCash，**不**消耗其 episode 入场权（有效期内若 lock 释放仍可重试；有效期尽则作废并消耗）。跨日：已有重试母订单占用 lock 时，新信号同样 `REJECT_DUPLICATE_STOCK`；
- 组合已满时后续信号进入有效期重试，不跨有效期排队。

**题材簇上限时点语义（v13 唯一）：** ThemeRoom 在**入场分配时**按当日 §5.3 连通分量计算（entry-time constraint），并按 origin 永久归属。若持仓后因连通分量动态合并导致某 persistent/当日簇合计暴露 >20%：
...
每位证券持仓子账必须含：`SettledShares`、`UnsettledBuyShares`、`SellableQty`、`PendingSellReserved`、`AvgCost`、`trade_lots`、FIFO `tax_lots`、`entitlements`。母订单必须含：`ParentOrderTargetValue`、`LifecycleRemainingBudget`、`IntradaySpendableBudget`、`CommissionReserve`、`CumLifecycleNetBuyCost`、`signal_id`、`parent_order_id`。

### 15.3.1 Lot 会计唯一口径（v13）
...
**现金分红与经济成本（v13）：** 组合 `AvgCost` **不因** CASH_DIVIDEND 改变。record date 创建 entitlement（§15.5.1）；payment date 增加税后现金。税务成本基础若制度要求调整，**只**写入 `tax_lots.cost_basis`，公式按 `tax_rule_version` 冻结表，不得暗改 AvgCost。
...
**零碎股与最终清算（v13）：** 送转/拆合后允许非 100 股整数记账；常规卖出对 100 向下取整。若某退出腿要求清仓且 `SellableQty < 100` 且 >0：执行 `FINAL_ODD_LOT_LIQUIDATION`——允许卖出全部剩余股数（不受 100 股取整约束）；若连续 5 个交易日因跌停/停牌无法卖出，于第 5 日收盘按 `min(close, down_limit)` 现金清算并登记 `ODD_LOT_CASH_EXIT`。禁止零碎股使 episode 永久无法 CLOSE。

每次转移保存时间、原因、前后现金、预留、仓位、可卖库存与槽位快照哈希。

## 15.5 公司行动与证券身份账本（v13）
...
### 15.5.1 CorporateActionEntitlementLot（v13 唯一）

在 `record_date` 收盘后（`available_at` 已满足）按**当时有权持仓**冻结不可变应收权益，字段：

    entitlement_id, security_id, action_type, record_date,
    entitled_shares, source_trade_lot_ids,
    receivable_cash_gross, receivable_stock, rights_receivable,
    payment_or_effective_date, tax_basis_note

规则：

1. **权益由 record_date 持仓决定**，不由 payment/effective 日当前持仓决定；
2. record_date 持有、其后卖出：仍保留 entitlement，payment 日结算；
3. record_date 后买入：不产生该次 entitlement；
4. payment/effective 日**只结算已冻结 entitlement**，禁止重新扫描当前 tax_lots 决定谁有权；
5. NAV：`CashReceivable` / `StockReceivable` 按事实表估值计入，直至结算。

**生效时钟：**

1. 股数类行动：对已冻结 stock receivable，在 `max(ex_date 或 effective_date, available_at 下一交易日开盘)` 开盘前增加股数并调整每股成本使**总成本不变**（AvgCost 总额不变、股数变）；
2. 现金分红：`AvgCost` 不因除息改变；`payment_date` 开盘前 `Cash +=` entitlement 税后现金；无 `payment_date` → `INVALID_DATA`；
3. 迟到数据于 `available_at` 后首个交易日开盘前补建 entitlement / 补结算，登记 `CORP_ACTION_LATE`。
...
- **版本 α-wealth：** 正式 PASS 预算按**实际解封 HOLDOUT 的版本序号** k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)。**HOLDOUT 触碰日志（v13 唯一声明）：** v1–v12 均未解封 HOLDOUT，已消耗集合为空；**本 Canonical `SFL-1_v13.0` 为 k=1，\(\alpha_v=0.025\)**。后续任一新 SPEC 若解封 HOLDOUT 必须取下一 k，禁止重置；不得并行登记多个“当前 k=1”；
...
- **Outcome（隔离政策账户资本口径）：**
  `PolicyReturn(s) = RealizedNetPnL(s) / ParentOrderTargetValue(s)`，
  其中 RealizedNetPnL 含部分成交与公司行动现金流；未成交则 PnL=0 ⇒ PolicyReturn=0；
  等价于 `FillFraction_s × ReturnOnFilledCapital_s`（未成交现金 0 收益）；
  **禁止**把部分成交上的 NetAlpha20 直接当作 PolicyReturn；
- 正式汇总：`PolicyNetAlpha20 =` 信号等权均值；与 ParentFillRate 同列首页（§20.6）；
...
**科学问题选择（v13 唯一）：parent-order 配对退出增量。** 不构造共享有限现金组合，不回答端到端资金政策。

**外生冻结入场 tape（行粒度同前）：**

1. Tape 来源：隔离研究账户上固定20日入口政策的生产入场成交序列；哈希冻结后方可开庭；
2. 行字段：`signal_id, parent_order_id, attempt_date, fill_date, stock, fill_qty, fill_net_cost, origin_episode_id, original_target_value, ...`；
3. **配对单元：** 每个 `parent_order_id` 一条观测；
4. **Outcome：**
   `ExitInc_p = R^{net,dynamic}_p - R^{net,fixed20}_p`，
   两侧均从该 parent 的实际成交净成本出发、共享同一买入 tape 腿，仅退出规则不同；T+1 与费用同构；
5. 正式共同主 estimand：
   - \(\theta_{ExitMean}\) = parent 等权平均 `ExitInc`（MES_pass=0.01，MES_fail=0）；
   - \(\theta_{ExitComp}\) = 按 persistent overlap component 聚合后的平均 `ExitInc`（MES_pass=0.01）；
   二者 IUT（m=2）；
6. **禁止**再用 `DeployedCapitalBase = max ConcurrentGrossNotional` 作为强制同 tape 组合初始现金（该口径不能保证亏损后接受未来 tape）；该量仅诊断；
7. **禁止**隐含融资或路径相关注资而不改用 TWR；若报告端到端对照，标记 `DIAGNOSTIC_E2E_POLICY` 并允许 capital calls + TWR，不进 Gate 9 主终局。

**推断装置：** parent 层观测；依赖按 §20.2（ID1=persistent component，ID2=parent 首笔 fill 月）；稳健性用 persistent-component 整块 bootstrap。
...
### P10_NULL_EPISODE（v13 Stable Episode Generator）
...
**稳定运行（v13 核心，取代日度 Jaccard continuation）：**
...
年度覆盖（v13 公式）：令 \(N^{real}_y\) = 年 y 真实可评估题材日数；\(N^{epi}_y\) = `discovered_at∈y` 且 `P10_EVALUABLE_EPI=1` 的伪 episode 数；\(CoverDays_y\) = 年 y 中至少有一个可评估伪 episode 覆盖的交易日数。若

    N^{epi}_y < N^{real}_y × 40
    或 CoverDays_y < 0.90 × TradingDays_y

则 `P10_INSUFFICIENT_YEAR`（40 = 每真实评估日最低可评估伪 episode 数，= 200 controls × 最低存活率先验 0.20，冻结于 §27）。达标前不得报告 AnnualFalseEpisodes、不得用于 Phase-I/漂移。
...
**真实交易日 D 的唯一顺序（v13）：**
    ...
    D 09:35—10:00
      ├─ 执行由 D−1（及有效期内更早）生成的买单（连续封死拒单、StockOrderLock、预算拆分）
    ...
    D 15:00以后（信号日）
      ...
      ├─ load_or_build_nulls_v13；Gate1 冻 DISCOVERY_RESEARCH_* 
      ...
      ├─ 代表生成信号；StockOrderLock 竞争；预留 LifecycleRemainingBudget
...
# 25. 核心伪代码
    ...
    for D in trading_days:
        ...
        reject_and_release_reserves(buy_orders)  # gap/limit/StockOrderLock
        set_commission_reserves_from_lifecycle_budget(buy_orders)
        reallocate_cash_at_0935(eligible_buy_orders, PARAMS)
        buy_fills = execute_streaming_participation_sequential_impact(buy_orders)
        settle_day_commission_tail_adjust_last_minute_only(buy_fills)
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v13(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v13(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                ...
                m["nulls"] = load_or_build_nulls_v13(
        ...
        # StockOrderLock：全局排序后唯一母订单
        signals = select_signals_with_stock_order_lock(signal_candidates, PARAMS)
...
|P10 年伪episode/真实评估日下限|40|个/日|工程闸门（=200×0.20）|—|
...
|版本 α-wealth|0.05×2^{-k}（v13: k=1, α_v=0.025；HOLDOUT 从未消耗）|—|治理|—|
...
本规格的 Canonical 纪律：
    ...
    ConfirmedEventFlag 为吸收事实；EARLY_CAPACITY 不得重判确认斜率；
    主终点两侧按实际成交净成本同构；Gate8B = PnL/ParentTarget；
    P10 spawn 后 membership/fold 冻结运行；FalseSignalRate 仅 evaluable；
    LifecycleRemainingBudget ≠ CommissionReserve；record-date entitlement；
    AvgCost=旧成本移动平均；odd-lot 最终可清算；StockOrderLock；
    Gate9 = parent-order 配对退出增量；伪代码按真实日内时钟；
    α-wealth：v13 k=1（HOLDOUT 从未消耗）；路径 /sfl1_v13/。
...
|入场、执行与账本|§13–§16|预算拆分、TentativeValue、entitlement、odd-lot、StockOrderLock、真实日内时钟|
...
|9 退出|Track S；固定买入 tape；parent-order 配对|parent 首笔 fill 月|ExitInc 均值与 component 聚合均值|任一≤0|各0.01 / 0|persistent簇×月 CGM + component bootstrap|固定顺序第9；m=2 IUT|仅Gate1–8 PASS后；HOLDOUT一次|禁止共享现金组合主口径；PASS 仍需动态 §20.5(2–8)|
...
**index_date 规则（v13）：** `ID2 = YYYY-MM(index_date)`，必须回引本表，不得另选。

**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v13 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
|目标权重、StockOrderLock、ThemeRoom、T+1、trade/tax/entitlement lot、AvgCost|§15|§15.0/15.2–15.5|
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] ConfirmedEventFlag；EARLY_CAPACITY 不重判确认斜率；DISCOVERY_RESEARCH_COHORT_D
- [ ] P10：spawn 后 membership/fold 冻结；ID 含 control_index；FSR 仅 evaluable；年度覆盖公式
- [ ] 对照/真实收益按实际成交净成本同构；Gate8B=PnL/ParentTarget；ParentFillRate
- [ ] LifecycleRemainingBudget ≠ IntradaySpendableBudget；TentativeValue 公式；TailAdjust 仅最后一分钟
- [ ] record-date entitlement；AvgCost=旧成本移动平均；odd-lot 最终清算；StockOrderLock
- [ ] Gate9 parent-order 配对 ExitInc；伪代码：开盘前→09:35买→14:30卖→收盘后信号；确认日重载 EPISODE_NULL
- [ ] α-wealth：v13 k=1；路径 /sfl1_v13/；spec_version=SFL-1_v13.0
...
## 执行与治理
- [ ] 账本恒等式零失败
- [ ] Run-in 20日完成，并覆盖黄金路径：开盘涨停后开板；无成交三日 CommissionReserve 不侵蚀母目标；多日部分成交佣金；record-date 持有/payment 前卖出仍收息；record 后买入不收旧息；odd-lot 最终关闭 episode；同股双题材 StockOrderLock；确认日 DAILY→EPISODE null 切换；伪代码卖出与 T+1；P10 连续5日稳定 membership；ParentFillRate；Gate9 parent 配对；隔离 no-gap；ThemeRoom 不迁移
