...
**简称：** SFL-TDCRMC
**版本：** v17.0 Tax-Lot–Selection Final Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v17 Tax-Lot–Selection Final Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为 Tax-Lot–Selection Final Closure（per-tax-lot expected_tax、PREOPEN_ACTION_CUTOFF、SettledCorporateActionCash 边界、对称配对 unresolved guard、successor StockPositionLock 迁移）；不新增题材指标、角色、状态、对照、Gate 或退出腿。已 PASS 的 P10 episode-day、ExecutionAllocatedBudget 顺序、zero-target Intent、Gate9 cutoff 维持不变。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v17.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v17.0"）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v17(theme, D, ...)`：
...
- **StockPositionLock（v17）：** 只要证券（或其 `SecurityIdentityLockGroup` 内任一 alias）满足任一：
  ...
  **CODE_CHANGE / ABSORPTION（原子迁移）：** effective 开盘前（`PREOPEN_ACTION_CUTOFF`）将 predecessor 的持仓、entitlement、成本、PendingSell、**StockPositionLock 与全部 owner 字段**一次性迁移至 `successor_security_id`；迁移完成前 predecessor 与 successor 为同一 LockGroup。
...
|`tax_lots`|分红税持有期限与税务成本基础|FIFO；字段 `{lot_id, shares, cost_basis, acquire_date, settle_state}`|
...
**分红税（v17 唯一算法，覆盖 §15.5.2）：** 税率 = `tax_rate(holding_days, board, tax_rule_version)`。holding_days 与重估见 §15.5.2；仅当税表要求**额外**卖出追缴且该金额尚未计入 `TaxPayable` 时登记 `DIV_TAX_CLAWBACK`。
...
### 15.5.1 CorporateActionEntitlementLot（v17 唯一）
...
### 15.5.2 Receivable 估值与税（v17 唯一）

**现金股息税口径：**

    CashReceivable = receivable_cash_gross
    TaxPayable     = expected_tax   # 必须由下款逐 tax-lot 求和

**禁止**对 CashReceivable 再做 `(1-tax)` 折现同时又记 TaxPayable。

**逐 tax-lot `expected_tax`（强制；禁止最早日/加权平均日/parent 平均日）：**

对 entitlement e 的每个 source tax lot \(l\)：

    GrossDividend_(e,l)
    = receivable_cash_gross_e × entitled_shares_(e,l) / total_entitled_shares_e

    TaxHoldingEndDate_l(t)
    = min(t, actual_disposal_date_l)   # 未卖出则 actual_disposal_date_l = +∞

    holding_days_as_of_t,l
    = TradingDays(acquire_date_l, TaxHoldingEndDate_l(t))

    HoldingDaysToFixed20Exit_l
    = TradingDays(acquire_date_l, planned_fixed20_exit_signal_date)

    ExpectedHoldingDays_(e,l,t)
    = max(holding_days_as_of_t,l, HoldingDaysToFixed20Exit_l)
      # 若 lot 已卖出：holding_days_as_of_t,l 冻结在 disposal 日，不得继续增长至 payment

    TaxPayable_(e,t)
    = Σ_l GrossDividend_(e,l) × tax_rate(ExpectedHoldingDays_(e,l,t), board, tax_rule_version_asof_t)

**禁止** `tax_rate(加权平均持有日)` 替代 `Σ 加权税率`。
...
1. **每日开盘前（payment 前）：** 按上式逐 lot 重算 `TaxPayable`；持有档位变化 → `TAX_HOLDING_REMEASURE`；税制版本变化 → `TAX_RULE_REMEASURE`；只向前调整；
2. **payment 开盘前结算：** 用 `PREOPEN_ACTION_CUTOFF(payment_date)` 可知税制与冻结后的各 lot holding days 计算 `TaxPayable_payment`；税后现金 = `CashReceivable - TaxPayable_payment`；清零 receivable 与 TaxPayable；
3. **`DIV_TAX_CLAWBACK`：** 仅结算后税表要求的**额外**追缴，且不得重复扣已结算 TaxPayable。

|类型|RECOGNIZED 起估值|
|---|---|
|CashReceivable|`receivable_cash_gross`（毛额）；配套逐 lot `TaxPayable`|
...
|CODE_CHANGE / ABSORPTION|开盘前映射 successor：持仓、entitlement、成本、**StockPositionLock 与 lock_owner_*** 原子迁移至 `successor_security_id`；迁移完成前 predecessor/successor 属同一 `SecurityIdentityLockGroup`（任一 alias 有持仓/订单/share-entitlement 均阻止新 Parent）；见 §15.0|
...
- **版本 α-wealth：** 正式 PASS 预算按**实际解封 HOLDOUT 的版本序号** k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)。**HOLDOUT 触碰日志（v17 唯一声明）：** v1–v16 均未解封 HOLDOUT，已消耗集合为空；**本 Canonical `SFL-1_v17.0` 为 k=1，\(\alpha_v=0.025\)**。后续任一新 SPEC 若解封 HOLDOUT 必须取下一 k，禁止重置；不得并行登记多个“当前 k=1”；
...
**真实交易日 D 的唯一顺序（v13）：**
    ...
    D 15:00以后（信号日）
      ...
      ├─ load_or_build_nulls_v17；Gate1 冻 DISCOVERY_RESEARCH_* 
    ...
    for D in trading_days:
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v17(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v17(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                ...
                m["nulls"] = load_or_build_nulls_v17(
...
|版本 α-wealth|0.05×2^{-k}（v17: k=1, α_v=0.025；HOLDOUT 从未消耗）|—|治理|—|
...
本规格的 Canonical 纪律：
    ...
    per-tax-lot TaxPayable；卖出后 holding 冻结；PREOPEN_ACTION_CUTOFF；
    SellNetProceeds vs SettledCorporateActionCash 无双计；successor LockGroup 原子迁移；
    对称配对 CI(ΔU)⊂[±0.05]；绝对 unresolved≤0.20；α-wealth v17 k=1；路径 /sfl1_v17/。
...
**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v17 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] per-tax-lot TaxPayable；disposal 冻结 holding；PREOPEN_ACTION_CUTOFF 用于开盘前税制
- [ ] SellNetProceeds 含全部股份卖出；SettledCorporateActionCash 仅非市场现金流
- [ ] successor StockPositionLock/owner 原子迁移；SecurityIdentityLockGroup
- [ ] Gate9：Z_p 配对；CGM CI(ΔU)⊂[±tol]；绝对 unresolved≤0.20→否则 INCONCLUSIVE
- [ ] 维持：P10 R_y；09:35 预算顺序；零金额 Intent=0；G9_OUTCOME_CUTOFF；AllIn provisional/final
- [ ] α-wealth：v17 k=1；路径 /sfl1_v17/；spec_version=SFL-1_v17.0
...
## 执行与治理
- [ ] 账本恒等式零失败
- [ ] Run-in 20日完成，并覆盖黄金路径：两 tax-lot 不同持有期 → TaxPayable=逐 lot 税率之和≠平均日税率；D15 卖出后 holding 不增至 payment；ex 开盘前税制只用 PREOPEN_ACTION_CUTOFF；stock entitlement 股份卖出只进 SellNetProceeds；absorption 后 successor 上新信号被 LockGroup 拒绝；dynamic unresolved 高于 fixed 时 CI 越界 → Gate9 INCONCLUSIVE；|ΔU| 两方向均受 ±0.05 约束
