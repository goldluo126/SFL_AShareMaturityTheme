...
**简称：** SFL-TDCRMC
**版本：** v18.0 Tax-Slice–Identity Collision Closure
**状态：** `FREEZE-CANDIDATE`；规范文本已自包含（v18 Tax-Slice–Identity Collision Closure）。仅在 §4 数据闸门、§8.3 生成器验收（NG01–NG07）、§17 机制研究闸门、§20.2 正式推断算法验收、§20.4-bis 可行性核查、§21.1 分区清单物化与附录 C 全部通过后方可冻结
...
**修订范围纪律：** 本版本严格限定为 Tax-Slice–Identity Collision Closure（entitlement→tax-lot allocation、partial-disposal tax slices、actual-disposal 优先于计划退出、TradingDays 算子、LockGroup 形成时点、owner collision 政策）；不新增题材指标、角色、状态、对照、Gate 或退出腿。已 PASS 的 PREOPEN_ACTION_CUTOFF、SettledCorporateActionCash 边界、对称 unresolved guard 及更早闭包维持不变。

## Canonical 与自包含声明

1. **唯一规范源：** 本文件是 SFL-1 v18.0 的唯一规范性来源。实现、回测、验收、影子运行和正式判词不得引用任何早期版本、审查报告、变更日志或口头解释补全规则。
...
- **总回报（v17 唯一，Gate 8A/8B/9 共用）：**
  \(BuyNetCost\) = 生命周期实际净买入成本（冲击单次计入）；
  **`UnsettledEntitlementTerminalValue_p`（仅 RECOGNIZED，净额唯一公式）：**
  \[
  \sum_{e\in\mathrm{CashRecog}_p}(\mathrm{CashReceivable}_e-\mathrm{TaxPayable}_e)
  +\sum_{e\in\mathrm{StockRecog}_p}\mathrm{StockReceivableValue}_e
  +\sum_{e\in\mathrm{RightsRecog}_p}\mathrm{RightsReceivableValue}_e
  \]
  ...
  **科目边界（强制，防双计）：**
  ...
  `SettledCorporateActionCash_p` = **仅**非市场卖出现金流：现金股息、退市现金、cash-in-lieu、其他 PIT 法定现金结算；
  **禁止**将 SETTLED 股票权益股份的市场卖出款再次计入 `SettledCorporateActionCash`；
  \(TotalNetProceeds_p = SellNetProceeds_p + SettledCorporateActionCash_p + UnsettledEntitlementTerminalValue_p - DIV\_TAX\_CLAWBACK_p\)；
  **`DIV_TAX_CLAWBACK`** 仅减尚未计入 `TaxPayable` 的后续追缴；禁止与 TaxPayable 双减；
...
全文任何 \(Hash(f_1,f_2,\dots,f_n)\) 形式的种子或分片函数均按以下唯一协议实现：
    ...
    其中：日期序列化为 YYYYMMDD；整数为十进制无前导零；
          字符串原样；spec_version 为完整版本串（如 "SFL-1_v18.0"）
...
## 6.10 TradingDays 持有期算子（全文唯一；v18）
...
4. **load-or-build 协议（v8，唯一）：** 伪代码与生产入口函数必须实现为 `load_or_build_nulls_v18(theme, D, ...)`：
...
**分红税（v18 唯一算法，覆盖 §15.5.2）：** 税率 = `tax_rate(holding_days, board, tax_rule_version)`。holding_days 与重估见 §15.5.2；仅当税表要求**额外**卖出追缴且该金额尚未计入 `TaxPayable` 时登记 `DIV_TAX_CLAWBACK`。
...
### 15.5.1 CorporateActionEntitlementLot（v18 唯一）
...
对 entitlement e 的每个 slice s（shares>0）：
    ...
    if disposal_date_s is not null:   # 已实际卖出部分
        ExpectedHoldingDays_(e,s,t)
        = TradingDays(acquire_date_s, disposal_date_s)
        # 实际卖出优先；禁止再用 planned fixed20 / 计划退出日覆盖
    else:                             # 尚未卖出部分
        holding_days_as_of_t,s = TradingDays(acquire_date_s, t)
        HoldingDaysToPlannedExit_s
        = TradingDays(acquire_date_s, planned_exit_signal_date_of_arm)
        # Gate 9：dynamic arm 与 fixed20 arm 各自使用本臂计划退出日；
        # 生产组合默认用固定20日入口验证退出信号日
        ExpectedHoldingDays_(e,s,t)
        = max(holding_days_as_of_t,s, HoldingDaysToPlannedExit_s)

    TaxPayable_(e,t)
    = Σ_s GrossDividend_(e,s) × tax_rate(ExpectedHoldingDays_(e,s,t), board, tax_rule_version_asof_t)
...
1. **每日开盘前（payment 前）：** 按上式逐 slice 重算 `TaxPayable`；档位变化 → `TAX_HOLDING_REMEASURE`；税制变化 → `TAX_RULE_REMEASURE`；只向前调整；
2. **payment 开盘前结算：** 用 `PREOPEN_ACTION_CUTOFF(payment_date)` 可知税制与各 slice 的 ExpectedHoldingDays 计算 `TaxPayable_payment`；税后现金 = `CashReceivable - TaxPayable_payment`；清零 receivable 与 TaxPayable；
3. **`DIV_TAX_CLAWBACK`：** 仅结算后税表要求的额外追缴，且不得重复扣已结算 TaxPayable。

|类型|RECOGNIZED 起估值|
|---|---|
|CashReceivable|`receivable_cash_gross`（毛额）；配套逐 slice `TaxPayable`|
...
### 15.5.3 Parent / lot 分摊

公司行动现金流与 receivable 终值按 `source_trade_lot_ids` 的股数比例分摊至各 `parent_order_id`，供 §0.3 `TotalNetProceeds` 使用。

|action_type|账本效果|
|---|---|
|CASH_DIVIDEND|record：LEGAL_ONLY；ex：RECOGNIZED CashReceivable+TaxPayable；payment：SETTLED 税后现金，清零 receivable/TaxPayable；AvgCost 不变|
...
结算税后现金 = `receivable_cash_gross - TaxPayable`（以结算日 TaxPayable 余额为准）。

DG16：漏建 entitlement、漏结算或破坏 NAV → `LEDGER_INVALID`。
...
- **版本 α-wealth：** 正式 PASS 预算按**实际解封 HOLDOUT 的版本序号** k 分配：\(\alpha_v(k)=0.05\times2^{-k}\)。**HOLDOUT 触碰日志（v18 唯一声明）：** v1–v17 均未解封 HOLDOUT，已消耗集合为空；**本 Canonical `SFL-1_v18.0` 为 k=1，\(\alpha_v=0.025\)**。后续任一新 SPEC 若解封 HOLDOUT 必须取下一 k，禁止重置；不得并行登记多个“当前 k=1”；
...
**真实交易日 D 的唯一顺序（v13）：**
    ...
    D 15:00以后（信号日）
      ...
      ├─ load_or_build_nulls_v18；Gate1 冻 DISCOVERY_RESEARCH_* 
    ...
    for D in trading_days:
        ...
        for theme in themes:
            ...
            if episode is None:
                ...
                nulls = load_or_build_nulls_v18(theme, D, mode="DAILY_NULL",
                                              donor_index=donor_index)
            else:
                base = episode.B_E
                leaders = episode.L_E
                capacity = episode.K_E
                nulls = load_or_build_nulls_v18(theme, D, mode="EPISODE_NULL",
                                              donor_index=donor_index)
        ...
        for theme, m in theme_metrics.items():
            ...
            if role_cohort.just_frozen:
                ...
                m["nulls"] = load_or_build_nulls_v18(
...
|版本 α-wealth|0.05×2^{-k}（v18: k=1, α_v=0.025；HOLDOUT 从未消耗）|—|治理|—|
...
本规格的 Canonical 纪律：
    ...
    source_tax_lot_alloc 冻结；tax slice 部分卖出拆分；实际卖出优先于计划退出；
    TradingDays(a,b)=|{a<d≤b}|；LockGroupFormationPreOpen；owner collision→LEDGER_INVALID；
    α-wealth v18 k=1；路径 /sfl1_v18/。
...
|入场、执行与账本|§13–§16|ExecutionAllocatedBudget、AllInUnitCost、entitlement 三时点、odd-lot 真实清算、StockPositionLock|
...
**MES 说明：** 以上为预注册工程/经济最小效应，不是市场自然常数。修改即新版本。复合 Gate 使用 §18.0 IUT，核心9 Gate 使用固定顺序 gatekeeping（每 Gate α=α_v，v18 为 0.025）；Gate S 才使用 Holm。所有效果量输出双侧95% CI，并输出正式单侧 \(1-\alpha_v\) 下界与反向 false-death 预算下的 Bonferroni 同时上界（\(1-0.05/(9m)\)）。
...
## 定义与实现
- [ ] A 类阻断 = 0；B 类阻断 = 0
- [ ] source_tax_lot_alloc；disposed/remaining tax slices；实际卖出优先于 planned exit
- [ ] TradingDays(a,b) 端点唯一；Gate9 两臂各自 disposal / planned exit
- [ ] LockGroupFormationPreOpen；CORP_ACTION_LOCK_COLLISION→LEDGER_INVALID
- [ ] 维持：PREOPEN_ACTION_CUTOFF；SellNet/SettledCash 分账；对称 CI(ΔU)；P10/预算/零Intent/cutoff
- [ ] α-wealth：v18 k=1；路径 /sfl1_v18/；spec_version=SFL-1_v18.0
...
## 执行与治理
- [ ] 账本恒等式零失败
- [ ] Run-in 20日完成，并覆盖黄金路径：record 冻结 source_tax_lot_alloc 后部分卖出不改写 alloc；D10/D30 两次部分卖出拆成三 slices；D15 实际卖出后 ExpectedHoldingDays=15 而非 max(15,20)；TradingDays 端点临界档位测试；公司行动 available 后、effective 前 successor 新信号被拒；predecessor 与 successor 已有不同 owner → LEDGER_INVALID；无碰撞 CODE_CHANGE 锁与 owner 原子迁移
